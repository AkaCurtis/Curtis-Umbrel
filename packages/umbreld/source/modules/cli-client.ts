...
const trpcEndpoint =
  process.env.UMBREL_TRPC_ENDPOINT ?? `http://localhost/trpc`;

async function signJwt() {
  const secret = await fse.readFile(`${dataDir}/secrets/jwt`, {
    encoding: "utf8",
  });
...
const trpc = createTRPCClient<AppRouter>({
  links: [
    splitLink({
      condition: (operation) =>
        httpOnlyPaths.includes(
          operation.path as (typeof httpOnlyPaths)[number],
        ),
      true: httpLink({
        url: trpcEndpoint,
        headers: async () => ({
      ...
      false: wsLink({
        client: createWSClient({
          url: async () => `${trpcEndpoint}?token=${await signJwt()}`,
        }),
      }),
    }),
  ],
