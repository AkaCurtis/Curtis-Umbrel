...
describe(`backupProgress()`, () => {
  ...
  test("returns backup progress during backup operation", async () => {
    ...
    const removeListener = umbreld.instance.eventBus.on(
      ...
      (progress) =>
        void collectedEvents.push(JSON.parse(JSON.stringify(progress))),
    );
    ...
    // Start the backup
    const backupPromise = umbreld.client.backups.backup.mutate({
      repositoryId,
    });
  ...
  test.skip("handles multiple concurrent backups", async () => {
    ...
    const backup1Promise = umbreld.client.backups.backup.mutate({
      repositoryId: repositoryId1,
    });
    const backup2Promise = umbreld.client.backups.backup.mutate({
    ...
    const progressInProgress =
      await umbreld.client.backups.backupProgress.query();
    expect(progressInProgress).toHaveLength(2);

    const repo1Progress = progressInProgress.find(
      (p) => p.repositoryId === repositoryId1,
    );
    const repo2Progress = progressInProgress.find(
      (p) => p.repositoryId === repositoryId2,
    );
