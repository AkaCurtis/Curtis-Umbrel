...
export default class Backups {
  ...
  constructor(umbreld: Umbreld) {
    ...
    this.internalMountPath = nodePath.join(
  ...
  async start() {
    ...
    await this.unmountAll().catch((error) =>
      this.logger.error("Error unmounting backups", error),
    );
  ...
  async stop() {
    ...
    for (const process of this.runningKopiaProcesses)
      process.kill("SIGTERM", { forceKillAfterTimeout: ONE_SECOND * 3 });
  ...
  async backupOnInterval() {
    ...
    while (this.running) {
      ...
      const shouldRun =
        ...
        Date.now() - lastRun >= this.backupInterval;
      ...
      for (const repository of repositories) {
        ...
        const isAlreadyBackingUp = this.backupsInProgress.some(
          (progress) => progress.repositoryId === repository.id,
        );
        ...
        const hoursSinceLastBackup =
          (Date.now() - (lastBackup || this.startedAt!)) / (1000 * 60 * 60);
        if (hoursSinceLastBackup > 24) {
          this.logger.error(
            `Backup for ${repository.path} has not run in over 24 hours`,
          );
          await this.#umbreld.notifications
            .add(`backups-failing:${repository.id}`)
            .catch(() => {});
        }
      }
  ...
  async kopia(
    flags: string[] = [],
    {
      onOutput,
      bypassQueue = true,
    }: { onOutput?: (output: string) => void; bypassQueue?: boolean } = {},
  ) {
    // Refuse to spawn new kopia processes if we're shutting down
    if (!this.running)
      throw new Error("[shutting-down] Refusing to spawn new kopia processes");
    ...
    const spawnKopiaProcess = async () => {
      ...
      process
        .finally(
          () =>
            (this.runningKopiaProcesses = this.runningKopiaProcesses.filter(
              (p) => p !== process,
            )),
        )
    ...
    return bypassQueue
      ? spawnKopiaProcess()
      : this.kopiaQueue.add(spawnKopiaProcess);
  }
  ...
  async addRepository(virtualPath: string, password: string, createNew = true) {
    ...
    const systemPath = await this.#umbreld.files
      .virtualToSystemPath(virtualPath)
      .catch(() => "");
    const isNetworkPath = systemPath.startsWith(
      this.#umbreld.files.getBaseDirectory("/Network"),
    );
    const isExternalPath = systemPath.startsWith(
      this.#umbreld.files.getBaseDirectory("/External"),
    );
    if (!isNetworkPath && !isExternalPath)
    ...
    if (createNew) {
      ...
      await fse.mkdir(systemPath, { recursive: false }).catch((error) => {
        if (error.code === "EEXIST")
          throw new Error(`Repository already exists at ${virtualPath}`);
        throw error;
      });
    ...
    await this.#umbreld.store.getWriteLock(async ({ set }) => {
      ...
      const repositoryExists = repositories.some(
        (existingRepository) => existingRepository.id === id,
      );
      if (!repositoryExists)
        repositories.push({ id, path: virtualPath, password });
    ...
    await this.connect(id).catch(async (error) => {
      ...
      if (isConnectingToExistingRepository)
        await this.forgetRepository(id).catch(() => {});
      throw error;
    });
  ...
  async restoreBackup(backupId: string) {
    if (this.restoreStatus.running)
      throw new Error("[in-progress] Restore already in progress");
    let success = false;
    ...
    if (diskUsage.available < neededSpace)
      throw new Error(
        "[not-enough-space] Not enough free space to restore backup",
      );
    ...
    try {
      ...
      const internalBackupMountpoint = nodePath.join(
        this.internalMountPath,
        backupDirectoryName,
      );
      ...
      await copyWithProgress(
        ...
        (progress) => {
          ...
          if (previousProgress !== this.restoreStatus.progress) {
            ...
            this.logger.log(
              `Restored ${this.restoreStatus.progress}% of backup`,
            );
          }
        },
      );
      ...
      await fse
        .ensureFile(`${temporaryData}/${BACKUP_RESTORE_FIRST_START_FLAG}`)
        .catch(() => {});
      ...
      if (!success || process.env.UMBRELD_RESTORE_SKIP_REBOOT === "true")
  ...
  private async connect(repositoryId: string) {
    ...
    const systemPath = this.#umbreld.files.virtualToSystemPathUnsafe(
      repository.path,
    );
    await this.kopia([
      "repository",
  ...
  async repository(
    ...
    // Run the command
    return this.kopia(
      [...flags, `--config-file=/kopia/config/${repositoryId}.config`],
      { onOutput, bypassQueue },
    );
  }
  ...
  async getRepositorySize(repositoryId: string) {
    ...
    const stats = await this.repository(repository.id, [
      "content",
      "stats",
      "--raw",
    ]);
    ...
    const status = await this.repository(repository.id, [
      "repository",
      "status",
      "--json",
    ]);
  ...
  async backup(repositoryId: string) {
    ...
    try {
      ...
      await this.repository(
        repository.id,
        ["snapshot", "create", this.#umbreld.dataDirectory],
        {
          onOutput: (output) => {
      ...
      await this.#umbreld.notifications
        .clear(`backups-failing:${repository.id}`)
        .catch(() => {});
      ...
      this.backupsInProgress = this.backupsInProgress.filter(
  ...
  async createIgnoreFile() {
    const ignoreFilePath = nodePath.join(
      this.#umbreld.dataDirectory,
      ".kopiaignore",
    );
    ...
    [...alwaysIgnoredPaths, ...userIgnoredPaths].forEach((path) => {
      try {
        ...
        this.logger.error(
          `Failed to get system path for ignored path ${path}`,
          error,
        );
      }
    });
    ...
    await Promise.all(
      this.#umbreld.apps.instances.map(async (app) => {
        ...
        const isIgnored = await app.isBackupIgnored().catch((error) => {
          ...
          this.logger.error(
            `Failed to get backup ignored status for ${app.id}`,
            error,
          );
          return false;
        });
        ...
        const backupIgnore = await app
          .getBackupIgnoredFilePaths()
          .catch((error) => {
            // If some app is in a broken state don't kill the whole backup
            this.logger.error(
              `Failed to get backup ignored file paths for ${app.id}`,
              error,
            );
            return [];
          });
    ...
    ignoreFileContents = ignoreFileContents.map((path) => {
      ...
      if (path.startsWith(this.#umbreld.dataDirectory))
        path = nodePath.relative(this.#umbreld.dataDirectory, path);
    ...
    await fse.move(temporaryIgnoreFilePath, ignoreFilePath, {
      overwrite: true,
    });
  }
  ...
  async listBackups(repositoryId: string) {
    ...
    const snapshots = await this.repository(repository.id, [
      "snapshot",
      "list",
      "--json",
    ]);
  ...
  async listAllBackups() {
    ...
    await Promise.all(
      repositories.map(async (repository) => {
        const repositoryBackups = await this.listBackups(repository.id).catch(
          (error) => {
            // If we can't list backups for a repository don't kill the whole backup list
            this.logger.error(
              `Failed to list backups for ${repository.id}`,
              error,
            );
            return [];
          },
  ...
  async listBackupFiles(backupId: string, path = "/") {
    ...
    const ls = await this.repository(repositoryId, [
      "ls",
      `${snapshotId}${path}`,
    ]);
  ...
  async mountBackup(backupId: string) {
    ...
    const internalMountpoint = nodePath.join(
      this.internalMountPath,
      directoryName,
    );
    await fse.mkdir(internalMountpoint, { recursive: true });
    let mountProcessExitCode = null;
    this.repository(repositoryId, ["mount", snapshotId, internalMountpoint], {
      bypassQueue: true,
    })
    ...
    while (true) {
      ...
      if (Date.now() - startTime > timeout)
        throw new Error(`Mount timeout after ${timeout}ms`);

      // Check if process has exited
      if (mountProcessExitCode !== null)
        throw new Error(`Mount exited with code ${mountProcessExitCode}`);
    ...
    await execa("mount", [
      "--bind",
      nodePath.join(internalMountpoint, "home"),
      homeMount,
    ]);
    await execa("mount", [
      "--bind",
      nodePath.join(internalMountpoint, "app-data"),
      appsMount,
    ]);
  ...
  async unmountBackup(directoryName: string) {
    ...
    await fse
      .remove(backupRoot)
      .catch((error) =>
        this.logger.error(`Failed to remove ${backupRoot}: ${error.message}`),
      );
    ...
    await execa("umount", [internalMountpoint]).catch((error) =>
      this.logger.error(
        `Failed to unmount ${internalMountpoint}: ${error.message}`,
      ),
    );
    await fse
      ...
      .catch((error) =>
        this.logger.error(
          `Failed to remove ${internalMountpoint}: ${error.message}`,
        ),
      );
  ...
  async unmountAll(): Promise<void> {
    ...
    await Promise.all(
      backups.map((backup) =>
        this.unmountBackup(backup).catch((error) =>
          this.logger.error(`Failed to unmount ${backup}: ${error.message}`),
        ),
      ),
    );
    ...
    const internalMounts = await fse
      .readdir(this.internalMountPath)
      .catch(() => []);
    await Promise.all(
      internalMounts.map((internalMount) =>
        this.unmountBackup(internalMount).catch((error) =>
          this.logger.error(
            `Failed to unmount ${internalMount}: ${error.message}`,
          ),
        ),
      ),
    );
