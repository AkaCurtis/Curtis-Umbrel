...
export default router({
  ...
  getRepositories: privateProcedure.query(async ({ ctx }) => {
    ...
    return repositories.map(({ id, path, lastBackup }) => ({
      id,
      path,
      lastBackup,
    }));
  ...
  getRepositorySize: privateProcedure
    ...
    .query(async ({ ctx, input }) =>
      ctx.umbreld.backups.getRepositorySize(input.repositoryId),
    ),
  ...
  createRepository: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.createRepository(input.path, input.password),
    ),
  ...
  forgetRepository: privateProcedure
    .input(z.object({ repositoryId: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.forgetRepository(input.repositoryId),
    ),
  ...
  backup: privateProcedure
    .input(z.object({ repositoryId: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.backup(input.repositoryId),
    ),
  ...
  listBackups: publicProcedureWhenNoUserExists
    .input(z.object({ repositoryId: z.string() }))
    .query(async ({ ctx, input }) =>
      ctx.umbreld.backups.listBackups(input.repositoryId),
    ),

  // List all backups for all repositories
  listAllBackups: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.backups.listAllBackups(),
  ),
  ...
  listBackupFiles: privateProcedure
    ...
    .query(async ({ ctx, input }) =>
      ctx.umbreld.backups.listBackupFiles(input.backupId, input.path),
    ),
  ...
  mountBackup: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.mountBackup(input.backupId),
    ),
  ...
  unmountBackup: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.unmountBackup(input.directoryName),
    ),

  // Get progress of backup operations
  backupProgress: privateProcedure.query(
    async ({ ctx }) => ctx.umbreld.backups.backupsInProgress,
  ),

  // Get ignored paths
  getIgnoredPaths: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.backups.getIgnoredPaths(),
  ),
  ...
  addIgnoredPath: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.addIgnoredPath(input.path),
    ),
  ...
  removeIgnoredPath: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.removeIgnoredPath(input.path),
    ),
  ...
  connectToExistingRepository: publicProcedureWhenNoUserExists
    .input(z.object({ path: z.string(), password: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.connectToExistingRepository(
        input.path,
        input.password,
      ),
    ),
  ...
  restoreBackup: publicProcedureWhenNoUserExists
    .input(z.object({ backupId: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.backups.restoreBackup(input.backupId),
    ),

  // Get status of restore operations
  restoreStatus: publicProcedureWhenNoUserExists.query(
    async ({ ctx }) => ctx.umbreld.backups.restoreStatus,
  ),
});
