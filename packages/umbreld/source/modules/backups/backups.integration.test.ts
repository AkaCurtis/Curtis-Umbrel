...
async function createBackupShare(
  umbreld: Awaited<ReturnType<typeof createTestUmbreld>>,
  directoryName = "Backups",
) {
  await umbreld.client.files.createDirectory.mutate({
...
test("createRepository() creates and stores a repository on external storage", async () => {
  ...
  await fse.mkdir(`${umbreld.instance.dataDirectory}/external/SanDisk`, {
    recursive: false,
  });
...
test("getRepositories() returns repositories with lastBackup date", async () => {
  ...
  await expect(
    umbreld.client.backups.getRepositories.query(),
...
test("backup() creates a backup successfully", async () => {
  ...
  await expect(
    umbreld.client.backups.listBackups.query({ repositoryId }),
  ).resolves.toHaveLength(0);

  // Do the backup
  await expect(
    umbreld.client.backups.backup.mutate({ repositoryId }),
  ).resolves.toBe(true);

  // Verify backup was created by listing backups
  const backups = await umbreld.client.backups.listBackups.query({
    repositoryId,
  });
  ...
  const files = await umbreld.client.backups.listBackupFiles.query({
...
test("multiple backups create separate snapshots", async () => {
  ...
  await fse.mkdir(`${umbreld.instance.dataDirectory}/home`, {
    recursive: true,
  });
  ...
  // Create first backup
  await umbreld.client.backups.backup.mutate({ repositoryId });
  const firstBackups = await umbreld.client.backups.listBackups.query({
    repositoryId,
  });
  ...
  await umbreld.client.backups.backup.mutate({ repositoryId });
  const secondBackups = await umbreld.client.backups.listBackups.query({
    repositoryId,
  });
...
test("mountBackup() mounts and unmountBackup() unmounts a backup", async () => {
  ...
  await umbreld.client.backups.unmountBackup.mutate({
    directoryName: dateDirs[0],
  });
...
test("listAllBackups() returns backups from all repositories", async () => {
  ...
  const repo1Backups = allBackups.filter((backup) =>
    backup.id.startsWith(`${repositoryId1}:`),
  );
  const repo2Backups = allBackups.filter((backup) =>
    backup.id.startsWith(`${repositoryId2}:`),
  );
  ...
  expect(allBackups).toEqual([
    expect.objectContaining({
      id: expect.stringMatching(`^${repositoryId1}:`),
    }),
    expect.objectContaining({
      id: expect.stringMatching(`^${repositoryId2}:`),
    }),
    expect.objectContaining({
      id: expect.stringMatching(`^${repositoryId1}:`),
    }),
...
test("backup() reports progress during backup operation", async () => {
  ...
  const removeListener = umbreld.instance.eventBus.on(
    ...
    (progress) =>
      void progressEvents.push(JSON.parse(JSON.stringify(progress))),
  );

  // Test we start with no backups in progress
  await expect(
    umbreld.client.backups.backupProgress.query(),
  ).resolves.toMatchObject([]);
  ...
  expect(progressEvents.at(-2)).toMatchObject([
    { repositoryId, percent: expect.any(Number) },
  ]);
...
test("backups respect user ignored paths", async () => {
  ...
  let backups = await umbreld.client.backups.listBackups.query({
    repositoryId,
  });
  expect(backups).toHaveLength(1);
  let files = await umbreld.client.backups.listBackupFiles.query({
    backupId: backups[0].id,
  });
  expect(files).toContain("home");
  ...
  await expect(
    umbreld.client.backups.getIgnoredPaths.query(),
  ).resolves.not.toContain("/Home");

  // Ignore /Home
  await expect(
    umbreld.client.backups.addIgnoredPath.mutate({ path: "/Home" }),
  ).resolves.toBe(true);
  ...
  await expect(
    umbreld.client.backups.addIgnoredPath.mutate({ path: "/App/foo" }),
  ).rejects.toThrow("Path to exclude must be in /Home");
  ...
  files = await umbreld.client.backups.listBackupFiles.query({
  ...
  // Remove ignored path
  await expect(
    umbreld.client.backups.removeIgnoredPath.mutate({ path: "/Home" }),
  ).resolves.toBe(true);
  ...
  await expect(
    umbreld.client.backups.removeIgnoredPath.mutate({ path: "/External" }),
  ).rejects.toThrow("Path to exclude must be in /Home");
...
test("backups respect app backupIgnore glob patterns", async () => {
  // Install app
  await expect(
    umbreld.client.apps.install.mutate({ appId: "sparkles-hello-world" }),
  ...
  await fse.writeFile(
    `${appDataDir}/important-data/config.json`,
    "important config",
  );
  ...
  const manifest = yaml.load(
    await fse.readFile(manifestPath, "utf8"),
  ) as AppManifest;
...
test("backups handle disconnected network shares gracefully", async () => {
  ...
  await expect(
    umbreld.client.files.list.query({ path: mountPath }),
  ).rejects.toThrow("EHOSTDOWN");

  // Attempt another backup that fails
  await expect(
    umbreld.client.backups.backup.mutate({ repositoryId }),
  ).rejects.toThrow("host is down");
...
test("getRepositorySize() returns the size of a repository", async () => {
  ...
  await expect(
    umbreld.client.backups.getRepositorySize.query({ repositoryId }),
  ).resolves.toMatchObject({
    used: 0,
...
test("backups sets user notification if backups have not run in over 24 hours", async () => {
  ...
  await expect(umbreld.client.notifications.get.query()).resolves.toHaveLength(
    0,
  );
  ...
  // TODO: For some reason if we remove this line the test fails, look into why
  await expect(
    umbreld.client.files.list.query({ path: backupNetworkSharePath }),
  ).rejects.toThrow("EHOSTDOWN");
  ...
  const viNow = vi
    .spyOn(Date, "now")
    .mockImplementation(() => now + TWENTY_FOUR_HOURS);
  ...
  await pRetry(
    () =>
      umbreld.client.backups.backup.mutate({
...
test("backup can be restored on the current Umbrel install", async () => {
  ...
  const removeListener = umbreld.instance.eventBus.on(
    ...
    (progress) =>
      void restoreProgressEvents.push(JSON.parse(JSON.stringify(progress))),
  );
  ...
  const getSystemDiskUsageSpy = vi
    .spyOn(system, "getSystemDiskUsage")
    .mockResolvedValue({
      size: ONE_GB * 10,
      totalUsed: ONE_GB * 6,
      available: ONE_GB * 4,
  ...
  await expect(
    umbreld.client.backups.restoreBackup.mutate({ backupId: latestBackup.id }),
  ).rejects.toThrow("[not-enough-space]");
  getSystemDiskUsageSpy.mockRestore();

  // Now restore should succeed with normal disk usage
  await umbreld.client.backups.restoreBackup.mutate({
  ...
  // Verify we received progress events
  expect(restoreProgressEvents.at(0)).toMatchObject({
    backupId: latestBackup.id,
    progress: 0,
    running: true,
  });
  ...
  expect(restoreProgressEvents.at(-1)).toMatchObject({
    running: false,
    progress: 100,
    error: false,
  });
  ...
  await expect(
    umbreld.client.backups.restoreStatus.query(),
  ).resolves.toMatchObject({
    running: false,
    progress: 100,
    error: false,
  ...
  expect(
    await fse.pathExists(
      `${umbreld.instance.dataDirectory}/home/original-umbrel`,
    ),
  ).toBe(false);
...
test("backup can be restored on a fresh umbrel during setup", async () => {
  ...
  const newUmbreld = await createTestUmbreld({
    autoLogin: false,
    autoStart: true,
  });
  ...
  const newRepositoryId =
    await newUmbreld.client.backups.connectToExistingRepository.mutate({
      path: `/External/SanDisk`,
      password: "test-password",
    });
  ...
  // Get the latest backup and restore it
  const backups = await newUmbreld.client.backups.listBackups.query({
    repositoryId: newRepositoryId,
  });
  const latestBackup = backups.at(-1)!;
  expect(latestBackup).toBeDefined();
  await newUmbreld.client.backups.restoreBackup.mutate({
    backupId: latestBackup.id,
  });
  ...
  // Verify current progress is not running (final status is 100% after success)
  await expect(
    newUmbreld.client.backups.restoreStatus.query(),
  ).resolves.toMatchObject({
    running: false,
    progress: 100,
    error: false,
  ...
  expect(
    await fse.pathExists(
      `${newUmbreld.instance.dataDirectory}/home/original-umbrel`,
    ),
  ).toBe(false);
...
test("connectToExistingRepository() cleans up failed connection details", async () => {
  ...
  await newUmbreld.client.backups.listBackups.query({
    repositoryId: newRepositoryId,
  });
...
test("getRepositories() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.getRepositories.query(),
  ).rejects.toThrow("Invalid token");
});

test("getRepositorySize() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.getRepositorySize.query({
...
test("backup() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.backup.mutate({
      repositoryId: "test-repo",
    }),
  ).rejects.toThrow("Invalid token");
});
...
test("listBackups() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.listBackups.query({
      repositoryId: "test-repo",
    }),
  ).rejects.toThrow("Invalid token");
});
...
test("listAllBackups() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.listAllBackups.query(),
  ).rejects.toThrow("Invalid token");
});

test("listBackupFiles() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.listBackupFiles.query({
      backupId: "test-repo:test-backup",
    }),
  ).rejects.toThrow("Invalid token");
});
...
test("mountBackup() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.mountBackup.mutate({
      backupId: "test-repo:test-backup",
    }),
  ).rejects.toThrow("Invalid token");
});
...
test("unmountBackup() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.unmountBackup.mutate({
...
test("backupProgress() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.backupProgress.query(),
  ).rejects.toThrow("Invalid token");
});

test("getIgnoredPaths() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.getIgnoredPaths.query(),
  ).rejects.toThrow("Invalid token");
});

test("addIgnoredPath() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.addIgnoredPath.mutate({
      path: "/Home/test",
    }),
  ).rejects.toThrow("Invalid token");
});
...
test("removeIgnoredPath() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.backups.removeIgnoredPath.mutate({
      path: "/Home/test",
    }),
  ).rejects.toThrow("Invalid token");
});
