...
describe.sequential("Static IP rollback", () => {
  ...
  const requestStatus = async () => {
    ...
    return await got(
      `http://localhost:${umbreld.vm.httpPort}/manager-api/v1/system/update-status`,
      {
        retry: { limit: 0 },
  ...
  const getSetStaticIpResult = () => {
    if (!setStaticIpResult)
      throw new Error("Expected static IP mutation to be started");
  ...
  test("applies an unconfirmed static IP and becomes unreachable", async () => {
    const networkInterface = (
      await umbreld.client.system.getNetworkInterfaces.query()
    )[0]!;
    iface = {
  ...
  test("becomes reachable again after rollback", async () => {
    ...
    if (result.success)
      throw new Error("Expected static IP change to be rolled back");
