...
describe("Static IP configuration", () => {
  ...
  test("switching to a static IP is reflected in getNetworkInterfaces()", async () => {
    ...
    let after!: Awaited<
      ReturnType<typeof umbreld.client.system.getNetworkInterfaces.query>
    >[number];
    await pWaitFor(
      async () => {
        try {
    ...
    after = (await umbreld.client.system.getNetworkInterfaces.query()).find(
      (i) => i.mac === interfaceMac,
    )!;

    // Only the method should change, everything else stays the same
    expect(after.ipMethod).toBe("static");
    expect(after.configuredStaticSettings).toEqual({
      ip,
      subnetPrefix,
      gateway,
      dns,
