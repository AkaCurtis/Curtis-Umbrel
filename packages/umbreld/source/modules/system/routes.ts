...
export default router({
  ...
  setExternalDns: privateProcedure
    .input(z.boolean())
    .mutation(async ({ ctx, input }) => {
      const previousExternalDns = await ctx.umbreld.store.get(
        "settings.externalDns",
        true,
      );
  ...
  hiddenService: privateProcedure.query(async ({ ctx }) => {
    try {
      return await fse.readFile(
        `${ctx.umbreld.dataDirectory}/tor/data/web/hostname`,
        "utf-8",
      );
    } catch (error) {
  ...
  systemDiskUsage: privateProcedure.query(({ ctx }) =>
    getSystemDiskUsage(ctx.umbreld),
  ),
  diskUsage: privateProcedure.query(({ ctx }) => getDiskUsage(ctx.umbreld)),
  systemMemoryUsage: privateProcedure.query(({ ctx }) =>
    getSystemMemoryUsage(),
  ),
  ...
  getNetworkInterfaces: privateProcedure.query(({ ctx }) =>
    getNetworkInterfaces(ctx.umbreld),
  ),
  setHostname: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      setHostname(ctx.umbreld, input.hostname),
    ),
  setStaticIp: privateProcedure
    .input(
      z.object({
        ...
        gateway: z
          .string()
          .ip({ version: "v4", message: "Invalid IPv4 gateway" }),
        dns: z
          .array(
            z
              .string()
              .ip({ version: "v4", message: "Invalid IPv4 DNS address" }),
          )
          .min(1),
      }),
    )
  ...
  logs: privateProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.type === "umbrelos") {
        process =
          await $`journalctl --unit umbrel --unit umbreld-production --unit umbreld --unit ui --lines 1500`;
      }
  ...
  factoryReset: publicProcedureWhenNoUserExists
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const raidMountFailure =
        await ctx.umbreld.hardware.raid.checkRaidMountFailure();
      if (!raidMountFailure) {
        if (
          !input.password ||
          !(await ctx.user.validatePassword(input.password))
        ) {
          throw new TRPCError({
            code: "UNAUTHORIZED",
