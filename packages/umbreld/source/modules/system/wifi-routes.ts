...
export default router({
  ...
  connect: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const previousWifiCredentials =
        await ctx.umbreld.store.get("settings.wifi");
      try {
        await connectToWiFiNetwork({
          ssid: input.ssid,
          password: input.password,
        });
        await ctx.umbreld.store.set("settings.wifi", {
          ssid: input.ssid,
          password: input.password,
        });
      } catch (error) {
        ...
        if (previousWifiCredentials) {
          ctx.umbreld.logger.error(
            `Failed to connect to WiFi network, attempting to restore previous credentials...`,
          );
          connectToWiFiNetwork(previousWifiCredentials).catch((error) => {
            ctx.umbreld.logger.error(
              `Failed to restore previous WiFi connection`,
              error,
            );
          });
        }
