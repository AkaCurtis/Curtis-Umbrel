...
describe("Hostname configuration", () => {
  ...
  test("setHostname() updates the live system hostname", async () => {
    ...
    configuredHostname =
      currentHostname === "umbrel-test" ? "umbrel-host" : "umbrel-test";
    await umbreld.client.system.setHostname.mutate({
    ...
    expect(etcHosts).toMatch(
      new RegExp(`^127\\.0\\.(?:0|1)\\.1\\s+${configuredHostname}$`, "m"),
    );
  });
