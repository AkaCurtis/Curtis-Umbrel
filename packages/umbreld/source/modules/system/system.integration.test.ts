...
describe("cpuTemperature", async () => {
  ...
  test.skipIf(!isCpuTemperatureSupported)(
    "should return cpu temperature",
    async () => {
      expect(await router.cpuTemperature()).toBeTypeOf("number");
    },
  );
  test.skipIf(isCpuTemperatureSupported)(
    "should throw error if cpu temp is unsupported",
    async () => {
      expect(router.cpuTemperature).rejects.toThrow(
        "Could not get CPU temperature",
      );
    },
  );

  test("should be behind authentication", async () => {
    expect(unAuthedRouter.cpuTemperature()).rejects.toHaveProperty(
      "code",
      "UNAUTHORIZED",
    );
  });
...
describe("getDiskUsage", () => {
  ...
  test("should be behind authentication", async () => {
    expect(unAuthedRouter.diskUsage()).rejects.toHaveProperty(
      "code",
      "UNAUTHORIZED",
    );
  });
...
describe("memoryUsage", () => {
  ...
  test("should be behind authentication", async () => {
    expect(unAuthedRouter.memoryUsage()).rejects.toHaveProperty(
      "code",
      "UNAUTHORIZED",
    );
  });
