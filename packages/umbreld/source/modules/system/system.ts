...
export async function getDiskUsageByPath(
  path: string,
  ...
  const [size, totalUsed, available] = df.stdout
    .split("\n")
    .slice(-1)[0]
    .split(" ")
    .map(Number);
...
async function getDockerContainerIds(): Promise<Map<string, string>> {
  try {
    const { stdout } =
      await $`docker ps --no-trunc --format={{.ID}}|{{.Names}}`;
...
async function getContainerCgroupMemory(
  containerId: string,
): Promise<{ used: number; swap: number }> {
  try {
    ...
    const [currentStr, memoryStat, swapStr] = await Promise.all([
      ...
      fse
        .readFile(`${cgroupPath}/memory.swap.current`, "utf8")
        .catch(() => "0"),
    ]);
    const current = clampNonNegativeNumber(parseInt(currentStr.trim(), 10));
    const inactiveFile = clampNonNegativeNumber(
      parseInt(memoryStat.match(/^inactive_file (\d+)$/m)?.[1] ?? "0", 10),
    );
    return {
...
async function getZramRamFactor(): Promise<number> {
  try {
    ...
    if (
      !Number.isFinite(origDataSize) ||
      !Number.isFinite(memUsedTotal) ||
      origDataSize <= 0
    )
      return 0;
...
async function getReclaimableZfsArcSize(): Promise<number> {
  try {
    ...
    const getField = (name: string) => {
      const match = arcstats.match(
        new RegExp(`^${name}\\s+\\d+\\s+(\\d+)$`, "m"),
      );
...
async function getSystemMemoryFromMeminfo(): Promise<{
  size: number;
  totalUsed: number;
}> {
  const [meminfo, arcSize] = await Promise.all([
    fse.readFile("/proc/meminfo", "utf8"),
    getReclaimableZfsArcSize(),
  ]);
  const size = clampByteCount(getProcMemoryField(meminfo, "MemTotal") ?? 0);
  const memAvailable = clampByteCount(
    getProcMemoryField(meminfo, "MemAvailable") ?? 0,
  );
...
export async function getMemoryUsage(umbreld: Umbreld): Promise<{
  ...
  const [containerIds, zramRamFactor] = await Promise.all([
  ...
  const apps = await Promise.all(
    umbreld.apps.instances.map(async (app) => {
      try {
        ...
        const appUsed = clampByteCount(
          cgroupResults.reduce((total, cg) => total + cg.used, 0),
        );
        const appSwap = clampByteCount(
          cgroupResults.reduce((total, cg) => total + cg.swap, 0),
        );
  ...
  const appsTotal = clampByteCount(
    apps.reduce((total, app) => total + app.used, 0),
  );
...
export async function getCpuUsage(umbreld: Umbreld): Promise<{
  ...
  const totalUsed = processes.reduce(
...
export async function detectDevice() {
  let { manufacturer, model, serial, uuid, sku, version } =
...
export async function getWifiNetworks() {
  const listNetworks =
    await $`nmcli --terse --fields IN-USE,SSID,SECURITY,SIGNAL device wifi list`;
  ...
  const filteredNetworks = networks
    ...
    .filter((network, index, list) => {
      ...
      const indexOfFirstEntry = list.findIndex(
    ...
    .map((network) => {
      network.active =
        network.active ||
        networks.some((item) => item.ssid === network.ssid && item.active);
      return network;
    })
    // Order by SSID
...
export async function deleteWifiConnections({
  inactiveOnly = false,
}: {
  inactiveOnly?: boolean;
}) {
  const connections =
    await $`nmcli --terse --fields UUID,TYPE,ACTIVE connection`;
...
export async function connectToWiFiNetwork({
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Failed to cleanup WiFi connections: ${(error as Error).message}`,
      );
    }
    ...
    if (connection.exitCode === 1 || connection.exitCode === 4)
...
export async function restoreWiFi(umbreld: Umbreld): Promise<void> {
  ...
  while (true) {
    umbreld.logger.log(
      `Attempting to restore WiFi connection to ${wifiCredentials.ssid}...`,
    );
    try {
      ...
      umbreld.logger.error(
        `Failed to restore WiFi connection, retrying in 1 minute`,
        error,
      );
...
export function getIpAddresses(): string[] {
  ...
  return (
    Object.entries(os.networkInterfaces())
      ...
      .filter(
        ([name]) =>
          !excludeInterfaceNames.some((expression) => expression.test(name)),
      )
      ...
      .filter(
        (entry) =>
          entry.family === "IPv4" && !entry.internal && isIPv4(entry.address),
      )
      // Omit addresses within excluded ranges
      .filter(
        (entry) =>
          !excludeAddressRanges.some((expression) =>
            expression.test(entry.address),
          ),
      )
...
export async function getNetworkInterfaces(
  ...
  const { stdout: deviceOutput } =
    await $`nmcli --terse --fields DEVICE,TYPE,STATE,CONNECTION device status`;
  const configuredStaticSettings =
    (await umbreld?.store.get("settings.staticIp")) ?? {};
  ...
  const interfaces = await Promise.all(
    deviceOutput
      ...
      .map(async ({ device, type, state, connection }) => {
        ...
        if (type === "ethernet") {
          const isPhysical = await fse.pathExists(
        ...
        const mac = (
          await fse.readFile(`/sys/class/net/${device}/address`, "utf8")
        ).trim();
        ...
        if (configuredStaticSetting)
          iface.configuredStaticSettings = configuredStaticSetting;
...
async function getInterfaceByMac(mac: string): Promise<NetworkInterface> {
  const networkInterface = (await getNetworkInterfaces()).find(
    (iface) => iface.mac === mac,
  );
...
async function getConnectionByDevice(device: string): Promise<string | false> {
  ...
  const { stdout: uuidsOutput } =
    await $`nmcli --terse --fields UUID connection show`;
  for (const uuid of uuidsOutput.split("\n")) {
    if (!uuid.trim()) continue;
    try {
      const { stdout } =
        await $`nmcli --terse --fields connection.interface-name,connection.id connection show ${uuid}`;
...
export async function setStaticIp(
  ...
  if (networkInterface.type === "wifi")
    throw new Error("Static IP is not supported for WiFi interfaces");

  // Capture previous config for rollback
  const currentSettings = await umbreld.store.get("settings.staticIp");
  const previousConfig = currentSettings?.[mac]
    ? { mac, ...currentSettings[mac] }
    : null;
  ...
  confirmedStaticIp = "";
  try {
    await pWaitFor(() => confirmedStaticIp === ip, {
      interval: 100,
      timeout: 30_000,
    });
  } catch {
    ...
    throw new Error(
      "Static IP change was not confirmed within 30 seconds, reverted to previous settings",
    );
  }
...
export async function clearStaticIp(
  umbreld: Umbreld,
  { mac }: { mac: string },
) {
  const networkInterface = await getInterfaceByMac(mac);
...
export async function restoreStaticIp(umbreld: Umbreld): Promise<void> {
  ...
  for (const [mac, config] of Object.entries(settings)) {
    try {
      ...
      if (networkInterface.type === "wifi") {
        umbreld.logger.log(
          `Skipping static IP restore for ${mac}: WiFi interfaces are not supported`,
        );
        continue;
      }
...
export async function syncDns() {
  return await syncDnsQueue.add(async () => {
    const { mtimeMs: mtimeBefore } =
      await fse.promises.stat("/etc/resolv.conf");
...
export async function waitForSystemTime(
  umbreld: Umbreld,
  timeout: number,
): Promise<void> {
  try {
    ...
    umbreld.logger.log(
      "Checking if system time is synced before continuing...",
    );
    let tries = 0;
    while (tries < timeout) {
      tries++;
      const timeStatus = await $`timedatectl status`;
      const isSynced = timeStatus.stdout.includes(
    ...
    umbreld.logger.error(
      "System time is not synced but timeout was reached. Continuing...",
    );
  } catch (error) {
...
async function applyHostname(umbreld: Umbreld, hostname: string) {
  ...
  const previousHostnameInEtcHostsRe =
    previousHostname &&
    new RegExp(
      `^(\\s*127\\.0\\.(?:0|1)\\.1\\s+)${escapeSpecialRegExpLiterals(previousHostname)}\\s*$`,
      "m",
    );
  ...
  if (updatedEtcHosts !== etcHosts)
    await fse.writeFile("/etc/hosts", updatedEtcHosts);
...
export async function setHostname(umbreld: Umbreld, hostname: string) {
  const previousConfiguredHostname =
    await umbreld.store.get("settings.hostname");

  await umbreld.store.set("settings.hostname", hostname);
  try {
    return await applyHostname(umbreld, hostname);
  } catch (error) {
    if (previousConfiguredHostname)
      await umbreld.store.set("settings.hostname", previousConfiguredHostname);
