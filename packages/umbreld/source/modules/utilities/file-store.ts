...
type StorePath<T, P extends string> =
  DotProp<T, P> extends never
    ? "The provided path does not exist in the store"
    : P;
...
export default class FileStore<T extends Serializable> {
  ...
  async #set<P extends string>(
    property: StorePath<T, P>,
    value: DotProp<T, P>,
  ) {
  ...
  async get<P extends string>(
    property?: StorePath<T, P>,
    defaultValue?: DotProp<T, P>,
  ) {
    const store = await this.#read();

    return getProperty(store, property as string, defaultValue) as DotProp<
      T,
      P
    >;
  }
