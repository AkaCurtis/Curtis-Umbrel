...
export async function pull(
  ...
  return new Promise((resolve, reject) => {
    docker.pull(image, (error: Error, stream: NodeJS.ReadableStream) => {
      ...
      function progress() {
        const totalProgress = Object.values(layerProgress).reduce(
          (total, layer) => total + layer,
          0,
        );
      ...
      function onProgress(event: any) {
        ...
        if (event.status === "Extracting") {
          ...
          layerProgress[event.id] =
            DOWNLOADING_PERCENT + extractPercent * EXTRACTING_PERCENT;
          updateProgress(progress());
        }
      }
    ...
    images.map(async (image) =>
      pull(
        ...
        (progress) => {
          ...
          const totalProgress =
            Object.values(imageProgress).reduce(
