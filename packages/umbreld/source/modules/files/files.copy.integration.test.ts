...
vi.mock("../system/system.js", async (importOriginal) => {
  ...
  return {
    ...original,
    getDiskUsageByPath: async (umbreld: any) =>
      getDiskUsageByPathReturnValue ?? original.getDiskUsageByPath(umbreld),
  };
});
...
test("copy() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.copy.mutate({
      path: "/Home/Documents",
      toDirectory: "/Home/Documents-copy",
    }),
...
test("copy() handles copying files to same directory by appending numbers regardless of collision strategy", async () => {
  ...
  await expect(fse.pathExists(`${testDirectory}/original.txt`)).resolves.toBe(
    true,
  );
  await expect(
    fse.pathExists(`${testDirectory}/original (2).txt`),
  ).resolves.toBe(true);
  ...
  await expect(
    fse.pathExists(`${testDirectory}/original (3).txt`),
  ).resolves.toBe(true);
...
test("copy() handles copying directories to same parent directory by appending numbers regardless of collision strategy", async () => {
  ...
  await expect(
    fse.pathExists(`${testDirectory}/original/file.txt`),
  ).resolves.toBe(true);
  await expect(fse.pathExists(`${testDirectory}/original (2)`)).resolves.toBe(
    true,
  );
  await expect(
    fse.pathExists(`${testDirectory}/original (2)/file.txt`),
  ).resolves.toBe(true);
  ...
  await expect(fse.pathExists(`${testDirectory}/original (3)`)).resolves.toBe(
    true,
  );
  await expect(
    fse.pathExists(`${testDirectory}/original (3)/file.txt`),
  ).resolves.toBe(true);
...
test('copy(path, {collision: "keep-both"}) keeps both directories by appending numbers', async () => {
  ...
  await fse.writeFile(
    `${testDirectory}/destination/docs/file.txt`,
    "destination content",
  );
  ...
  await expect(
    fse.pathExists(`${testDirectory}/destination/docs`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${testDirectory}/destination/docs (2)`),
  ).resolves.toBe(true);

  // Verify the contents are preserved in both directories
  await expect(
    fse.readFile(`${testDirectory}/destination/docs/file.txt`, "utf8"),
  ).resolves.toBe("destination content");
  await expect(
    fse.readFile(`${testDirectory}/destination/docs (2)/file.txt`, "utf8"),
  ).resolves.toBe("source content");
...
test('copy(path, {collision: "replace"}) completely replaces existing directories', async () => {
  ...
  await fse.mkdir(`${testDirectory}/docs`);
  await fse.writeFile(
    `${testDirectory}/docs/file1.txt`,
    "file 1 source content",
  );
  await fse.writeFile(
    `${testDirectory}/docs/file2.txt`,
    "file 2 source content",
  );
  await fse.mkdir(`${testDirectory}/destination`);
  await fse.mkdir(`${testDirectory}/destination/docs`);
  await fse.writeFile(
    `${testDirectory}/destination/docs/file1.txt`,
    "file 1 destination content",
  );
  await fse.writeFile(
    `${testDirectory}/destination/docs/file3.txt`,
  ...
  await expect(
    fse.readFile(`${testDirectory}/destination/docs/file1.txt`, "utf8"),
  ).resolves.toBe("file 1 source content");
  await expect(
    fse.readFile(`${testDirectory}/destination/docs/file2.txt`, "utf8"),
  ).resolves.toBe("file 2 source content");

  // Verify file3.txt no longer exists (since it was only in the destination)
  await expect(
    fse.pathExists(`${testDirectory}/destination/docs/file3.txt`),
  ).resolves.toBe(false);
...
test("copy() throws on too many duplicate names from existing paths", async () => {
  ...
  await expect(fse.readdir(`${testDirectory}`)).resolves.toMatchObject(
    expect.arrayContaining([
      "source.txt",
      ...Array.from({ length: maxPossibleCopies - 2 }).map(
        (_, index) => `source (${index + 2}).txt`,
      ),
    ]),
  );
