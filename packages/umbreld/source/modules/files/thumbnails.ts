...
export default class Thumbnails {
  ...
  #backgroundThumbnailDebouncers = new Map<
    string,
    DebouncedFunction<() => Promise<void>>
  >();
  ...
  async start() {
    ...
    await fse.ensureDir(this.thumbnailDirectory).catch((error) => {
      this.logger.error(
        `Failed to ensure directory '${this.thumbnailDirectory}' exists`,
        error,
      );
    });
    ...
    this.#removeDiskChangeListener = this.#umbreld.eventBus.on(
      "system:disk:change",
  ...
  #debouncedGenerateThumbnail(systemPath: string): void {
    ...
    if (!debouncer) {
      const generateThumbnailAndCleanup = async () => {
        ...
        await this.#generateThumbnail(systemPath, { background: true }).catch(
          (error) => {
            ...
            this.logger.error(
              `Failed to generate thumbnail for ${systemPath}`,
              error,
            );
          },
        );
  ...
  hashToThumbnailSystemPath(hash: string): string {
    ...
    const systemPath = nodePath.normalize(
      nodePath.join(this.thumbnailDirectory, filename),
    );
  ...
  async getThumbnailOnDemand(virtualPath: string): Promise<string> {
    ...
    const systemPath =
      await this.#umbreld.files.virtualToSystemPath(virtualPath);
    if (!(await this.#isValidFileForThumbnail(systemPath))) {
      throw new Error(
        `Unsupported file type for thumbnail: ${nodePath.extname(virtualPath).toLowerCase()}`,
      );
    }

    // Generate the thumbnail in the on-demand queue
    const hash = await this.#generateThumbnail(systemPath, {
      background: false,
    });
  ...
  async getExistingThumbnail(systemPath: string): Promise<string | undefined> {
    ...
    await fse.utimes(thumbnailSystemPath, now, now).catch((error) => {
      ...
      this.logger.error(
        `Failed to touch thumbnail ${thumbnailSystemPath}`,
        error,
      );
    });
  ...
  async #pruneOldestThumbnails(): Promise<void> {
    ...
    try {
      ...
      for await (const thumbnailPath of getDirectoryStream(
      ...
      for (let i = 0; i < excessCount && i < thumbnails.length; i++) {
        ...
        try {
          ...
          this.logger.error(
            `Failed to remove thumbnail ${thumbnail.path}`,
            error,
          );
        }
      }
  ...
  async stop() {
    ...
    this.#backgroundThumbnailDebouncers.forEach((debouncer) =>
      debouncer.cancel(),
    );
  }
}
