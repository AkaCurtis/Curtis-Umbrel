...
export default class Favorites {
  ...
  async #handleFileChange(event: FileChangeEvent) {
    ...
    const virtualDeletedPath = this.#umbreld.files.systemToVirtualPath(
      event.path,
    );
    const deletedFavorites = favorites.filter((favorite) =>
      favorite.startsWith(virtualDeletedPath),
    );
  ...
  async listFavorites() {
    ...
    const mappedFavorites = await Promise.all(
      favorites.map(async (favorite) => {
        const systemPath =
          await this.#umbreld.files.virtualToSystemPath(favorite);
        const file = await this.#umbreld.files
          .status(systemPath)
          .catch(() => undefined);
    ...
    const filteredFavorites = mappedFavorites.filter(
  ...
  async addFavorite(virtualPath: string) {
    ...
    if (!allowedOperations.includes("favorite"))
