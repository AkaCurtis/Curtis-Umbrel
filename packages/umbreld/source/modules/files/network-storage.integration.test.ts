...
async function createNetworkShare(
...
describe("listNetworkShares()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.listNetworkShares.query(),
...
describe("addNetworkShare()", () => {
  ...
  test("successfully adds and mounts a network share", async () => {
    ...
    const networkFiles = await umbreld.client.files.list.query({
      path: mountPath,
    });
...
describe("removeNetworkShare()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.removeNetworkShare.mutate({
        mountPath: "/Network/test/share",
      }),
    ).rejects.toThrow("Invalid token");
  });
  ...
  test("throws error when removing non-existent share", async () => {
    await expect(
      ...
      "Share with mount path /Network/non-existent/share not found",
    );
  });
  ...
  test("successfully removes a network share", async () => {
    ...
    const result = await umbreld.client.files.removeNetworkShare.mutate({
      mountPath,
    });
...
describe("discoverNetworkShareServers()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.discoverNetworkShareServers.query(),
    ).rejects.toThrow("Invalid token");
  });
  ...
  test.skip("returns array of discovered servers", async () => {
    const servers =
      await umbreld.client.files.discoverNetworkShareServers.query();
...
describe("discoverNetworkSharesOnServer()", () => {
  ...
  test("throws error with invalid credentials", async () => {
    ...
    // Add directory as a Samba share
    await umbreld.client.files.addShare.mutate({
      path: "/Home/discover-invalid-test",
    });
  ...
  test("discovers shares on local Samba server", async () => {
    ...
    await umbreld.client.files.addShare.mutate({
      path: "/Home/discover-test-1",
    });
    await umbreld.client.files.addShare.mutate({
      path: "/Home/discover-test-2",
    });
    ...
    const shares =
      await umbreld.client.files.discoverNetworkSharesOnServer.query({
        host: "localhost",
        username: "umbrel",
        password: sharePassword,
      });
    ...
    expect(shares).toMatchObject(
      expect.arrayContaining([
        "discover-test-1 (Umbrel)",
        "discover-test-2 (Umbrel)",
      ]),
    );
  });
});
...
describe("isServerAnUmbrelDevice()", () => {
  test("returns true for an umbrel device", async () => {
    ...
    const isServerAnUmbrelDevice =
      await umbreld.client.files.isServerAnUmbrelDevice.query({ address });
...
describe("file permissions", () => {
  test("allows hard deletion of network files", async () => {
    const mountPath = await createNetworkShare(
      umbreld,
      "network-deletion-test",
    );

    // Attempt to hard delete a file from the network share
    await expect(
      umbreld.client.files.delete.mutate({
        path: `${mountPath}/test-file.txt`,
      }),
  ...
  test("does not allow soft trash of network files", async () => {
    ...
    await expect(
      umbreld.client.files.trash.mutate({ path: `${mountPath}/test-file.txt` }),
  ...
  test("network mount points are protected paths", async () => {
    const mountPath = await createNetworkShare(
      umbreld,
      "network-protected-test",
    );

    // Test each level of the mount path is protected
    expect(mountPath).toBe(
      "/Network/localhost/network-protected-test (Umbrel)",
    );
    const hostnamePath = "/Network/localhost";
    const networkPath = "/Network";
    for (const path of [networkPath, hostnamePath, mountPath]) {
      // Trash
      await expect(umbreld.client.files.trash.mutate({ path })).rejects.toThrow(
        "[operation-not-allowed]",
      );
      // Delete
      await expect(
        umbreld.client.files.delete.mutate({ path }),
      ).rejects.toThrow("[operation-not-allowed]");
      // Move
      await expect(
        umbreld.client.files.move.mutate({ path, toDirectory: "/Home" }),
      ).rejects.toThrow("[operation-not-allowed]");
      // Rename
      await expect(
        umbreld.client.files.rename.mutate({
          path,
          newName: "Renamed Network Share",
        }),
      ).rejects.toThrow("[operation-not-allowed]");
      ...
      if (path !== networkPath) {
        ...
        await expect(
          umbreld.client.files.createDirectory.mutate({ path: siblingPath }),
        ).rejects.toThrow("[operation-not-allowed]");
      }
    }
  });
...
describe("behaviour", () => {
  test("auto mounts an added network share on startup", async () => {
    ...
    await pRetry(
      async () => {
        const networkFilesAfterRestart = await umbreld.client.files.list.query({
          path: mountPath,
        });
  ...
  test("auto mounts remounts a network share if it goes offline and then comes back online", async () => {
    ...
    await umbreld.client.files.removeShare.mutate({
      path: "/Home/reconnect-test",
    });
    ...
    const systemMountPath =
      await umbreld.instance.files.virtualToSystemPath(mountPath);
