...
function acceptsEmbeddedSvg(request: express.Request) {
  ...
  const acceptedMimeTypes = acceptHeader
    ...
    .filter(Boolean);
  const explicitlyAcceptsSvg =
    acceptedMimeTypes.includes("image/svg+xml") ||
    acceptedMimeTypes.includes("image/*");
  const acceptsDocument =
    acceptedMimeTypes.includes("text/html") ||
    acceptedMimeTypes.includes("application/xhtml+xml");
...
export default function api({ publicApi, privateApi, umbreld }: ApiOptions) {
  ...
  privateApi.get("/download", async (request, response) => {
    ...
    if (typeof request.query.path === "string")
      virtualPaths = [String(request.query.path)];
    if (Array.isArray(request.query.path))
      virtualPaths = request.query.path.map(String);
    ...
    try {
      ...
      const filename = umbreld.files.archive.zipName(files, {
        defaultName: "umbrel-files.zip",
      });
  ...
  privateApi.get("/view", async (request, response) => {
    try {
      ...
      const systemPath = await umbreld.files.virtualToSystemPath(
        request.query.path,
      );
      const status = await umbreld.files.status(systemPath);
      if (status.type === "directory")
        return response.status(400).json({ error: "cannot view a directory" });
      ...
      if (
        mimeType &&
        (inlineViewMimeTypes.has(mimeType) ||
          (isImageEmbed && embedOnlyInlineViewMimeTypes.has(mimeType)))
      ) {
  ...
  privateApi.post("/upload", async (request, response) => {
    ...
    const collision =
      typeof request.query.collision === "string"
        ? request.query.collision
        : "error";
    const isValidCollisionParameter = [
      "error",
      "keep-both",
      "replace",
    ].includes(collision);
    if (!isValidCollisionParameter) {
      ...
      return response
        .status(400)
        .json({ error: "invalid collision parameter" });
    }

    // Check path is valid
    let systemPath = await umbreld.files
      .virtualToSystemPath(request.query.path)
      .catch((error) => {
        response.setHeader("Connection", "close");
    ...
    // Write the file
    await pipeline(request, fse.createWriteStream(temporarySystemPath)).catch(
      async (error) => {
    ...
    return response
      .status(200)
      .json({ path: umbreld.files.systemToVirtualPath(systemPath) });
  });
}
