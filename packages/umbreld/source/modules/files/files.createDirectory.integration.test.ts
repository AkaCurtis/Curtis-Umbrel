...
test("createDirectory() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.createDirectory.mutate({
...
test("createDirectory() throws on directory traversal attempt", async () => {
  await expect(
    umbreld.client.files.createDirectory.mutate({
      path: "/Home/../../../../etc/new-dir",
    }),
...
test("createDirectory() throws on symlink traversal attempt", async () => {
  ...
  await expect(
    umbreld.client.files.createDirectory.mutate({
      path: "/Home/symlink-to-root/new-dir",
    }),
...
test("createDirectory() throws on relative paths", async () => {
  await Promise.all(
    ["", " ", ".", "..", "Home", "Home/new-dir", "Home/../new-dir"].map(
      (path) =>
        expect(
          umbreld.client.files.createDirectory.mutate({ path }),
...
test("createDirectory() throws when containing directory doesn't exist", async () => {
  ...
  await expect(
    umbreld.client.files.createDirectory.mutate({ path }),
  ).rejects.toThrow("[parent-not-exist]");
});
...
test("createDirectory() throws when creating directory inside a file", async () => {
  ...
  await expect(
    umbreld.client.files.createDirectory.mutate({ path }),
  ).rejects.toThrow("[parent-not-directory]");
});
...
test("createDirectory() creates directory with correct permissions", async () => {
  ...
  const stats = await fse.stat(
    umbreld.instance.dataDirectory + "/home/permissions-test",
  );
