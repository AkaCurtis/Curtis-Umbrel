...
afterEach(async () => {
  ...
  for (const file of await fse.readdir(trashDir))
    await fse.remove(`${trashDir}/${file}`);
  for (const file of await fse.readdir(trashMetaDir))
    await fse.remove(`${trashMetaDir}/${file}`);
});

test("restore() throws invalid error without auth token", async () => {
  // Now try to restore without auth
  await expect(
    umbreld.unauthenticatedClient.files.restore.mutate({ path: "/Trash/foo" }),
...
test("restore() throws on relative paths", async () => {
  await Promise.all(
    ["", " ", ".", "..", "Trash", "Trash/..", "Trash/file.txt"].map(
      async (path) => {
        await expect(
          umbreld.client.files.restore.mutate({
            path,
...
test("restore() throws when metadata file is missing", async () => {
  ...
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/trash/no-meta.txt`,
    "content",
  );
...
test("restore() successfully restores a file from trash", async () => {
  ...
  await expect(
    fse.pathExists(`${testDirectory}/restore-file.txt`),
  ).resolves.toBe(false);

  // Now restore the file
  await expect(
    umbreld.client.files.restore.mutate({ path: trashPath }),
  ).resolves.toBe(virtualPath);

  // Verify the file no longer exists in trash
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash/restore-file.txt`),
  ).resolves.toBe(false);
  ...
  await expect(
    fse.readFile(`${testDirectory}/restore-file.txt`, "utf8"),
  ).resolves.toBe("test content");

  // Verify metadata file is deleted
  await expect(
    fse.pathExists(
      `${umbreld.instance.dataDirectory}/trash-meta/restore-file.txt.json`,
    ),
  ).resolves.toBe(false);
...
test("restore() successfully restores a directory with contents from trash", async () => {
  ...
  await fse.writeFile(
    `${testDirectory}/restore-dir/nested/file3.txt`,
    "content3",
  );
  ...
  await expect(fse.pathExists(`${testDirectory}/restore-dir`)).resolves.toBe(
    false,
  );
  ...
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash/restore-dir`),
  ).resolves.toBe(false);
  ...
  await expect(
    fse.pathExists(`${testDirectory}/restore-dir/file1.txt`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${testDirectory}/restore-dir/file2.txt`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${testDirectory}/restore-dir/nested/file3.txt`),
  ).resolves.toBe(true);

  // Verify metadata file is deleted
  await expect(
    fse.pathExists(
      `${umbreld.instance.dataDirectory}/trash-meta/restore-dir.json`,
    ),
  ).resolves.toBe(false);
...
test("restore() successfully restores a child directory from trash", async () => {
  ...
  // Verify the directory is moved to trash
  await expect(
    fse.pathExists(`${testDirectory}/restore-child-dir`),
  ).resolves.toBe(false);
  ...
  await expect(
    fse.pathExists(
      `${umbreld.instance.dataDirectory}/trash/restore-child-dir-test/child-dir`,
    ),
  ).resolves.toBe(false);

  // Verify the directory exists at the original location
  await expect(fse.pathExists(`${testDirectory}/child-dir`)).resolves.toBe(
    true,
  );
});
...
test("restore() successfully restores multiple subdirectories from trash one after another", async () => {
  ...
  await expect(
    fse.pathExists(
      `${umbreld.instance.dataDirectory}/trash/restore-multiple-subdirs-test`,
    ),
  ).resolves.toBe(true);
  ...
  await expect(
    fse.pathExists(`${testDirectory}/subdir1/file1.txt`),
  ).resolves.toBe(true);

  // Verify the second subdirectory is still in trash
  await expect(
    fse.pathExists(
      `${umbreld.instance.dataDirectory}/trash/restore-multiple-subdirs-test/subdir2`,
    ),
  ).resolves.toBe(true);
  ...
  await expect(
    fse.pathExists(`${testDirectory}/subdir2/file2.txt`),
  ).resolves.toBe(true);
...
test('restore(path, {collision: "keep-both"}) handles name conflict', async () => {
  ...
  await expect(
    fse.pathExists(`${testDirectory}/conflict-file.txt`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${testDirectory}/conflict-file (2).txt`),
  ).resolves.toBe(true);

  // Verify the contents are preserved
  await expect(
    fse.readFile(`${testDirectory}/conflict-file.txt`, "utf8"),
  ).resolves.toBe("new content");
  await expect(
    fse.readFile(`${testDirectory}/conflict-file (2).txt`, "utf8"),
...
test('restore(path, {collision: "replace"}) handles name conflict', async () => {
  ...
  // Verify the file no longer exists in trash
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash/conflict-file.txt`),
  ).resolves.toBe(false);
...
test('restore(path, {collision: "replace"}) with directory replaces entire target directory instead of merging', async () => {
  ...
  await expect(fse.pathExists(`${testDirectory}/file1.txt`)).resolves.toBe(
    true,
  );
  await expect(fse.pathExists(`${testDirectory}/file2.txt`)).resolves.toBe(
    true,
  );
  await expect(fse.pathExists(`${testDirectory}/file3.txt`)).resolves.toBe(
    true,
  );

  // Verify the new directory content is gone (replaced, not merged)
  await expect(fse.pathExists(`${testDirectory}/fileA.txt`)).resolves.toBe(
    false,
  );
  await expect(fse.pathExists(`${testDirectory}/fileB.txt`)).resolves.toBe(
    false,
  );

  // Verify the content of restored files is correct
  await expect(
    fse.readFile(`${testDirectory}/file1.txt`, "utf8"),
  ).resolves.toBe("original file1");
  await expect(
    fse.readFile(`${testDirectory}/file2.txt`, "utf8"),
  ).resolves.toBe("original file2");
  await expect(
    fse.readFile(`${testDirectory}/file3.txt`, "utf8"),
  ).resolves.toBe("original file3");
...
test("restore() creates parent directories if they do not exist", async () => {
  ...
  // Verify the file exists at the original location
  await expect(
    fse.pathExists(`${testDirectory}/nested-restore.txt`),
  ).resolves.toBe(true);

  // Verify the content is preserved
  await expect(
    fse.readFile(`${testDirectory}/nested-restore.txt`, "utf8"),
  ).resolves.toBe("test content");
