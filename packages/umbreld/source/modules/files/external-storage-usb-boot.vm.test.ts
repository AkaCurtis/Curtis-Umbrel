...
const getMountpoints = (device: LsblkDevice) =>
  device.mountpoints?.filter(
    (mountpoint): mountpoint is string => !!mountpoint,
  ) ?? [];
...
const flattenFileSystems = (
  fileSystems: FindmntFileSystem[],
): FindmntFileSystem[] =>
  fileSystems.flatMap((fileSystem) => [
    fileSystem,
    ...flattenFileSystems(fileSystem.children ?? []),
  ]);
...
describe("External storage on USB boot disks", () => {
  ...
  async function getUsbBootDisk() {
    const { blockdevices } = JSON.parse(
      await umbreld.vm.ssh("lsblk --json --output NAME,TYPE,TRAN,MOUNTPOINTS"),
    ) as {
    ...
    const usbBootDisk = blockdevices.find((device) => {
      ...
      return (
        ...
        partitions.some((partition) =>
          getMountpoints(partition).some((mountpoint) =>
  ...
  test("boots the generic NAS from a USB transport disk", async () => {
    ...
    const partitions =
      usbBootDisk.children?.filter((partition) => partition.type === "part") ??
      [];

    expect(partitions.length).toBeGreaterThan(1);
    expect(
      partitions.some((partition) => getMountpoints(partition).length === 0),
    ).toBe(true);
  });
  ...
  test("does not detect or mount USB boot disk partitions as external storage", async () => {
    ...
    const { filesystems = [] } = JSON.parse(
      await umbreld.vm.ssh("findmnt --json --output SOURCE,TARGET"),
    ) as {
      filesystems?: FindmntFileSystem[];
    };
