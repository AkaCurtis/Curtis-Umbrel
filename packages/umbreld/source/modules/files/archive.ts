...
export default class Archive {
  ...
  async createZipStream(systemPaths: string[]) {
    ...
    const directories = systemPaths.map((systemPath) =>
      nodePath.dirname(systemPath),
    );
    const uniqueDirectories = new Set(directories);
    if (uniqueDirectories.size > 1)
      throw new Error("paths must be in same directory");
      ...
      if (status.isDirectory()) {
        ...
        archive.directory(
          ...
          (entry) => {
            ...
            if (
              zipEntry.stats?.isFile() &&
              this.#shouldSkipCompression(zipEntry.name)
            )
              zipEntry.store = true;
            return zipEntry;
          },
  ...
  async createZipFile(virtualPaths: string[]) {
    ...
    const systemPaths = await Promise.all(
      virtualPaths.map((virtualPath) =>
        this.#umbreld.files.virtualToSystemPath(virtualPath),
      ),
    );

    // Calculate the zip path
    let zipPath = nodePath.join(
      nodePath.dirname(systemPaths[0]),
      this.zipName(systemPaths),
    );
  ...
  async unarchive(virtualPath: string) {
    ...
    const allowedOperations =
      await this.#umbreld.files.getAllowedOperations(virtualPath);
