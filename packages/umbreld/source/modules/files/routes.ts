...
export default router({
    ...
    .query(async ({ ctx, input }) => {
      ...
      directoryListing.files.sort((fileA, fileB) => {
        ...
        if (typeof a === "string" && typeof b === "string")
          return textSort.compare(a, b);
        if (typeof a === "number" && typeof b === "number")
          return a - b || textSort.compare(fileA.name, fileB.name);
        return 0;
      });
      ...
      let startIndex = 0;
      if (input.lastFile) {
        const lastFileIndex = directoryListing.files.findIndex(
          (file) => file.name === input.lastFile,
        );
      ...
      const paginatedFiles = directoryListing.files.slice(
        startIndex,
  ...
  createDirectory: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.createDirectory(input.path),
    ),
  ...
  operationProgress: privateProcedure.query(
    async ({ ctx }) => ctx.umbreld.files.operationsInProgress,
  ),
  ...
  rename: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.rename(input.path, input.newName),
    ),
  ...
  restore: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.restore(input.path, { collision: input.collision }),
    ),

  // Empty the trash
  emptyTrash: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.files.emptyTrash(),
  ),
  ...
  // Get favorites
  favorites: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.files.favorites.listFavorites(),
  ),
  ...
  addFavorite: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.favorites.addFavorite(input.path),
    ),
  ...
  removeFavorite: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.favorites.removeFavorite(input.path),
    ),

  // Get recent files
  recents: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.files.recents.get(),
  ),
  ...
  viewPreferences: publicProcedureWhenNoUserExists.query(async ({ ctx }) =>
    ctx.umbreld.files.getViewPreferences(),
  ),
  ...
  updateViewPreferences: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.updateViewPreferences(input),
    ),
  ...
  archive: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.archive.archive(input.paths),
    ),
  ...
  unarchive: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.archive.unarchive(input.path),
    ),
  ...
  getThumbnail: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.thumbnails.getThumbnailOnDemand(input.path),
    ),

  // Get the share password
  sharePassword: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.files.samba.getSharePassword(),
  ),

  // Get shares
  shares: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.files.samba.listShares(),
  ),
  ...
  addShare: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.samba.addShare(input.path),
    ),
  ...
  removeShare: privateProcedure
    .input(z.object({ path: z.string() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.samba.removeShare(input.path),
    ),
  ...
  formatExternalDevice: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.externalStorage.formatExternalDevice(input),
    ),
  ...
  unmountExternalDevice: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.externalStorage.unmountExternalDevice(input.deviceId, {
        remove: true,
      }),
    ),

  // Check if an external drive is connected on non-Umbrel Home hardware
  isExternalDeviceConnectedOnUnsupportedDevice: privateProcedure.query(
    ({ ctx }) =>
  ...
  search: privateProcedure
    ...
    .query(async ({ ctx, input }) =>
      ctx.umbreld.files.search.search(input.query, input.maxResults),
    ),
  ...
  addNetworkShare: publicProcedureWhenNoUserExists
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.networkStorage.addShare(input),
    ),
  ...
  removeNetworkShare: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.files.networkStorage.removeShare(input.mountPath),
    ),

  // Discover available network share servers
  discoverNetworkShareServers: publicProcedureWhenNoUserExists.query(
    async ({ ctx }) => ctx.umbreld.files.networkStorage.discoverServers(),
  ),
  ...
  discoverNetworkSharesOnServer: publicProcedureWhenNoUserExists
    ...
    .query(async ({ ctx, input }) =>
      ctx.umbreld.files.networkStorage.discoverSharesOnServer(
        input.host,
        input.username,
        input.password,
      ),
  ...
  isServerAnUmbrelDevice: privateProcedure
    ...
    .query(async ({ ctx, input }) =>
      ctx.umbreld.files.networkStorage.isServerAnUmbrelDevice(input.address),
    ),
});
