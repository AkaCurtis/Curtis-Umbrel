...
test("list() throws invalid error whithout auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.list.query({ path: "/" }),
  ).rejects.toThrow("Invalid token");
});

test("list() throws on directory traversal attempt", async () => {
  await expect(
    umbreld.client.files.list.query({ path: "/Home/../../../../etc" }),
...
test("list() throws on symlink traversal attempt", async () => {
  ...
  await expect(
    umbreld.client.files.list.query({ path: "/Home" }),
  ...
  // Attempt to list it
  await expect(
    umbreld.client.files.list.query({ path: "/Home/symlink-to-root" }),
...
test("list() throws on relative paths", async () => {
  await Promise.all(
    ["", " ", ".", "..", "Home", "Home/..", "Home/Documents"].map((path) =>
      expect(umbreld.client.files.list.query({ path })).rejects.toThrow(
        "[path-not-absolute]",
      ),
    ),
  );
...
test("list() throws on non existent paths", async () => {
  await Promise.all([
    expect(
      umbreld.client.files.list.query({ path: "/DoesNotExist" }),
    ).rejects.toThrow("[invalid-base]"),
    expect(
      umbreld.client.files.list.query({ path: "/Home/DoesNotExist" }),
...
test("list() lists the root directory", async () => {
  await expect(
    umbreld.client.files.list.query({ path: "/" }),
  ).resolves.toMatchObject({
    name: "",
    path: "/",
    type: "directory",
    ...
    operations: [],
    files: ["Apps", "Backups", "External", "Home", "Network", "Trash"].map(
      (name) => ({
        name,
        path: `/${name}`,
        type: "directory",
...
test("list() returns correct types for various files and directories", async () => {
  ...
  // Query the directory
  const mimeTypes = await umbreld.client.files.list.query({
    path: "/Home/mime-test",
  });
  ...
  [
    ...
    expect(mimeTypes.files.find((file) => file.name === name)?.type).toEqual(
      type,
    );
  });
...
test("list() paginates directory listings", async () => {
  ...
  await Promise.all(
    Array.from({ length: 150 }, (_, i) => i + 1).map((i) =>
      fse.writeFile(
        `${testDirectory}/file${i.toString().padStart(3, "0")}.txt`,
        "",
      ),
    ),
  );
  ...
  const firstPage = await umbreld.client.files.list.query({
...
test("list() paginates directory listings with a custom limit", async () => {
  ...
  expect(result.files[customLimit - 1].name).toBe(
    `file${customLimit.toString().padStart(3, "0")}.txt`,
  );
...
test("list() truncates a listing if it's larger than the max listing size", async () => {
  ...
  await expect(
    umbreld.client.files.list.query({ path: "/Home/max-listing-size" }),
  ).resolves.not.toHaveProperty("truncatedAt");
  ...
  await expect(
    umbreld.client.files.list.query({ path: "/Home/max-listing-size" }),
  ).resolves.toHaveProperty("truncatedAt", maxListingSize);
