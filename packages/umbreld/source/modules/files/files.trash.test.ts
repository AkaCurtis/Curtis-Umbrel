...
test("trash() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.trash.mutate({
      path: "/Home/Documents",
    }),
...
test("trash() throws on relative paths", async () => {
  await Promise.all(
    ["", " ", ".", "..", "Home", "Home/..", "Home/Documents"].map(
      async (path) =>
        expect(umbreld.client.files.trash.mutate({ path })).rejects.toThrow(
          "[path-not-absolute]",
        ),
    ),
  );
...
test("trash() successfully moves a file to trash", async () => {
  ...
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash-meta`),
  ).resolves.toBe(true);

  // Trash the file
  await expect(
    umbreld.client.files.trash.mutate({
      path: "/Home/trash-file-test/file.txt",
    }),
  ).resolves.toBe("/Trash/file.txt");

  // Verify the file is moved to trash
  await expect(fse.pathExists(`${testDirectory}/file.txt`)).resolves.toBe(
    false,
  );
  ...
  await expect(fse.readFile(trashSystemPath, "utf8")).resolves.toBe(
    "test content",
  );
...
test("trash() successfully moves a directory with contents to trash", async () => {
  ...
  await expect(
    umbreld.client.files.trash.mutate({
      path: "/Home/trash-directory-test/subdir",
    }),
  ).resolves.toBe("/Trash/subdir");
  ...
  await expect(fse.pathExists(`${trashSystemPath}/file1.txt`)).resolves.toBe(
    true,
  );
  await expect(fse.pathExists(`${trashSystemPath}/file2.txt`)).resolves.toBe(
    true,
  );
  await expect(
    fse.pathExists(`${trashSystemPath}/nested/file3.txt`),
  ).resolves.toBe(true);
...
test("trash() handles name conflicts by appending numbers", async () => {
  ...
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash/file.txt`),
  ).resolves.toBe(true);
  await expect(
    fse.pathExists(`${umbreld.instance.dataDirectory}/trash/file (2).txt`),
  ).resolves.toBe(true);
...
test("trash() preserves symlinks when trashing", async () => {
  ...
  await fse.mkdir(`${umbreld.instance.dataDirectory}/home/trash-symlink-test`, {
    recursive: true,
  });
  await fse.writeFile(
