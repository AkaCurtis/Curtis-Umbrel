...
test("emptyTrash() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.emptyTrash.mutate(),
...
test("emptyTrash() successfully empties trash with a single file", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/test-file.txt.json`,
    JSON.stringify({ path: "/Home/test-file.txt" }),
  );

  // Verify the file exists in trash
  await expect(fse.pathExists(`${trashDir}/test-file.txt`)).resolves.toBe(true);
  await expect(fse.pathExists(`${metaDir}/test-file.txt.json`)).resolves.toBe(
    true,
  );
  ...
  await expect(fse.pathExists(`${trashDir}/test-file.txt`)).resolves.toBe(
    false,
  );
  await expect(fse.pathExists(`${metaDir}/test-file.txt.json`)).resolves.toBe(
    false,
...
test("emptyTrash() successfully empties trash with multiple files", async () => {
  ...
  for (let i = 1; i <= numberOfFiles; i++) {
    ...
    await fse.writeFile(
      `${metaDir}/file${i}.txt.json`,
      JSON.stringify({ path: `/Home/file${i}.txt` }),
    );
  }

  // Verify the files exist in trash
  for (let i = 1; i <= numberOfFiles; i++) {
    await expect(fse.pathExists(`${trashDir}/file${i}.txt`)).resolves.toBe(
      true,
    );
    await expect(fse.pathExists(`${metaDir}/file${i}.txt.json`)).resolves.toBe(
      true,
    );
  }
...
test("emptyTrash() successfully empties trash with directories", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/test-dir.json`,
    JSON.stringify({ path: "/Home/test-dir" }),
  );
...
test("emptyTrash() handles metadata files without corresponding trash files", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/orphaned1.txt.json`,
    JSON.stringify({ path: "/Home/orphaned1.txt" }),
  );
  await fse.writeFile(
    `${metaDir}/orphaned2.txt.json`,
    JSON.stringify({ path: "/Home/orphaned2.txt" }),
  );

  // Verify the metadata files exist
  await expect(fse.pathExists(`${metaDir}/orphaned1.txt.json`)).resolves.toBe(
    true,
  );
  await expect(fse.pathExists(`${metaDir}/orphaned2.txt.json`)).resolves.toBe(
    true,
  );
...
test("emptyTrash() handles mixed content types", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/file1.txt.json`,
    JSON.stringify({ path: "/Home/file1.txt" }),
  );
  ...
  await fse.writeFile(
    `${metaDir}/orphaned.txt.json`,
    JSON.stringify({ path: "/Home/orphaned.txt" }),
  );
  ...
  await expect(fse.pathExists(`${metaDir}/file1.txt.json`)).resolves.toBe(
    false,
  );
  ...
  await expect(fse.pathExists(`${metaDir}/orphaned.txt.json`)).resolves.toBe(
    false,
  );
});
...
test("emptyTrash() reports failures correctly", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/good-file.txt.json`,
    JSON.stringify({ path: "/Home/good-file.txt" }),
  );

  // Create a file that will cause an error when deleted
  await fse.writeFile(`${trashDir}/bad-file.txt`, "bad content");
  await fse.writeFile(
    `${metaDir}/bad-file.txt.json`,
    JSON.stringify({ path: "/Home/bad-file.txt" }),
  );
  ...
  try {
    ...
    await expect(fse.pathExists(`${trashDir}/good-file.txt`)).resolves.toBe(
      false,
    );
    await expect(fse.pathExists(`${metaDir}/good-file.txt.json`)).resolves.toBe(
      false,
    );

    // The bad file should still exist
    await expect(fse.pathExists(`${trashDir}/bad-file.txt`)).resolves.toBe(
      true,
    );
    await expect(fse.pathExists(`${metaDir}/bad-file.txt.json`)).resolves.toBe(
      true,
    );
  } finally {
