...
test("POST /api/files/upload throws 400 error without path parameter", async () => {
  const error = await umbreld.api
    .post("files/upload", { body: "test content" })
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
...
test("POST /api/files/upload throws 400 error on directory traversal attempt", async () => {
  const error = await umbreld.api
    .post("files/upload?path=/Home/../../../../etc/dangerous-file.txt", {
...
test("POST /api/files/upload throws 400 error on relative path", async () => {
  ...
  for (const path of paths) {
    const error = await umbreld.api
      .post(`files/upload?path=${path}`, { body: "test content" })
      .catch((error) => error);
    expect(error).toBeInstanceOf(Error);
    expect(error.response.statusCode).toBe(400);
    expect(error.response.body).toMatchObject({ error: "invalid path" });
...
test("POST /api/files/upload throws 400 error on symlink traversal attempt", async () => {
  ...
  const error = await umbreld.api
    .post("files/upload?path=/Home/symlink-to-root/etc/dangerous-file.txt", {
      body: "malicious content",
    })
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
...
test("POST /api/files/upload successfully uploads a file with valid cookie and returns success response", async () => {
  // Upload a file
  const response = await umbreld.api.post(
    "files/upload?path=/Home/new-file.txt",
    {
  ...
  const exists = await fse.pathExists(
    `${umbreld.instance.dataDirectory}/home/new-file.txt`,
  );
...
test("POST /api/files/upload creates parent directories if they do not exist", async () => {
  ...
  const response = await umbreld.api.post(
    "files/upload?path=/Home/new-dir/sub-dir/new-file.txt",
    {
      body: "nested content",
    },
  );
  ...
  const exists = await fse.pathExists(
    `${umbreld.instance.dataDirectory}/home/new-dir/sub-dir/new-file.txt`,
  );
  expect(exists).toBe(true);
...
test("POST /api/files/upload handles files with special characters in name", async () => {
  ...
  // Upload the file
  const response = await umbreld.api.post(
    `files/upload?path=/Home/${encodeURIComponent(fileName)}`,
    {
  ...
  // Verify the file was created
  const exists = await fse.pathExists(
    `${umbreld.instance.dataDirectory}/home/${fileName}`,
  );
  expect(exists).toBe(true);
...
test("POST /api/files/upload handles empty files correctly", async () => {
  ...
  const response = await umbreld.api.post(
    "files/upload?path=/Home/empty-file.txt",
    { body: "" },
  );
  ...
  // Verify the file was created
  const exists = await fse.pathExists(
    `${umbreld.instance.dataDirectory}/home/empty-file.txt`,
  );
  expect(exists).toBe(true);
...
test("POST /api/files/upload handles binary data correctly", async () => {
  ...
  // Upload binary file
  const response = await umbreld.api.post(
    "files/upload?path=/Home/binary-file.bin",
    {
      body: binaryData,
    },
  );
  ...
  // Verify the file was created
  const exists = await fse.pathExists(
    `${umbreld.instance.dataDirectory}/home/binary-file.bin`,
  );
  expect(exists).toBe(true);
...
test("POST /api/files/upload creates file with correct permissions", async () => {
  // Upload a file
  const response = await umbreld.api.post(
    "files/upload?path=/Home/permissions-test.txt",
    {
  ...
  const stats = await fse.stat(
    `${umbreld.instance.dataDirectory}/home/permissions-test.txt`,
  );
...
test("POST /api/files/upload handles write errors correctly", async () => {
  ...
  vi.spyOn(fse, "createWriteStream").mockImplementation((path) => {
    const mockStream = new Writable({
      write: (chunk, encoding, callback) =>
        callback(new Error("Simulated disk full error")),
    }) as any;
  ...
  const error = await umbreld.api
    .post("files/upload?path=/Home/should-fail.txt", {
      body: "This upload should fail due to a simulated disk error",
    })
    .catch((err) => err);
...
test("POST /api/files/upload correctly handles streaming data in chunks", async () => {
  ...
  for (const chunk of chunks) {
    ...
    await expect(fse.readFile(temporarySystemPath, "utf8")).resolves.toContain(
      chunk.toString(),
    );
  }
...
test("POST /api/files/upload with collision=error (default) throws 400 when file already exists", async () => {
  // Create a file
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/collision-test.txt`,
    "original content",
  );
...
test("POST /api/files/upload with collision=error throws 400 when explicitly set and file already exists", async () => {
  // Create a file
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/explicit-error-test.txt`,
    "original content",
  );

  // Try to upload to the same path with explicit error strategy
  const error = await umbreld.api
    .post("files/upload?path=/Home/explicit-error-test.txt&collision=error", {
      body: "new content",
    })
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
...
test("POST /api/files/upload with collision=keep-both creates uniquely named file when file already exists", async () => {
  // Create a file
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/keep-both-test.txt`,
    "original content",
  );

  // Upload to the same path with keep-both strategy
  const response = await umbreld.api.post(
    "files/upload?path=/Home/keep-both-test.txt&collision=keep-both",
    {
      body: "new content",
    },
  );
  ...
  const originalContent = await fse.readFile(
    `${umbreld.instance.dataDirectory}/home/keep-both-test.txt`,
    "utf8",
  );
  expect(originalContent).toBe("original content");

  const newContent = await fse.readFile(
    `${umbreld.instance.dataDirectory}/home/keep-both-test (2).txt`,
    "utf8",
  );
...
test("POST /api/files/upload with collision=keep-both increments number for multiple collisions", async () => {
  ...
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/multiple-test.txt`,
    "original content",
  );
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/multiple-test (2).txt`,
    "first duplicate",
  );

  // Upload to the same path with keep-both strategy
  const response = await umbreld.api.post(
    "files/upload?path=/Home/multiple-test.txt&collision=keep-both",
    {
  ...
  const firstDuplicate = await fse.readFile(
    `${umbreld.instance.dataDirectory}/home/multiple-test (2).txt`,
    "utf8",
  );
  expect(firstDuplicate).toBe("first duplicate");

  const secondDuplicate = await fse.readFile(
    `${umbreld.instance.dataDirectory}/home/multiple-test (3).txt`,
    "utf8",
  );
...
test("POST /api/files/upload with collision=replace overwrites existing file", async () => {
  // Create a file
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/replace-test.txt`,
    "original content",
  );

  // Upload to the same path with replace strategy
  const response = await umbreld.api.post(
    "files/upload?path=/Home/replace-test.txt&collision=replace",
    {
...
test("POST /api/files/upload with invalid collision parameter returns 400 error", async () => {
  const error = await umbreld.api
    .post(
      "files/upload?path=/Home/invalid-collision-test.txt&collision=invalid",
      { body: "test content" },
    )
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
  expect(error.response.statusCode).toBe(400);
  expect(error.response.body).toMatchObject({
    error: "invalid collision parameter",
  });
});
