...
for (const forceSlowMoveWithProgress of forceSlowMoveWithProgressValues) {
  describe(`move() with UMBRELD_FORCE_SLOW_MOVE_WITH_PROGRESS=${forceSlowMoveWithProgress}`, () => {
    beforeEach(() => {
      process.env.UMBRELD_FORCE_SLOW_MOVE_WITH_PROGRESS =
        forceSlowMoveWithProgress;
    });
    ...
    test("move() throws on symlink traversal attempt in source path", async () => {
      ...
      // Clean up
      await fse.remove(
        `${umbreld.instance.dataDirectory}/home/symlink-to-root`,
      );
    });
    ...
    test("move() throws when trying to move a protected path out of /Apps/", async () => {
      ...
      // Clean up
      await fse.remove(testDirectory);
      await umbreld.client.apps.uninstall.mutate({
    ...
    test("move() does not throw when moving an unprotected path out of /Apps/", async () => {
      ...
      await fse.mkdir(
        `${umbreld.instance.dataDirectory}/app-data/not-an-app-id`,
      );
    ...
    test('move() throws on too many duplicate names from existing paths when using the "keep-both" collision strategy', async () => {
      ...
      for (let i = 2; i <= maxPossibleCopies; i++) {
        await expect(
          ...
          `/Home/move-existing-test/destination/source (${i}).txt`,
        );
    ...
    test("move() moves a single file to a directory", async () => {
      ...
      await expect(
        fse.readdir(`${testDirectory}/destination`),
      ...
      await expect(
        fse.readdir(`${testDirectory}/destination`),
      ).resolves.toMatchObject(["source.txt"]);
      await expect(
        fse.pathExists(`${testDirectory}/source/source.txt`),
      ).resolves.toBe(false);
    ...
    test("move() handles moving a file to a different directory by throwing on name conflict by default", async () => {
      ...
      await fse.mkdir(`${testDirectory}/destination`);
      await fse.writeFile(
        `${testDirectory}/destination/file.txt`,
        "destination content",
      );
      ...
      await expect(
        fse.readFile(`${testDirectory}/source/file.txt`, "utf8"),
      ).resolves.toBe("source content");
      await expect(
        fse.readFile(`${testDirectory}/destination/file.txt`, "utf8"),
      ).resolves.toBe("destination content");
    ...
    test('move(path, {collision: "keep-both"}) keeps both files by appending a number to the moved file', async () => {
      ...
      await expect(
        fse.pathExists(`${testDirectory}/destination/file.txt`),
      ).resolves.toBe(true);
      await expect(
        fse.pathExists(`${testDirectory}/destination/file (2).txt`),
      ).resolves.toBe(true);
      ...
      await expect(
        fse.readFile(`${testDirectory}/destination/file (2).txt`, "utf8"),
      ).resolves.toBe("source content");

      // Verify the source file no longer exists
      await expect(
        fse.pathExists(`${testDirectory}/source/file.txt`),
      ).resolves.toBe(false);
    ...
    test("move() moves a directory with contents", async () => {
      ...
      await fse.writeFile(
        `${testDirectory}/source/subdir/file3.txt`,
        "content3",
      );
      await fse.mkdir(`${testDirectory}/destination`);
      ...
      await expect(
        fse.readdir(`${testDirectory}/destination/source`),
      ).resolves.toMatchObject(["file1.txt", "file2.txt", "subdir"]);
      await expect(
        fse.readdir(`${testDirectory}/destination/source/subdir`),
      ).resolves.toMatchObject(["file3.txt"]);
      await expect(fse.pathExists(`${testDirectory}/source`)).resolves.toBe(
        false,
      );
    ...
    test("move() handles moving a directory to the current containing directory by doing nothing", async () => {
      ...
      await expect(fse.pathExists(`${testDirectory}/subdir`)).resolves.toBe(
        true,
      );
      await expect(
        fse.pathExists(`${testDirectory}/subdir/file.txt`),
      ).resolves.toBe(true);
      await expect(
        fse.readFile(`${testDirectory}/subdir/file.txt`, "utf8"),
    ...
    test("move() handles moving a directory to a different directory by throwing on name conflict by default", async () => {
      ...
      await fse.writeFile(
        `${testDirectory}/destination/source/file.txt`,
        "destination content",
      );
      ...
      await expect(
        fse.readFile(`${testDirectory}/destination/source/file.txt`, "utf8"),
      ).resolves.toBe("destination content");
    ...
    test('move(path, {collision: "keep-both"}) keeps both directories by appending a number to the moved directory', async () => {
      ...
      expect(result).toBe(
        "/Home/move-dir-keep-both-test/destination/source (2)",
      );

      // Verify both directories exist at the destination
      await expect(
        fse.pathExists(`${testDirectory}/destination/source`),
      ).resolves.toBe(true);
      await expect(
        fse.pathExists(`${testDirectory}/destination/source (2)`),
      ).resolves.toBe(true);
      ...
      await expect(
        fse.readFile(
          `${testDirectory}/destination/source (2)/file.txt`,
          "utf8",
        ),
      ).resolves.toBe("source content");
    ...
    test('move(path, {collision: "replace"}) replaces the existing directory with the moved directory', async () => {
      ...
      await fse.mkdir(`${testDirectory}/source`);
      await fse.writeFile(
        `${testDirectory}/source/file1.txt`,
        "file 1 source content",
      );
      await fse.writeFile(
        `${testDirectory}/source/file2.txt`,
        "file 2 source content",
      );
      await fse.mkdir(`${testDirectory}/destination`);
      await fse.mkdir(`${testDirectory}/destination/source`);
      await fse.writeFile(
        `${testDirectory}/destination/source/file1.txt`,
        "file 1 destination content",
      );
      await fse.writeFile(
        `${testDirectory}/destination/source/file3.txt`,
        "file 3 destination content",
      );
      ...
      await expect(
        fse.readFile(`${testDirectory}/destination/source/file1.txt`, "utf8"),
      ).resolves.toBe("file 1 source content");
      await expect(
        fse.readFile(`${testDirectory}/destination/source/file2.txt`, "utf8"),
      ).resolves.toBe("file 2 source content");

      // Verify the destination-only file no longer exists (was replaced)
      await expect(
        fse.pathExists(`${testDirectory}/destination/source/file3.txt`),
      ).resolves.toBe(false);
    ...
    test("move() moves symlinks as symlinks", async () => {
      ...
      await fse.symlink(
        `${testDirectory}/source/file.txt`,
        `${testDirectory}/source/link`,
      );
      await fse.mkdir(`${testDirectory}/destination`);
      ...
      await expect(
        fse.pathExists(`${testDirectory}/source/link`),
      ).resolves.toBe(false);
    ...
    test("move() moves files inside a symlink", async () => {
      ...
      await expect(
        fse.readdir(`${testDirectory}/destination`),
      ).resolves.toMatchObject(["file.txt"]);
      await expect(
        fse.pathExists(`${testDirectory}/source/file.txt`),
      ).resolves.toBe(false);
