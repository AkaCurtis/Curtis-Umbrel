...
test("GET /api/files/download throws 404 error when file does not exist", async () => {
  const error = await umbreld.api
    .get("files/download?path=/Home/does-not-exist")
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
...
test("GET /api/files/download throws 404 error on directory traversal attempt", async () => {
  const error = await umbreld.api
    .get("files/download?path=/Home/../../../../etc/passwd")
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
  expect(error.response.statusCode).toBe(404);
  expect(error.response.body).toMatchObject({ error: "not found" });
...
test("GET /api/files/download throws 404 error on symlink traversal attempt", async () => {
  ...
  const error = await umbreld.api
    .get("files/download?path=/Home/symlink-to-root/etc/passwd")
    .catch((error) => error);
  expect(error).toBeInstanceOf(Error);
  expect(error.response.statusCode).toBe(404);
  expect(error.response.body).toMatchObject({ error: "not found" });
...
test("GET /api/files/download throws 404 error when one of multiple files does not exist", async () => {
  // Create one file
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/file.txt`,
    "contents",
  );
...
test("GET /api/files/download downloads a file with a valid cookie", async () => {
  ...
  // Download the file
  const response = await umbreld.api.get("files/download?path=/Home/file.txt", {
    responseType: "text",
  });
...
test("GET /api/files/download downloads a file with special characters in name", async () => {
  ...
  const response = await umbreld.api.get(
    `files/download?path=/Home/${encodeURIComponent(fileName)}`,
    {
      responseType: "text",
    },
  );
...
test("GET /api/files/download creates a zip archive for multiple files", async () => {
  ...
  // Download the files
  const response = await umbreld.api.get(
    "files/download?path=/Home/file1.txt&path=/Home/file2.txt",
    {
      responseType: "buffer",
    },
  );
  ...
  expect(response.headers["content-disposition"]).toBe(
    `attachment; filename*=UTF-8''umbrel-files.zip`,
  );
...
test("GET /api/files/download creates a zip archive for a directory", async () => {
  ...
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/testdir/file1.txt`,
    "contents1",
  );
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/testdir/file2.txt`,
    "contents2",
  );
  await fse.ensureDir(`${umbreld.instance.dataDirectory}/home/testdir/subdir`);
  await fse.writeFile(
    `${umbreld.instance.dataDirectory}/home/testdir/subdir/file3.txt`,
    "contents3",
  );
  ...
  expect(response.headers["content-type"]).toBe("application/zip");
  expect(response.headers["content-disposition"]).toBe(
    `attachment; filename*=UTF-8''testdir.zip`,
  );
...
test("GET /api/files/download handles empty directories correctly", async () => {
  ...
  // Download the directory
  const response = await umbreld.api.get(
    "files/download?path=/Home/empty-dir",
    {
      responseType: "buffer",
    },
  );
  ...
  expect(response.headers["content-type"]).toBe("application/zip");
  expect(response.headers["content-disposition"]).toBe(
    `attachment; filename*=UTF-8''empty-dir.zip`,
  );
...
test("GET /api/files/download handles files with zero content correctly", async () => {
  ...
  // Download the file
  const response = await umbreld.api.get(
    "files/download?path=/Home/empty-file.txt",
  );
  ...
  expect(response.headers["content-disposition"]).toBe(
    `attachment; filename*=UTF-8''empty-file.txt`,
  );
});
...
test("GET /api/files/download handles binary files correctly", async () => {
  ...
  // Download the file
  const response = await umbreld.api.get(
    "files/download?path=/Home/binary-file.bin",
    {
      responseType: "buffer",
    },
  );
  ...
  expect(response.headers["content-disposition"]).toBe(
    `attachment; filename*=UTF-8''binary-file.bin`,
  );
});
