...
describe("getThumbnail", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.getThumbnail.mutate({
        path: "/Home/test.jpg",
      }),
  ...
  test.todo(
    "cannot generate thumbnail outside of /Home /Apps /Trash /External",
  );
  ...
  test(
    "generates thumbnails on-demand when no thumbnail exists",
    { retry: 5 },
    async () => {
      // Create test image
      const testDir = `${umbreld.instance.dataDirectory}/home/thumbnail-test`;
  ...
  test("returns same thumbnail for renamed files", async () => {
    ...
    const virtualPath = "/Home/thumbnail-test/master-lossless-image.png";
    const originalThumbnailUrl = await umbreld.client.files.getThumbnail.mutate(
      { path: virtualPath },
    );

    // Rename the file
    const renamedPath = nodePath.join(
      testDir,
      `renamed${nodePath.extname(originalPath)}`,
    );
  ...
  test("returns same thumbnail for moved files", async () => {
    ...
    const movedVirtualPath =
      "/Home/thumbnail-test/destination/master-lossless-image.png";
    const movedThumbnailUrl = await umbreld.client.files.getThumbnail.mutate({
      path: movedVirtualPath,
    });
  ...
  test("generates a new thumbnail when source file is modified", async () => {
    ...
    const modifiedVirtualPath =
      "/Home/thumbnail-test/master-lossless-image.png";
    const modifiedThumbnailUrl = await umbreld.client.files.getThumbnail.mutate(
      { path: modifiedVirtualPath },
    );
  ...
  test("returns existing thumbnails when they exist without generating a new one", async () => {
    ...
    const virtualPath = "/Home/thumbnail-test/master-lossless-image.png";
    const thumbnailUrl1 = await umbreld.client.files.getThumbnail.mutate({
      path: virtualPath,
    });
    ...
    const thumbnailUrl2 = await umbreld.client.files.getThumbnail.mutate({
      path: virtualPath,
    });
...
describe("Background file watcher", () => {
  ...
  test(
    "does not update thumbnails when files are moved",
    { retry: 5 },
    async () => {
      // Create test directories
  ...
  test(
    "does not update thumbnails when files are renamed",
    { retry: 5 },
    async () => {
      // Create test directory
  ...
  test("updates thumbnails when files are modified", { retry: 5 }, async () => {
    ...
    await pollUntil(
      ...
      },
      {
        timeoutMs: 15000,
        errorMessage:
          "Watcher did not generate new thumbnail after file modification",
      },
    );
    ...
    const newThumbnailExists = finalThumbnails.some(
      (t) => !t.startsWith(initialThumbnailHash),
    );
    expect(newThumbnailExists).toBe(true);
  });
  ...
  test(
    "ignores unsupported file types for thumbnails",
    { retry: 5 },
    async () => {
      // Create test directory
      const testDir = `${umbreld.instance.dataDirectory}/home/thumbnail-test`;
...
describe("files.list() [thumbnail specific]", () => {
  test("includes thumbnail for a file with an existing thumbnail", async () => {
    ...
    const files = await umbreld.client.files.list.query({
      path: "/Home/thumbnail-test",
    });
...
describe("recents() [thumbnail specific]", () => {
  test(
    "includes thumbnail for a recent file with an existing thumbnail",
    { retry: 5 },
    async () => {
      // Create test directory
      ...
      await pollUntil(
        ...
        },
        {
          timeoutMs: 15000,
          errorMessage:
            "Watcher did not generate initial thumbnail for recent file",
        },
      );
...
describe("Thumbnail housekeeping", () => {
  test(
    "removes oldest thumbnails when exceeding cleanup threshold",
    { retry: 5 },
    async () => {
      ...
      await pollUntil(
        ...
        },
        {
          timeoutMs: 15000,
          errorMessage:
            "Watcher didn't create expected thumbnails for first batch",
        },
      );
      ...
      expect(firstBatchThumbnails.length).toBeGreaterThanOrEqual(
        initialBatchSize,
      );
      ...
      await pollUntil(
        async () => {
          const thumbnails = await fse.readdir(thumbnailDir);
          return (
            thumbnails.length <= thumbnailsInstance.maxThumbnailCount &&
            thumbnails.length > 0
          );
        },
        {
    ...
    for (const thumbnail of finalThumbnails) {
      ...
      if (match) {
        ...
        expect(index).toBeGreaterThanOrEqual(
          totalThumbnailCount - maxThumbnailCount,
        );
      }
    }
  });
...
describe("Queue selection", () => {
  test(
    "uses background queue for watcher events and on-demand queue for explicit requests",
    { retry: 5 },
    async () => {
      // Create test directory
      ...
      const thumbnailsInstance = umbreld.instance.files.thumbnails;
      const backgroundAddSpy = vi.spyOn(
        thumbnailsInstance.backgroundQueue,
        "add",
      );
      ...
      await pollUntil(
        ...
        },
        {
          timeoutMs: 15000,
          errorMessage:
            "Background queue was not used for watcher-triggered thumbnail",
        },
      );
      ...
      const virtualPath =
        "/Home/thumbnail-queue-test/master-lossless-image.png";
