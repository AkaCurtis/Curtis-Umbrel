...
export default class Samba {
  ...
  async start() {
    ...
    this.#removeExternalStorageChangeListener = this.#umbreld.eventBus.on(
      "files:external-storage:change",
      () => {
        this.applyShares().catch((error) =>
          this.logger.error("Failed to reapply shares", error),
        );
      },
    );
  }
  ...
  async stop() {
    ...
    await $`systemctl stop smbd`.catch((error) =>
      this.logger.error(`Failed to stop samba`, error),
    );
    await $`systemctl stop wsdd2`.catch((error) =>
      this.logger.error(`Failed to stop wsdd2`, error),
    );
  }
  ...
  async getSharePassword() {
    ...
    const sharePassword = await fse
      .readFile(sharePasswordFile, "utf8")
      .catch(async () => {
  ...
  async applyShares({ excludePaths }: { excludePaths?: string[] } = {}) {
    ...
    for (const share of shares) {
      ...
      const systemPath = await this.#umbreld.files.virtualToSystemPath(
        share.path,
      );
      ...
      if (
        excludePaths?.some(
          (p) => share.path === p || share.path.startsWith(p + "/"),
        )
      ) {
  ...
  async #handleFileChange(event: FileChangeEvent) {
    ...
    const deletedShares = shares.filter(
      (share) =>
        share.path === virtualDeletedPath ||
        share.path.startsWith(virtualDeletedPath + "/"),
    );
  ...
  async addShare(virtualPath: string) {
    ...
    if (!allowedOperations.includes("share"))
