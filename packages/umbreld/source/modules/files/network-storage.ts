...
export default class NetworkStorage {
  ...
  async stop() {
    ...
    await Promise.race([
      ...
      (async () => {
        ...
        await this.#unmountAllShares().catch((error) =>
          this.logger.error("Error unmounting shares", error),
        );
      })(),
    ]);
  }
  ...
  async #isMounted(share: NetworkShare): Promise<boolean> {
    try {
      const systemMountPath =
        await this.#umbreld.files.virtualToSystemPathUnsafe(share.mountPath);
  ...
  async #mountShare(share: NetworkShare): Promise<void> {
    ...
    if (/[\r\n]/.test(share.username) || /[\r\n]/.test(share.password)) {
      throw new Error(
        "Network share username and password cannot contain newlines",
      );
    }

    // Ensure mount directory exists
    const systemMountPath = this.#umbreld.files.virtualToSystemPathUnsafe(
      share.mountPath,
    );
    ...
    try {
      ...
      credentialsDirectory = await fse.mkdtemp(
        nodePath.join(os.tmpdir(), "umbrel-cifs-credentials-"),
      );
      ...
      this.mountedShares.add(share.mountPath);
      this.logger.log(
        `Successfully mounted network share: ${smbPath} to ${share.mountPath}`,
      );
    } catch (error) {
      // Clean up the directory we created if mount fails
      this.logger.error(
        `Failed to mount network share: ${share.mountPath}, cleaning up mount directory`,
      );
      this.#unmountShare(share).catch((error) =>
        this.logger.error(
          `Failed to clean up mount directory after mount failure: ${share.mountPath}`,
          error,
        ),
      );
  ...
  async #unmountShare(share: NetworkShare): Promise<void> {
    ...
    try {
      ...
      const isParentChildOfNetwork =
        nodePath.dirname(parentDirectory) ===
        this.#umbreld.files.getBaseDirectory("/Network");
      if (isParentEmpty && isParentChildOfNetwork)
        await fse.rmdir(parentDirectory);

      this.mountedShares.delete(share.mountPath);
      this.logger.log(
        `Successfully unmounted network share: ${share.mountPath}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to unmount network share ${share.mountPath}`,
        error,
      );
    }
  }
  ...
  async addShare(newShare: Omit<NetworkShare, "mountPath">) {
    ...
    const alreadyExists = await this.getShare(mountPath)
      ...
      .catch(() => false);
    if (alreadyExists)
      throw new Error(`Share with mount path ${mountPath} already exists`);
    ...
    await this.#umbreld.store.getWriteLock(async ({ set }) => {
      ...
      if (
        shares.find(
          (existingShare) => existingShare.mountPath === share.mountPath,
        )
      )
        return;
      shares.push(share);
  ...
  async removeShare(sharePath: string) {
    ...
    await this.#umbreld.store.getWriteLock(async ({ set }) => {
      ...
      const newShares = shares.filter(
        (existingShare) => existingShare.mountPath !== sharePath,
      );
  ...
  async discoverServers() {
    const avahiBrowse =
      await $`avahi-browse --resolve --terminate _smb._tcp --parsable`;
  ...
  async discoverSharesOnServer(
    ...
    // we care about?
    const smbclient =
      await $`smbclient --list //${host} --user ${username} --password ${password} --grepable`;
  ...
  async isServerAnUmbrelDevice(address: string) {
    try {
      const responseText = (await ky(`http://${address}/trpc/system.version`, {
        timeout: 1000,
      }).text()) as any;
