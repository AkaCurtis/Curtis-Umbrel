...
describe("Reflink copy on ZFS", () => {
  ...
  test("copies a file using reflink (block cloning) instead of rsync", async () => {
    ...
    await umbreld.vm.ssh(
      `dd if=/dev/urandom of=~/umbrel/home/test-file.bin bs=1M count=${testFileSizeMb} 2>/dev/null`,
    );
    await umbreld.vm.ssh("sync");

    // Get block clone savings before copy
    const savedBefore = Number(
      (await umbreld.vm.ssh("zpool get -Hp -o value bclonesaved")).trim(),
    );
    ...
    expect(listing.files.some((f) => f.name === "test-file (2).bin")).toBe(
      true,
    );
    const sourceHash = (
      await umbreld.vm.ssh("md5sum ~/umbrel/home/test-file.bin")
    )
      .trim()
      .split(/\s+/)[0];
    const copyHash = (
      await umbreld.vm.ssh('md5sum ~/umbrel/home/"test-file (2).bin"')
    )
      .trim()
      .split(/\s+/)[0];
