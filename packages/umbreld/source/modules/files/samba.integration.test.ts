...
beforeEach(
  async () => (umbreld = await createTestUmbreld({ autoLogin: true })),
);
...
describe("shares()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.shares.query(),
  ...
  test("returns proper client-facing sharename for non /Home shares", async () => {
    ...
    // Add to shares
    await umbreld.client.files.addShare.mutate({
      path: "/Home/samba-clientname-test",
    });

    // Verify sharename is returned
    const shares = await umbreld.client.files.shares.query();
    const entry = shares.find(
      (s) => s.path === "/Home/samba-clientname-test",
    ) as any;
...
describe("#handleFileChange()", () => {
  test("automatically removes shares when directory is deleted", async () => {
    ...
    // Add both directories to shares
    await umbreld.client.files.addShare.mutate({
      path: "/Home/samba-auto-remove-test",
    });
    await umbreld.client.files.addShare.mutate({
      path: "/Home/samba-keep-test",
    });
  ...
  test("automatically removes shares when directory is renamed", async () => {
    ...
    const storedShares = await umbreld.instance.store.get("files.shares");
    const storedPaths = storedShares
      ? storedShares.map((share) => share.path)
      : [];
...
describe("addShare()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.addShare.mutate({
        path: "/Home/Documents",
      }),
    ).rejects.toThrow("Invalid token");
  });
  ...
  test("throws on non-directory paths", async () => {
    ...
    await expect(
      umbreld.client.files.addShare.mutate({
        path: "/Home/samba-test/file.txt",
      }),
  ...
  test("throws on unshareable app paths", async () => {
    ...
    await expect(
      umbreld.client.files.addShare.mutate({ path: "/Apps/bitcoin" }),
    ).rejects.toThrow("[operation-not-allowed]");
  });
  ...
  test("throws on relative paths", async () => {
    await Promise.all(
      ["", " ", ".", "..", "Home", "Home/shared-dir", "Home/../shared-dir"].map(
        (path) =>
          expect(
            umbreld.client.files.addShare.mutate({ path }),
          ).rejects.toThrow("[operation-not-allowed]"),
      ),
    );
  });
  ...
  test("throws on invalid base directory", async () => {
    await expect(
      umbreld.client.files.addShare.mutate({ path: "/Invalid/test-share" }),
    ).rejects.toThrow("[operation-not-allowed]");
  });
  ...
  test("throws error on duplicate shares", async () => {
    ...
    ).rejects.toThrow("[share-already-exists]");
  });
  ...
  test("auto-generates unique share names when basename conflicts", async () => {
    ...
    // Add both directories to shares
    await umbreld.client.files.addShare.mutate({
      path: "/Home/folder1/same-name",
    });
    await umbreld.client.files.addShare.mutate({
      path: "/Home/folder2/same-name",
    });
...
describe("removeShare()", () => {
  test("successfully removes a directory from shares", async () => {
    ...
    // Add directory to shares
    await umbreld.client.files.addShare.mutate({
      path: "/Home/samba-remove-test",
    });

    // Remove from shares
    const result = await umbreld.client.files.removeShare.mutate({
      path: "/Home/samba-remove-test",
    });
...
describe("sharePassword()", () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.sharePassword.query(),
    ).rejects.toThrow("Invalid token");
  });
...
describe("samba", () => {
  ...
  test("port is only listening where shares are active", async () => {
    ...
    // Add home directory to shares
    await expect(
      umbreld.client.files.addShare.mutate({ path: "/Home" }),
    ).resolves.toBe("/Home");
    ...
    // Remove share
    await expect(
      umbreld.client.files.removeShare.mutate({ path: "/Home" }),
    ).resolves.toBe(true);
  ...
  test("share name has (Umbrel appended)", async () => {
    // Add home directory to shares
    await expect(
      umbreld.client.files.addShare.mutate({ path: "/Home/Documents" }),
    ).resolves.toBe("/Home/Documents");
  ...
  test("client can interact with share", { retry: 5 }, async () => {
    ...
    // Test write
    await expect(
      smb.writeFile("file.txt", "hello world", { encoding: "utf8" }),
    ).resolves.toBe(undefined);

    // Test file exists on filesystem
    await expect(
      fse.exists(`${umbreld.instance.dataDirectory}/home/file.txt`),
    ).resolves.toBe(true);

    // Test read
    await expect(smb.readFile("file.txt", { encoding: "utf8" })).resolves.toBe(
      "hello world",
    );
  ...
  test("reloads config when shares are updated", async () => {
    ...
    await expect(smbDocuments.exists("non-existent-file.txt")).rejects.toThrow(
    ...
    await expect(smbHome.exists("non-existent-file.txt")).resolves.toBe(false);
    await expect(smbDocuments.exists("non-existent-file.txt")).resolves.toBe(
      false,
    );
  });
  ...
  test("doesn't allow escaping shared directories via path traversal", async () => {
    ...
    // Test ..\\ syntax works
    await expect(
      smb.readFile("test\\..\\test\\file.txt", { encoding: "utf8" }),
    ).resolves.toBe("");

    // Test traversal outside of share fails
    await expect(
      smb.readFile("..\\..\\secrets\\sensitive.txt", { encoding: "utf8" }),
    ).rejects.toThrow("STATUS_OBJECT_PATH_SYNTAX_BAD");
  });
  ...
  test("doesn't allow escaping shared directories via symlinks", async () => {
    ...
    // Add test directory to shares
    await umbreld.client.files.addShare.mutate({
      path: "/Home/symlink-traversal-test",
    });
    ...
    await expect(
      smb.readFile("normal-file", { encoding: "utf8" }),
    ).resolves.toBe("");
    await expect(
      smb.readFile("symlink-to-sensitive", { encoding: "utf8" }),
    ).rejects.toThrow("STATUS_OBJECT_NAME_NOT_FOUND");
  });
});
...
describe("wsdd2", () => {
  test("runs only while samba runs", async () => {
    ...
    await expect($`systemctl is-active wsdd2`).resolves.toMatchObject({
      stdout: "active",
    });
