...
export async function getBlockDevices() {
  ...
  const { blockdevices } = JSON.parse(stdout) as {
  ...
  for (const blockDevice of blockdevices) {
    ...
    device.isMounted = device.partitions.some(
      (partition) => partition.mountpoints.length > 0,
    );
...
export default class ExternalStorage {
  ...
  async start() {
    ...
    this.#removeDeviceChangeListener = this.#umbreld.eventBus.on(
      "system:disk:change",
      async () => {
    ...
    return this.#mountQueue.add(async () => {
      ...
      for (const device of externalStorageDevices) {
        ...
        for (const partition of device.partitions) {
          ...
          this.logger.log(
            `Mounting new partition ${device.name} ${partition.label}`,
          );
          try {
            // Derive mountpoint
            const externalBaseSystemPath =
              this.#umbreld.files.getBaseDirectory("/External");
            const sanitisedLabel = partition.label.replace(
              /[^a-zA-Z0-9 '\_\-]/g,
              "",
            );
            let systemMountpoint = nodePath.join(
              externalBaseSystemPath,
              sanitisedLabel,
            );
            systemMountpoint =
              await this.#umbreld.files.getUniqueName(systemMountpoint);
            ...
            // Log on success
            const virtualMountPoint =
              this.#umbreld.files.systemToVirtualPath(systemMountpoint);
            this.logger.log(
              `Mounted partition ${device.name} ${partition.label} as ${virtualMountPoint}`,
            );
          } catch (error) {
            // Just log the error and continue to the next partition
            this.logger.error(
              `Failed to mount partition ${device.name} ${partition.label}`,
              error,
            );
            ...
            await this.#cleanLeftOverMountPoints().catch((error) => {
              this.logger.error(
                `Failed to clean up left over mount points`,
                error,
              );
            });
          } finally {
  ...
  async unmountExternalDevice(deviceId: string, { remove = true } = {}) {
    ...
    return await this.#mountQueue.add(async () => {
      ...
      const blockDevice = externalBlockDevices.find(
      ...
      for (const partition of blockDevice.partitions) {
        ...
        await pRetry(() => $`umount --all-targets /dev/${partition.id}`, {
          ...
          this.logger.error(
            `Failed to unmount partition ${partition.id}`,
            error,
          );
          failedUnmounts = true;
        });
      ...
      if (remove)
        await fse.writeFile(`/sys/block/${deviceId}/device/delete`, "1");
  ...
  async formatExternalDevice({
    ...
    try {
      ...
      if (this.formatJobs.has(deviceId))
        throw new Error("[format-job-already-in-progress]");
      this.formatJobs.add(deviceId);

      // Check valid filessytem type
      if (filesystem !== "ext4" && filesystem !== "exfat")
      ...
      this.logger.log(
        `Formatting device ${deviceId} as ${filesystem} with label ${label}`,
      );

      // If the device is currently mounted, unmount it
      const mountedExternalDevices = await this.getMountedExternalDevices();
      const isDeviceMounted = mountedExternalDevices.some(
        (device) => device.id === deviceId,
      );
      if (isDeviceMounted)
        await this.unmountExternalDevice(deviceId, { remove: false });
      ...
      this.logger.log(
        `Removing all filesystem signatures for device ${deviceId}`,
      );
      ...
      this.logger.log(
        `Successfully formatted device ${deviceId} as ${filesystem} with label ${label}`,
      );
      return true;
    } finally {
  ...
  async #getExternalDevices() {
    ...
    return blockDevices.filter(
      (device) => device.transport === "usb" && !systemDiskIds.has(device.id),
    );
  }
    ...
    for (const blockDevice of blockDevices) {
      for (const partition of blockDevice.partitions) {
        const hasSystemMount = partition.mountpoints.some((mountpoint) =>
          systemPaths.some((systemPath) =>
            this.#isPathOnMountpoint(systemPath, mountpoint),
          ),
        );
  ...
  #getDiskIdForDeviceSource(
    ...
    for (const blockDevice of blockDevices) {
      ...
      if (blockDevice.partitions.some((partition) => partition.id === deviceId))
        return blockDevice.id;
    }
  ...
  async getMountedExternalDevices() {
    // Get all block devices
    const externalBlockDevices =
      await this.getExternalDevicesWithVirtualMountPoints();

    // Loop over devices
    for (const device of externalBlockDevices) {
      // Filter out partitions without mount points from device
      device.partitions = device.partitions.filter(
        (partition) => partition.mountpoints.length > 0,
      );
    }

    // Filter out block devices without partitions
    const mountedExternalDevices = externalBlockDevices.filter(
      (device) => device.partitions.length > 0,
    );
    ...
    for (const device of await this.getMountedExternalDevices()) {
      ...
      await this.unmountExternalDevice(device.id, { remove: false }).catch(
        (error) => {
          ...
          this.logger.error(
            `Failed to unmount external device ${device.id}`,
            error,
          );
        },
      );
  ...
  async #cleanLeftOverMountPoints() {
    ...
    for (const mountPoint of mountPoints) {
      try {
        ...
        const mountPointSystemPath = nodePath.join(
          externalBaseSystemPath,
          mountPoint,
        );
        const isMountPointEmpty =
          (await fse.readdir(mountPointSystemPath)).length === 0;
        const isMountPointUnmounted =
          (await $({ reject: false })`mountpoint ${mountPointSystemPath}`)
            .exitCode !== 0;
        ...
        this.logger.error(
          `Failed to clean up left over mount point ${mountPoint}`,
          error,
        );
      }
    }
