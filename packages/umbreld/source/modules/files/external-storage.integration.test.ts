...
vi.mock("execa", async () => {
  ...
  return {
    ...
    $: vi.fn(function () {
      ...
      if (mockResult)
        return Promise.resolve(
          typeof mockResult === "string" ? { stdout: mockResult } : mockResult,
        );
...
describe("enabled", () => {
  test("is enabled on Umbrel Home and amd64", async () => {
    await expect(
      umbreld.instance.files.externalStorage.supported(),
    ).resolves.toBe(true);
  });
...
describe("file permissions", () => {
  test("allows hard deletion of external files", async () => {
    ...
    await expect(
      umbreld.client.files.delete.mutate({
        path: "/External/My Portable SSD/file.txt",
      }),
  ...
  test("mount points are protected paths", async () => {
    ...
    // Trash
    await expect(
      umbreld.client.files.trash.mutate({ path: "/External/My Portable SSD" }),
    ).rejects.toThrow("[operation-not-allowed]");
    // Delete
    await expect(
      umbreld.client.files.delete.mutate({ path: "/External/My Portable SSD" }),
    ).rejects.toThrow("[operation-not-allowed]");
    // Move
    await expect(
...
describe("files.mountedExternalDevices", () => {
  test("throws unauthorized error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.externalDevices.query(),
  ...
  test("returns empty array when no external devices are attached", async () => {
    ...
    mockCommand = (command: string) => {
      if (command.startsWith("lsblk"))
        return JSON.stringify(LSBLK_NO_EXTERNAL_DISK);
    };
  ...
  test("excludes USB disks used by the running system", async () => {
    ...
    mockCommand = (command: string) => {
      if (command.startsWith("lsblk"))
        return JSON.stringify(getLsblkUsbSystemDisk());
...
describe("files.unmountExternalDevice", () => {
  test("throws unauthorized error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.unmountExternalDevice.mutate({
        deviceId: "sda",
      }),
    ).rejects.toThrow("Invalid token");
  });
  ...
  test("throws error on invalid device id", async () => {
    ...
    // Missing device
    await expect(
      umbreld.client.files.unmountExternalDevice.mutate({ deviceId: "sdz" }),
    ).rejects.toThrow("[invalid-device-id]");

    // Passed partition id instead of device id
    await expect(
      umbreld.client.files.unmountExternalDevice.mutate({ deviceId: "sda2" }),
    ).rejects.toThrow("[invalid-device-id]");
  });
  ...
  test("unmounts external device", async () => {
    ...
    await expect(
      umbreld.client.files.unmountExternalDevice.mutate({ deviceId: "sda" }),
    ).resolves.toBe(true);
...
describe("files.isExternalDeviceConnectedOnUnsupportedDevice", () => {
  ...
  test("returns false when on Umbrel Home and amd64 but no external devices connected", async () => {
    ...
    // Check the result
    const result =
      await umbreld.client.files.isExternalDeviceConnectedOnUnsupportedDevice.query();
...
describe("filesystem permissions", () => {
  test("/External cannot have new user created directories", async () => {
    await expect(
      umbreld.client.files.createDirectory.mutate({ path: "/External/test" }),
    ).rejects.toThrow("[operation-not-allowed]");
  });
...
const getLsblkUsbSystemDisk = () => {
  const systemDisk = structuredClone(
    LSBLK_EXTERNAL_DISK_ATTACHED.blockdevices[0],
  ) as any;
