...
export default class Search {
  ...
  async search(query: string, maxResults = 250) {
    ...
    const getBestResults = () =>
      results.sort((a, b) => b.score - a.score).slice(0, maxResults);

    // Iterate over all files in the home directory
    for await (const systemPath of this.#umbreld.files.streamContents(
      "/Home",
    )) {
      // Grab the filename
      ...
      if (score > this.matchThreshold) {
        ...
        if (results.length >= this.maxResultsDuringSearch)
    ...
    // Get file objects
    let fileReads = await Promise.allSettled(
      results.map((result) => this.#umbreld.files.status(result.systemPath)),
    );
