...
describe(`operationProgress()`, () => {
  test("throws invalid error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.operationProgress.query(),
    ).rejects.toThrow("Invalid token");
  });

  test("returns an empty array if no operations are in progress", async () => {
    await expect(
      umbreld.client.files.operationProgress.query(),
  ...
  test("returns copy progress", async () => {
    ...
    const KB = 1024;
    await fse.writeFile(
      `${testDirectory}/source/source.bin`,
      Buffer.alloc(100 * KB),
    );
    ...
    const removeListener = umbreld.instance.eventBus.on(
      ...
      (operations) =>
        void collectedEvents.push(JSON.parse(JSON.stringify(operations))),
    );
