...
export default class Files {
  ...
  async start() {
    ...
    await Promise.all(
      [...this.baseDirectories.keys()].map((baseDirectory) =>
        this.createDirectory(baseDirectory).catch((error) => {
          this.logger.error(
            `Failed to ensure directory '${baseDirectory}' exists`,
            error,
          );
        }),
      ),
    ...
    await fse.ensureDir(this.trashMetaDirectory).catch((error) => {
      this.logger.error(
        `Failed to ensure directory ${this.trashMetaDirectory} exists`,
        error,
      );
    });
    ...
    await this.watcher
      .start()
      .catch((error) => this.logger.error(`Failed to start watcher`, error));
    await this.samba
      .start()
      .catch((error) => this.logger.error(`Failed to start samba`, error));
    await this.externalStorage
      .start()
      .catch((error) =>
        this.logger.error(`Failed to start external storage`, error),
      );
    await this.networkStorage
      .start()
      .catch((error) =>
        this.logger.error(`Failed to start network storage`, error),
      );
    await this.recents
      .start()
      .catch((error) => this.logger.error(`Failed to start recents`, error));
    await this.favorites
      .start()
      .catch((error) => this.logger.error(`Failed to start favorites`, error));
    await this.thumbnails
      .start()
      .catch((error) => this.logger.error(`Failed to start thumbnails`, error));
  }

  async firstRun() {
    // Check if we've already setup favorites
    const isFavoritesInitialized =
      (await this.#umbreld.store.get("files.favorites")) === undefined;
    ...
    for (const favorite of defaultFavourites) {
      await this.createDirectory(favorite).catch((error) =>
        this.logger.error(
          `Failed to ensure directory '${favorite}' exists`,
          error,
        ),
      );
      await this.favorites
        ...
        .catch((error) =>
          this.logger.error(
            `Failed to initialize favorite '${favorite}'`,
            error,
          ),
        );
    }
  ...
  async stop() {
    ...
    await this.recents
      .stop()
      .catch((error) => this.logger.error(`Failed to stop recents`, error));
    await this.favorites
      .stop()
      .catch((error) => this.logger.error(`Failed to stop favorites`, error));
    await this.thumbnails
      .stop()
      .catch((error) => this.logger.error(`Failed to stop thumbnails`, error));
    await this.externalStorage
      .stop()
      .catch((error) =>
        this.logger.error(`Failed to stop external storage`, error),
      );
    await this.networkStorage
      .stop()
      .catch((error) =>
        this.logger.error(`Failed to stop network storage`, error),
      );
    await this.samba
      .stop()
      .catch((error) => this.logger.error(`Failed to stop samba`, error));
    await this.watcher
      .stop()
      .catch((error) => this.logger.error(`Failed to stop watcher`, error));
  }
  ...
  async createDirectory(virtualPath: string) {
    ...
    const containingDirectoryAllowedOperations =
      await this.getAllowedOperations(containingDirectory);
    if (!containingDirectoryAllowedOperations.includes("writable"))
  ...
  async list(virtualPath: string): Promise<DirectoryListing> {
    ...
    for await (const fileSystemPath of getDirectoryStream(systemPath)) {
      ...
      fileJobs.push(
        this.status(fileSystemPath).catch((error) => {
          this.logger.error(
            `Failed to get status for '${fileSystemPath}'`,
            error,
          );
          return undefined;
        }),
    ...
    const files = (await Promise.all(fileJobs)).filter(
      (file) => file !== undefined,
    ) as File[];
  ...
  async #copyWithProgress(
    ...
    if (destinationSystemPath.startsWith(sourceSystemPath))
      throw new Error("[subdir-of-self]");
    ...
    try {
      ...
      this.operationsInProgress = this.operationsInProgress.filter(
        (operation) => operation !== operationProgress,
      );
      this.#umbreld.eventBus.emit(
        "files:operation-progress",
        this.operationsInProgress,
  ...
  async copy(
    ...
    // Check if operation is allowed
    const allowedOperations = await this.getAllowedOperations(
    ...
    const destinationSystemDirectory = await this.virtualToSystemPath(
      destinationVirtualDirectory,
    );
    ...
    if (diskUsage.available < neededSpace)
    ...
    let destinationSystemPath = nodePath.join(
      destinationSystemDirectory,
      nodePath.basename(sourceSystemPath),
    );

    // Always use 'keep-both' collision handling for same directory copies
    const isSameDirectory =
      nodePath.dirname(sourceVirtualPath) === destinationVirtualDirectory;
  ...
  async move(
    ...
    if (nodePath.dirname(sourceVirtualPath) === destinationVirtualDirectory)
      return sourceVirtualPath;

    // Check if operation is allowed on source
    const allowedSourceOperations =
      await this.getAllowedOperations(sourceVirtualPath);
    if (!allowedSourceOperations.includes("move"))
      throw new Error("[operation-not-allowed]");

    // Check if operation is allowed on destination
    const allowedDestinationOperations = await this.getAllowedOperations(
      destinationVirtualDirectory,
    );
    ...
    // Error if the destination directory doesn't exist
    const targetDirectoryStats = await fse
      .stat(destinationSystemDirectory)
      .catch(() => {
        throw new Error("[destination-not-exist]");
      });

    // Add trailing slash to source path if it's a directoryso we only copy the contents
    if ((await fse.lstat(sourceSystemPath)).isDirectory())
      sourceSystemPath = `${sourceSystemPath}/`;
    ...
    const forceSlowMoveWithProgress =
      process.env.UMBRELD_FORCE_SLOW_MOVE_WITH_PROGRESS === "true";
    const isMovingAcrossFilesystems =
      sourceStats.dev !== targetDirectoryStats.dev;
  ...
  async rename(sourceVirtualPath: string, newName: string): Promise<string> {
    ...
    if (!isValidFilename(newName))
      throw new Error(`[invalid-filename] Invalid filename: '${newName}'`);
  ...
  async trash(virtualPath: string) {
    ...
    const trashSystemPath = await nodePath.join(
      trashSystemRoot,
      nodePath.basename(systemPath),
    );
    ...
    await pRetry(
      async () => {
        ...
        uniqueTrashSystemPath = await this.getUniqueName(trashSystemPath, {
          maxIndex: 1000,
        });
    ...
    await fse.writeFile(
      trashMetaSystemPath,
      JSON.stringify({ path: virtualPath } satisfies Trashmeta),
    );
  ...
  async restore(trashVirtualPath: string, { collision = "error" } = {}) {
    ...
    if (!allowedOperations.includes("restore"))
      throw new Error("[operation-not-allowed]");

    // Get the system path
    const trashSystemPath = await this.virtualToSystemPath(trashVirtualPath);
    if (!(await fse.pathExists(trashSystemPath)))
      throw new Error("[source-not-exists]");
    ...
    try {
      ...
      if (isChild)
        targetSystemPath = nodePath.join(
          targetSystemPath,
          pathSegments.slice(2).join("/"),
        );
    } catch (error) {
      if ((error as Error)?.message?.includes("ENOENT"))
        throw new Error("[trash-meta-not-exists]");
      throw error;
    }

    // Handle name conflicts
    if (collision === "keep-both")
      targetSystemPath = await this.getUniqueName(targetSystemPath);
  ...
  async emptyTrash() {
    ...
    for await (const systemPath of getDirectoryStream(trashDirectory)) {
      await fse.remove(systemPath).catch((error) => {
        this.logger.error(
          `Failed to remove '${nodePath.basename(systemPath)}' from trash`,
          error,
        );
        success = false;
      });
    }
    for await (const systemPath of getDirectoryStream(
      this.trashMetaDirectory,
    )) {
      await fse.remove(systemPath).catch((error) => {
        this.logger.error(
          `Failed to remove '${nodePath.basename(systemPath)}' from trash meta`,
          error,
        );
        success = false;
      });
  ...
  async delete(virtualPath: string) {
    ...
    if (virtualPath.startsWith("/External/")) {
      ...
      for (const share of shares) {
        if (share.path.startsWith(virtualPath))
  ...
  async getAllowedOperations(virtualPath: string): Promise<FileOperation[]> {
    ...
    if (isFile) {
      if (this.archive.isUnarchiveable(virtualPath))
        operations.add("unarchive");
    }
  ...
  async getUniqueName(systemPath: string, { maxIndex = 100 } = {}) {
    ...
    while (await fse.pathExists(uniquePath)) {
      ...
      uniquePath = nodePath.join(
        path,
        `${name} (${index})${extension ? extension : ""}`,
      );
      index++;
    }
  ...
  virtualToSystemPathUnsafe(virtualPath: string) {
    ...
    if (!nodePath.posix.isAbsolute(virtualPath))
    ...
    if (!basePath)
      throw new Error(
        `[invalid-base] No valid base directory found for path: ${virtualPath}`,
      );
  ...
  async virtualToSystemPath(virtualPath: string) {
    ...
    const realPath = systemPath.replace(
      deepestExistingPath,
  ...
  systemToVirtualPath(systemPath: string) {
    ...
    throw new Error(
      `[invalid-path] Path '${systemPath}' is not within any base directory`,
    );
  }
...
async function getDeepestExistingPath(path: string) {
  ...
  while (true) {
    ...
    if (parentPath === currentPath)
      throw new Error(
        `[cant-find-root] Can't validate path if entire tree doesn't exist`,
      );
...
export async function* getDirectoryStream(
  directory: string,
  ...
  try {
    ...
    for await (const file of directoryListing)
      yield nodePath.join((file as any).parentPath, file.name);
  } finally {
