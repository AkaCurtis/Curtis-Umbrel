...
describe("files.search()", () => {
  test('throws "Invalid token" error without auth token', async () => {
    await expect(
      umbreld.unauthenticatedClient.files.search.query({ query: "anything" }),
  ...
  test("finds files that match the query", async () => {
    ...
    // Perform the search
    const results = await umbreld.client.files.search.query({
      query: "hello-world",
    });
  ...
  test("respects maxResults", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      fileCreationPromises.push(
        fse.writeFile(`${limitDir}/alpha-${i}.txt`, String(i)),
      );
    }
