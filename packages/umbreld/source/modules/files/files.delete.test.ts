...
test("delete() throws invalid error without auth token", async () => {
  await expect(
    umbreld.unauthenticatedClient.files.delete.mutate({
      path: "/Trash/test-file.txt",
    }),
...
test("delete() silently succeeds when trying to delete a non-existent file", async () => {
  ...
  await expect(
    umbreld.client.files.delete.mutate({
      path: "/Trash/non-existent-file.txt",
    }),
  ).resolves.toBe(true);
});
...
test("delete() throws error when trying to delete a file outside of trash", async () => {
  ...
  await expect(fse.pathExists(`${homeDir}/protected-file.txt`)).resolves.toBe(
    true,
  );

  // Attempt to delete the file outside of trash
  await expect(
    umbreld.client.files.delete.mutate({ path: "/Home/protected-file.txt" }),
  ).rejects.toThrow();
...
test("delete() can delete a nested file in trash", async () => {
  ...
  await expect(
    fse.pathExists(`${trashDir}/test-dir/nested/file2.txt`),
  ).resolves.toBe(true);

  // Delete the nested file
  await umbreld.client.files.delete.mutate({
    path: "/Trash/test-dir/nested/file2.txt",
  });
  ...
  await expect(fse.pathExists(`${trashDir}/test-dir/nested`)).resolves.toBe(
    true,
  );
  await expect(fse.pathExists(`${trashDir}/test-dir/file1.txt`)).resolves.toBe(
    true,
  );
});
...
test("delete() handles errors gracefully", async () => {
  ...
  await expect(
    umbreld.client.files.delete.mutate({ path: "/Trash/test-file.txt" }),
  ).resolves.toBe(false);
...
test("delete() can handle files with special characters in the name", async () => {
  ...
  await fse.writeFile(
    `${metaDir}/${specialFileName}.json`,
    JSON.stringify({ path: `/Home/${specialFileName}` }),
  );

  // Verify the file exists
  await expect(fse.pathExists(`${trashDir}/${specialFileName}`)).resolves.toBe(
    true,
  );

  // Delete the file
  await umbreld.client.files.delete.mutate({
    path: `/Trash/${specialFileName}`,
  });
...
test("delete() throws on symlink traversal attempt", async () => {
  ...
  await expect(fse.pathExists(`${targetDir}/important-file.txt`)).resolves.toBe(
    true,
  );
