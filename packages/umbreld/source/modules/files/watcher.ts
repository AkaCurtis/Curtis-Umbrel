...
export default class Watcher {
        ...
        (error, events) => {
          if (error)
            return this.logger.error(
              `Failed to watch directory '${virtualPath}'`,
              error,
            );
          for (const event of events)
            this.#umbreld.eventBus.emit("files:watcher:change", event);
        },
      );
  ...
  async #healthCheck() {
    const systemPath =
      await this.#umbreld.files.virtualToSystemPath(HEALTH_CHECK_PATH);
    ...
    try {
      ...
      const eventReceived = new Promise<void>((resolve) => {
        ...
        const removeListener = this.#umbreld.eventBus.on(
      ...
      const result = await Promise.race([
        eventReceived.then(() => "ok" as const),
        timeout,
      ]);
