...
describe("archive()", () => {
  test("throws unauthorized error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.archive.mutate({
        paths: ["/Home/test.txt"],
      }),
  ...
  test("throws error when paths are in different directories", async () => {
    ...
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/dir1/file1.txt`,
      "content1",
    );
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/dir2/file2.txt`,
      "content2",
    );
  ...
  test("successfully creates a zip archive from a single file", async () => {
    // Create a test file
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/test-file.txt`,
      "test content",
    );
  ...
  test("successfully creates a zip archive from multiple files", async () => {
    // Create test files
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/file1.txt`,
      "content1",
    );
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/file2.txt`,
      "content2",
    );
  ...
  test("successfully creates a zip archive from a directory", async () => {
    ...
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/test-dir/file1.txt`,
      "content1",
    );
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/test-dir/file2.txt`,
      "content2",
    );
    await fse.ensureDir(
      `${umbreld.instance.dataDirectory}/home/test-dir/subdir`,
    );
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/test-dir/subdir/file3.txt`,
      "content3",
    );
  ...
  test("creates a uniquely named zip archive when a file with the same name already exists", async () => {
    // Create a test file
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/unique-test.txt`,
      "test content",
    );

    // Create a zip file that would conflict with the generated name
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/unique-test.txt.zip`,
      "existing zip",
    );
    ...
    const originalZipContent = await fse.readFile(
      `${umbreld.instance.dataDirectory}/home/unique-test.txt.zip`,
      "utf8",
    );
...
describe("unarchive()", () => {
  test("throws unauthorized error without auth token", async () => {
    await expect(
      umbreld.unauthenticatedClient.files.unarchive.mutate({
        path: "/Home/test.zip",
      }),
    ).rejects.toThrow("Invalid token");
  });
  ...
  test("throws error on unsupported file format", async () => {
    ...
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/test.txt`,
  ...
  test("extracts a zip archive correctly", async () => {
    ...
    await expect(fse.readFile(`${extractDir}/file1.txt`, "utf8")).resolves.toBe(
      "content1",
    );
    await expect(fse.readFile(`${extractDir}/file2.txt`, "utf8")).resolves.toBe(
      "content2",
    );
    await expect(
      fse.readFile(`${extractDir}/subdir/file3.txt`, "utf8"),
    ).resolves.toBe("content3");
  });
  ...
  test("creates a unique directory name if target already exists", async () => {
    ...
    await fse.writeFile(
      `${umbreld.instance.dataDirectory}/home/conflict-test/existing-file.txt`,
      "existing content",
    );
    ...
    await expect(
      fse.readFile(
        `${umbreld.instance.dataDirectory}/home/conflict-test (2)/new-file.txt`,
        "utf8",
      ),
  ...
  test("handles files with special characters in name", async () => {
    ...
    await expect(fse.readFile(extractedFile, "utf8")).resolves.toBe(
      "special content",
    );
  });
  ...
  for (const type of archiveTypes) {
    test(`extracts a ${type.extension} archive correctly`, async () => {
      ...
      await expect(fse.readdir(sourceDir)).resolves.toStrictEqual([
        "test-file.txt",
      ]);
      await expect(
        fse.readFile(`${sourceDir}/test-file.txt`, "utf8"),
