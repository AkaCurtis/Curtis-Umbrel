...
test.sequential(
  "exists() returns false when no user is registered",
  async () => {
...
test.sequential(
  "login() throws invalid error when no user is registered",
  async () => {
    await expect(
...
test.sequential(
  "register() throws error if username is not supplied",
  async () => {
    await expect(
...
test.sequential(
  "register() throws error if password is not supplied",
  async () => {
    await expect(
      umbreld.client.user.register.mutate({
...
test.sequential(
  "register() throws error if password is below min length",
  async () => {
    await expect(
      umbreld.client.user.register.mutate({
        name: testUserCredentials.name,
...
test.sequential("register() creates a user", async () => {
  await expect(
    umbreld.client.user.register.mutate(testUserCredentials),
  ).resolves.toBe(true);
});
...
test.sequential(
  "register() throws an error if the user is already registered",
  async () => {
    await expect(
      umbreld.client.user.register.mutate(testUserCredentials),
    ).rejects.toThrow("Attempted to register when user is already registered");
  },
);

test.sequential("login() throws an error for invalid credentials", async () => {
  await expect(
    umbreld.client.user.login.mutate({ password: "usdtothemoon" }),
  ).rejects.toThrow("Incorrect password");
});

test.sequential(
  "login() throws an error if password is not supplied",
  async () => {
    await expect(umbreld.client.user.login.mutate({} as any)).rejects.toThrow(
      /invalid_type.*password/s,
    );
  },
);

test.sequential("renewToken() throws if we're not logged in", async () => {
  await expect(umbreld.client.user.renewToken.mutate()).rejects.toThrow(
    "Invalid token",
  );
});

test.sequential(
  "isLoggedIn() returns false if we're not logged in",
  async () => {
...
test.sequential(
  "renewToken() returns a new token when we're logged in",
  async () => {
...
test.sequential("generateTotpUri() returns a 2FA URI", async () => {
  await expect(umbreld.client.user.generateTotpUri.query()).resolves.toContain(
    "otpauth://totp/Umbrel?secret=",
  );
});

test.sequential(
  "generateTotpUri() returns a unique 2FA URI each time",
  async () => {
...
test.sequential("login() requires 2FA token if enabled", async () => {
  await expect(
    umbreld.client.user.login.mutate(testUserCredentials),
...
test.sequential("set() sets the user's name", async () => {
  await expect(umbreld.client.user.set.mutate({ name: "Hal" })).resolves.toBe(
    true,
  );
  await expect(umbreld.client.user.get.query()).resolves.toMatchObject({
    name: "Hal",
  ...
  // Revert name change
  await expect(
    umbreld.client.user.set.mutate({ name: testUserCredentials.name }),
  ).resolves.toBe(true);
  await expect(umbreld.client.user.get.query()).resolves.toMatchObject({
    name: testUserCredentials.name,
  });
...
test.sequential("set() sets the user's language", async () => {
  await expect(
    umbreld.client.user.set.mutate({ language: testUserLanguage }),
  ).resolves.toBe(true);
  await expect(umbreld.client.user.get.query()).resolves.toMatchObject({
    language: testUserLanguage,
  });
});

test.sequential("set() sets the user's wallpaper", async () => {
  await expect(
    umbreld.client.user.set.mutate({ wallpaper: "1.jpg" }),
  ).resolves.toBe(true);
  await expect(umbreld.client.user.get.query()).resolves.toMatchObject({
    wallpaper: "1.jpg",
  });
  ...
  await expect(
    umbreld.client.user.set.mutate({ wallpaper: "2.jpg" }),
  ).resolves.toBe(true);
  await expect(umbreld.client.user.get.query()).resolves.toMatchObject({
    wallpaper: "2.jpg",
  });
...
test.sequential("set() throws on unknown property", async () => {
  ...
  await expect(umbreld.client.user.set.mutate({ foo: "bar" })).rejects.toThrow(
    "unrecognized_keys",
  );
});
...
test.sequential("language() is publically available", async () => {
  await expect(
    umbreld.unauthenticatedClient.user.language.query(),
  ).resolves.toBe(testUserLanguage);
});

test.sequential("language() returns the user's language", async () => {
  await expect(umbreld.client.user.language.query()).resolves.toBe(
    testUserLanguage,
  );
});
...
test.sequential("changePassword() throws on inavlid oldPassword", async () => {
  await expect(
    umbreld.client.user.changePassword.mutate({
      oldPassword: "fiat4lyfe",
      newPassword: "usdtothemoon",
    }),
  ).rejects.toThrow("Incorrect password");
});
