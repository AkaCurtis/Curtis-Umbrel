...
async function getDefaultWallpaper(): Promise<string> {
  ...
  if (
    device.productName === "Umbrel Home" ||
    device.productName === "Umbrel Pro"
  ) {
    return "22";
  }
  return "18";
...
export default router({
  ...
  register: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (await ctx.user.exists()) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "Attempted to register when user is already registered",
        });
      }
      ...
      const hasRaidDetails =
        input.raidType && input.raidDevices && input.raidDevices.length > 0;
      if (isUmbrelPro && !hasRaidDetails) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "RAID devices are required for Umbrel Pro",
        });
      }
  ...
  generateTotpUri: privateProcedure.query(async () =>
    totp.generateUri("Umbrel", "umbrel.local"),
  ),
  ...
  set: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.temperatureUnit)
        await ctx.user.setTemperatureUnit(input.temperatureUnit);
