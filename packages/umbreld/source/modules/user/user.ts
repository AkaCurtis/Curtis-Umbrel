...
export default class User {
  ...
  async setPassword(password: string) {
    ...
    const hashedPassword = (await bcrypt.hash(password, saltRounds)).replace(
      /^\$2a\$/,
      "$2b$",
    );
  ...
  async syncSystemPassword() {
    try {
      ...
      const hasUmbrelSystemUser = userFile
        .split("\n")
        .some((line) => line.startsWith("umbrel:"));
      const hashedPassword =
        (await this.#store.get("user.hashedPassword")) || "";
      ...
      if (hasUmbrelSystemUser && hashedPassword.length > 0) {
        ...
        if (supportsBcrypt) {
          ...
          if (bcryptRegex.test(hashedPassword)) {
            ...
            await $({
              input: `umbrel:${systemPassword}`,
            })`chpasswd --encrypted`;
            this.logger.log(`Synced system password`);
          } else {
            this.logger.error(
              `Failed to update system password: invalid password hash`,
            );
          }
        } else {
          this.logger.error(
            `Failed to update system password: bcrypt not supported`,
          );
        }
      }
    } catch (error) {
  ...
  async setHashedPassword(hashedPassword: string) {
    if (await this.#umbreld.hardware.raid.hasConfigStore()) {
      await this.#umbreld.hardware.raid.configStore.set(
        "user.hashedPassword",
        hashedPassword,
      );
    }
