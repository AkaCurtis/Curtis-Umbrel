...
export async function runPreMigrationChecks(
  ...
  if (onlyAllowUmbrelHardware) {
    const [isHome, isPro] = await Promise.all([
      isUmbrelHome(),
      umbreld.hardware.umbrelPro.isUmbrelPro(),
    ]);
    if (!isHome && !isPro) {
      throw new Error(
        "This feature is only supported on Umbrel Home or Umbrel Pro hardware",
      );
    }
  }
  ...
  if (await fse.exists(`${externalUmbrelInstall}/umbrel.yaml`)) {
    ...
    const data = await fse.readFile(
      `${externalUmbrelInstall}/umbrel.yaml`,
      "utf8",
    );
  ...
  const validVersionRange =
    ...
    semver.gte(umbreld.version, semver.coerce(externalVersion)!);
  if (!validVersionRange) {
    throw new Error(
      `Cannot migrate umbrelOS ${externalVersion} data into an umbrelOS ${umbreld.version} install.`,
    );
  }
  ...
  if (free < required) {
    throw new Error(
      `Not enough storage available. ${bytesToGB(free)} GB free, ${bytesToGB(required)} GB required.`,
    );
  }
