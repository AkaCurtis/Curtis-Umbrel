...
class Migration {
  ...
  async finalizeMenderToRugixStateMigration() {
    ...
    const isSymbolicLink = (
      await fse.lstat(osPersistentOverlayPath)
    ).isSymbolicLink();
    ...
    const menderToRugixMigrationAttempt =
      (await this.umbreld.store.get("migration.menderToRugixAttempt")) || 0;
    if (menderToRugixMigrationAttempt >= 3) {
      this.logger.error(
        "Mender to Rugix state migration has been attempted 5 times, giving up to prevent a boot loop!",
      );
      return { reboot: false };
    }

    // Increment the attempt count
    await this.umbreld.store.set(
      "migration.menderToRugixAttempt",
      menderToRugixMigrationAttempt + 1,
    );
  ...
  async migrateLegacyLinuxData() {
    ...
    await runPreMigrationChecks(
      currentInstall,
      externalUmbrelInstall as string,
      this.umbreld,
      false,
    );
  ...
  async activateImportedDataDirectory() {
    ...
    await fse.move(temporaryData, this.umbreld.dataDirectory, {
      overwrite: true,
    });
  }
  ...
  async migrateLegacyData() {
    ...
    if (legacyData.installedApps)
      await this.umbreld.store.set("apps", legacyData.installedApps);
    if (legacyData.remoteTorAccess)
      await this.umbreld.store.set("torEnabled", legacyData.remoteTorAccess);

    // Showcase widgets for migrating users
    await this.umbreld.store.set("widgets", [
      "umbrel:memory",
      "umbrel:system-stats",
      "umbrel:storage",
    ]);
  ...
  async migrateBackThatMacUpPort() {
    ...
    const isBackThatMacUpInstalled = (
      (await this.umbreld.store.get("apps")) || []
    ).includes("back-that-mac-up");
  ...
  async migrateDownloadsDirectory() {
    ...
    const newDownloadsPathHasData =
      (await fse.exists(newDownloadsPath)) &&
      (await fse.readdir(newDownloadsPath)).length > 0;
  ...
  async start() {
    ...
    try {
      ...
      this.logger.error(
        `Failed to finalize Mender to Rugix state migration`,
        error,
      );
    }
