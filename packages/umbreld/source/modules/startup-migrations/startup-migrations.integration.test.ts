...
async function waitForReportedSystemVersion() {
  ...
  await pWaitFor(
    async () => {
      ...
      try {
        reportedVersion =
          await createUnauthenticatedClient(port).system.version.query();
        return true;
      } catch {
        return false;
      }
...
beforeEach(
  async () => (umbreld = await createTestUmbreld({ autoStart: false })),
);
...
test("first run writes version without adding a notification", async () => {
  ...
  await expect(
    umbreld.instance.store.get("previousVersion"),
...
test("OS update adds a notification", async () => {
  ...
  await expect(umbreld.instance.store.get("previousVersion")).resolves.toBe(
    oldVersion,
  );
...
test("OS update after a previous boot reports current and previous versions", async () => {
  ...
  await expect(umbreld.instance.store.get("version")).resolves.toBe(
    currentVersion,
  );
