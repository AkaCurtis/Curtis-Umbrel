...
describe("RAID mount failure detection", () => {
  ...
  test("checkRaidMountFailure returns false when RAID is healthy", async () => {
    const failure =
      await umbreld.unauthenticatedClient.hardware.raid.checkRaidMountFailure.query();
  ...
  test("checkRaidMountFailureDevices returns expected device status with one SSD disconnected", async () => {
    const devices =
      await umbreld.unauthenticatedClient.hardware.raid.checkRaidMountFailureDevices.query();
    expect(devices).toHaveLength(2);
  ...
  test("can restart from recovery mode", async () => {
    ...
    await pWaitFor(
      async () => {
        const status = await umbreld.unauthenticatedClient.system.status
          .query()
          .catch(() => "");
