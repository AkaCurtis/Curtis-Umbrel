...
async function getSmartData(devicePath: string): Promise<SmartData> {
  try {
    ...
    if (
      typeof data.nvme_smart_health_information_log?.percentage_used ===
      "number"
    ) {
    ...
    const smartStatus =
      data.smart_status?.passed === false ? "unhealthy" : "healthy";
...
async function getDevicePciSlotNumber(
  deviceName: string,
...
export async function getInternalStorageDevices(): Promise<StorageDevice[]> {
  ...
  const { stdout } =
    await $`lsblk --output NAME,MODEL,SERIAL,SIZE,TYPE,TRAN,ROTA --json --bytes`;
  ...
  const internalBlockDevices = blockdevices.filter(
    (device) =>
      device.type === "disk" && supportedTransports.includes(device.tran ?? ""),
  );
...
export default class InternalStorage {
  ...
  async getDevices(): Promise<StorageDevice[]> {
    ...
    if (await this.#umbreld.hardware.umbrelPro.isUmbrelPro()) {
      ...
      devices = [
        ...ssdDevices.map((device) => ({
          ...device,
          slot: this.#umbreld.hardware.umbrelPro.getSsdSlotFromPciSlotNumber(
            device.pciSlotNumber,
          ),
        })),
        ...otherDevices,
      ];
    ...
    const haveMissingSlots = devices.some(
      (device) => device.slot === undefined,
    );
