...
function expectSizeToBeWithinTenPercentOfGb(
...
describe("RAID HDD accelerator replacement with an offline device", () => {
  ...
  let replaceSubscription: ReturnType<
    typeof umbreld.subscribeToEvents<ReplaceStatus>
  >;
  ...
  test("removes one accelerator SSD and boots back up degraded", async () => {
    ...
    expect(
      status.accelerator?.devices?.some(
        (device) =>
          device.id === acceleratorDeviceId1 && device.status !== "ONLINE",
      ),
    ).toBe(true);
  });
  ...
  test("reports a non-zero accelerator size while degraded", async () => {
    ...
    expectSizeToBeWithinTenPercentOfGb(
      status.accelerator?.specialSize,
      expectedMirroredAcceleratorSpecialSizeGb,
    );
  });

  test("replaces one accelerator SSD while it is offline", async () => {
    replaceSubscription = umbreld.subscribeToEvents<ReplaceStatus>(
  ...
  test("waits for accelerator replacement to complete while it is offline", async () => {
    await pWaitFor(
      async () => {
        ...
        const statusHasFinished = ["finished", "canceled"].includes(
          status.replace?.state ?? "",
        );
        const eventsHaveFinished = replaceSubscription.collected.some((e) =>
          ["finished", "canceled"].includes(e.state),
        );
  ...
  test("reports accelerator replace events while it is offline", () => {
    ...
    for (let i = 1; i < progressFromEvents.length; i++) {
      expect(progressFromEvents[i]).toBeGreaterThanOrEqual(
        progressFromEvents[i - 1],
      );
    }
  ...
  test("reports accelerator replace status while it is offline", () => {
    ...
    const progressFromStatus = replaceStatusCalls.map(
      (status) => status.progress,
    );
    for (let i = 1; i < progressFromStatus.length; i++) {
      expect(progressFromStatus[i]).toBeGreaterThanOrEqual(
        progressFromStatus[i - 1],
      );
    }

    expect(replaceStatusCalls.at(-1)).toMatchObject({
      state: "finished",
      progress: 100,
    });
  });
  ...
  test("reports the replacement accelerator device after the offline replace", async () => {
    const status = await umbreld.client.hardware.raid.getStatus.query();
    const acceleratorIds = status
      .accelerator!.devices!.map((device) => device.id)
      .sort();
    expect(acceleratorIds).toEqual(
      [acceleratorDeviceId2, replacementAcceleratorDeviceId].sort(),
    );
    expectSizeToBeWithinTenPercentOfGb(
      status.accelerator?.l2arcSize,
      expectedStripedAcceleratorL2arcSizeGb,
    );
    expectSizeToBeWithinTenPercentOfGb(
      status.accelerator?.specialSize,
      expectedMirroredAcceleratorSpecialSizeGb,
