...
describe("RAID SATA SSD failsafe behavior", () => {
  ...
  test("detects SATA SSDs as SSD type with sata transport", async () => {
    ...
    const sataSsds = devices.filter(
      (device) => device.type === "ssd" && device.transport === "sata",
    );
  ...
  test("registers user with 2-disk SATA SSD failsafe config (triggers reboot)", async () => {
    await umbreld.signup({
      raidDevices: [sataSsdDeviceId1, sataSsdDeviceId2],
      raidType: "failsafe",
    });
  });
  ...
  test("rejects addMirror on SATA SSD raidz failsafe arrays", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [sataSsdDeviceId1, sataSsdDeviceId2],
      }),
  ...
  test("rejects adding HDD to SATA SSD failsafe arrays", async () => {
    await expect(
      umbreld.client.hardware.raid.addDevice.mutate({ deviceId: hddDeviceId }),
  ...
  test("reports all SATA SSDs in failsafe raidz array", async () => {
    ...
    expect(deviceIds).toEqual(
      [sataSsdDeviceId1, sataSsdDeviceId2, sataSsdDeviceId3].sort(),
    );
  });
});
