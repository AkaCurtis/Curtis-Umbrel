...
describe("RAID HDD multi-disk storage to failsafe mirror transition", () => {
  ...
  let transitionSubscription: ReturnType<
    typeof umbreld.subscribeToEvents<FailsafeTransitionStatus>
  >;
  let failed = false;
  ...
  test("detects both HDDs", async () => {
    const devices =
      await umbreld.unauthenticatedClient.hardware.internalStorage.getDevices.query();
  ...
  test("creates marker directory to verify data consistency", async () => {
    await umbreld.client.files.createDirectory.mutate({
      path: "/Home/multi-transition-marker",
    });
    const listing = await umbreld.client.files.list.query({ path: "/Home" });
    expect(
      listing.files.some((f) => f.name === "multi-transition-marker"),
    ).toBe(true);
  });

  test("writes test data so rebuild takes time", async () => {
    await umbreld.vm.ssh(
      "dd if=/dev/urandom of=~/test-data.bin bs=1M count=2000",
    );
  });
  ...
  test("detects all available HDDs for transition", async () => {
    const devices =
      await umbreld.client.hardware.internalStorage.getDevices.query();
    const hdds = devices.filter((d) => d.type === "hdd");
  ...
  test("rejects addMirror endpoint before mirror topology exists", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [hddDeviceId3, hddDeviceId4],
      }),
  ...
  test("rejects SSD raidz transition endpoint for HDD arrays", async () => {
    await expect(
      umbreld.client.hardware.raid.transitionToFailsafeRaidz.mutate({
        newDeviceId: hddDeviceId3,
      }),
    ).rejects.toThrow(
  ...
  test("rejects transition with wrong number of devices (1 instead of 2)", async () => {
    await expect(
      ...
      "Need exactly 2 mirror pair(s) to transition to failsafe mode",
    );
  });
  ...
  test("rejects transition with duplicate existing devices in pairs", async () => {
    await expect(
      ...
      `Duplicate existing device in mirror pairs: ${hddDeviceId1}`,
    );
  });
  ...
  test("rejects transition when a new mirror device does not exist", async () => {
    ...
    await expect(
      ...
      `Device not found: /dev/disk/by-umbrel-id/${missingDeviceId}`,
    );
  });
  ...
  test("transitions 2-disk storage to failsafe with 2 new HDDs without rebooting", async () => {
    ...
    transitionSubscription =
      umbreld.subscribeToEvents<FailsafeTransitionStatus>(
  ...
  test("reports mirror failsafe mode with all 4 devices", async () => {
    ...
    expect(deviceIds).toEqual(
      [hddDeviceId1, hddDeviceId2, hddDeviceId3, hddDeviceId4].sort(),
    );
  });
  ...
  test("preserves explicit mirror pair mapping in pool status output", async () => {
    ...
    const expectedPairs = [
      [hddDeviceId1, hddDeviceId3].sort(),
      [hddDeviceId2, hddDeviceId4].sort(),
    ].sort((a, b) => a.join(",").localeCompare(b.join(",")));
