...
const internalStorage = router({
  ...
  getDevices: publicProcedureWhenNoUserExists.query(async ({ ctx }) =>
    ctx.umbreld.hardware.internalStorage.getDevices(),
  ),
});
...
const raid = router({
  ...
  checkRaidMountFailure: publicProcedure.query(async ({ ctx }) =>
    ctx.umbreld.hardware.raid.checkRaidMountFailure(),
  ),

  // Get details about why RAID mount failed
  checkRaidMountFailureDevices: publicProcedureWhenNoUserExists.query(
    async ({ ctx }) => ctx.umbreld.hardware.raid.checkRaidMountFailureDevices(),
  ),

  // Get RAID pool status
  getStatus: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.hardware.raid.getStatus(),
  ),
  ...
  setup: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.setup(
        input.devices,
        input.raidType,
  ...
  addDevice: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.addDevice(input.deviceId),
    ),
  ...
  addMirror: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.addMirror(input.deviceIds),
    ),
  ...
  addAccelerator: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.addAccelerator(input.deviceIds),
    ),
  ...
  replaceDevice: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.replaceDevice(input.oldDevice, input.newDevice),
    ),
  ...
  transitionToFailsafeRaidz: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.transitionToFailsafeRaidz(input.newDeviceId),
    ),
  ...
  transitionToFailsafeMirror: privateProcedure
    .input(
      z.object({
        pairs: z
          .array(
            z.object({ existingDeviceId: z.string(), newDeviceId: z.string() }),
          )
          .min(1),
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.raid.transitionToFailsafeMirror(
        input.pairs,
        input.acceleratorDeviceId,
      ),
    ),
});
...
const umbrelPro = router({
  ...
  isUmbrelPro: privateProcedure.query(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.isUmbrelPro(),
  ),

  // TODO: These are exposed in factory builds for hardware testing. Remove when we have a better solution.

  // LED control
  setLedOff: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.setLedOff(),
  ),
  setLedStatic: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.setLedStatic(),
  ),
  setLedDefault: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.setLedDefault(),
  ),
  setLedColor: privateProcedure
    .input(z.object({ red: z.number(), green: z.number(), blue: z.number() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.umbrelPro.setLedColor(input),
    ),
  setLedWhite: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.setLedWhite(),
  ),
  setLedBlinking: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.setLedBlinking(),
  ),
  setLedBreathe: privateProcedure
    .input(z.object({ duration: z.number().optional() }).optional())
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.umbrelPro.setLedBreathe(input?.duration),
    ),
  ...
  setFanManagementEnabled: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.umbrelPro.setFanManagementEnabled(input.enabled),
    ),
  setMinFanSpeed: privateProcedure
    .input(z.object({ percent: z.number() }))
    .mutation(async ({ ctx, input }) =>
      ctx.umbreld.hardware.umbrelPro.setMinFanSpeed(input.percent),
    ),
  ...
  clearResetBootFlag: privateProcedure.mutation(async ({ ctx }) =>
    ctx.umbreld.hardware.umbrelPro.clearResetBootFlag(),
  ),
});
