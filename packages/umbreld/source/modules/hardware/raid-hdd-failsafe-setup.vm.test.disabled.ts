...
describe("RAID HDD failsafe setup and expansion", () => {
  ...
  test("creates marker directory to verify data consistency", async () => {
    await umbreld.client.files.createDirectory.mutate({
      path: "/Home/failsafe-setup-marker",
    });
    const listing = await umbreld.client.files.list.query({ path: "/Home" });
    expect(listing.files.some((f) => f.name === "failsafe-setup-marker")).toBe(
      true,
    );
  });
  ...
  test("rejects adding single HDD to mirror pool", async () => {
    await expect(
      umbreld.client.hardware.raid.addDevice.mutate({ deviceId: hddDeviceId3 }),
    ).rejects.toThrow(
  ...
  test("rejects addMirror with duplicate devices", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [hddDeviceId3, hddDeviceId3],
      }),
  ...
  test("rejects addMirror when one device is already in the RAID array", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [hddDeviceId1, hddDeviceId3],
      }),
  ...
  test("rejects addMirror when a device does not exist", async () => {
    ...
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [missingDeviceId, hddDeviceId3],
      }),
    ).rejects.toThrow(
  ...
  test("rejects adding SSD to mirror pool", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [ssdDeviceId, hddDeviceId3],
      }),
    ).rejects.toThrow("Cannot mix SSDs and HDDs");
  });
  ...
  test("detects new HDDs after reboot", async () => {
    ...
    hddDeviceId5 = newHdds.find(
      (d) => d.id !== hddDeviceId3 && d.id !== hddDeviceId4,
    )!.id!;
  ...
  test("adds mirror pair to failsafe array", async () => {
    await umbreld.client.hardware.raid.addMirror.mutate({
      deviceIds: [hddDeviceId4, hddDeviceId5],
    });
  });
  ...
  test("reports correct status with 4 devices", async () => {
    ...
    expect(deviceIds).toEqual(
      [hddDeviceId1, hddDeviceId2, hddDeviceId4, hddDeviceId5].sort(),
    );
  });
