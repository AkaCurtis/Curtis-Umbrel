...
describe("RAID storage to failsafe transition", () => {
  ...
  let syncSubscription: ReturnType<
    typeof umbreld.subscribeToEvents<FailsafeTransitionStatus>
  >;
  let rebuildSubscription: ReturnType<
    typeof umbreld.subscribeToEvents<FailsafeTransitionStatus>
  >;
  ...
  test("creates marker directory to verify data migration", async () => {
    await umbreld.client.files.createDirectory.mutate({
      path: "/Home/migration-test-directory",
    });
    const listing = await umbreld.client.files.list.query({ path: "/Home" });
    expect(
      listing.files.some((f) => f.name === "migration-test-directory"),
    ).toBe(true);
  });
  ...
  test("receives transition progress (sync phase 0-50%) via WebSocket and status endpoint", async () => {
    ...
    syncSubscription = umbreld.subscribeToEvents<FailsafeTransitionStatus>(
    ...
    const syncStatusCalls = transitionStatusCalls.filter(
      (s) => s.progress <= 50,
    );
  ...
  test("waits for VM to come back up after transition and logs in", async () => {
    ...
    rebuildSubscription = umbreld.subscribeToEvents<FailsafeTransitionStatus>(
      "raid:failsafe-transition-progress",
    );
  });
  ...
  test("receives transition progress (rebuild phase 51-100%) via WebSocket and status endpoint", async () => {
    ...
    const rebuildStatusCalls = transitionStatusCalls.filter(
      (s) => s.progress > 50,
    );
