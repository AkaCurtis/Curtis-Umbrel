...
const toGB = (bytes?: number) =>
  bytes != null ? `${(bytes / 1e9).toFixed(2)} GB` : "N/A";
const logSpace = (
  label: string,
  s: {
    totalSpace?: number;
  ...
  describe("Incremental expansion tests", () => {
    ...
    test("writes 2GB of data in storage mode", async () => {
      ...
      await umbreld.vm.ssh(
        "dd if=/dev/urandom of=~/data-1ssd.bin bs=1M count=1000",
      );
      ...
      console.log(
        `After 1GB write: usedSpace=${toGB(status.usedSpace)}, remaining to 2GB=${toGB(remaining)}`,
      );
      if (remaining > 0) {
        const remainingMiB = Math.ceil(remaining / (1024 * 1024));
        await umbreld.vm.ssh(
          `dd if=/dev/urandom of=~/data-1ssd-pad.bin bs=1M count=${remainingMiB}`,
        );
      }
    });
    ...
    test("starts transition to failsafe mode with second device", async () => {
      const result =
        await umbreld.client.hardware.raid.transitionToFailsafeRaidz.mutate({
    ...
    test("waits for transition to complete (2 devices in array)", async () => {
      let status: Awaited<
        ReturnType<typeof umbreld.client.hardware.raid.getStatus.query>
      >;
      await pWaitFor(
        async () => {
          try {
            ...
            if (status.failsafeTransitionStatus?.state === "complete")
              return true;
            if (
              !status.failsafeTransitionStatus &&
    ...
    test("space with 2 SSDs is within 5% of initial 2-SSD setup", async () => {
      ...
      const totalDiscrepancy =
        Math.abs(status.totalSpace! - totalSpaceWith2Ssds) /
        totalSpaceWith2Ssds;
      console.log(
      ...
      const usableDiscrepancy =
        Math.abs(status.usableSpace! - usableSpaceWith2Ssds) /
        usableSpaceWith2Ssds;
      console.log(
    ...
    test("adds third SSD to RAID array", async () => {
      const expansionSubscription = umbreld.subscribeToEvents<ExpansionStatus>(
    ...
    test("space with 3 SSDs is within 5% of initial 3-SSD setup", async () => {
      ...
      const totalDiscrepancy =
        Math.abs(status.totalSpace! - totalSpaceWith3Ssds) /
        totalSpaceWith3Ssds;
      console.log(
      ...
      const usableDiscrepancy =
        Math.abs(status.usableSpace! - usableSpaceWith3Ssds) /
        usableSpaceWith3Ssds;
      console.log(
    ...
    test("space with 4 SSDs is within 5% of initial 4-SSD setup", async () => {
      ...
      const totalDiscrepancy =
        Math.abs(status.totalSpace! - totalSpaceWith4Ssds) /
        totalSpaceWith4Ssds;
      console.log(
      ...
      const usableDiscrepancy =
        Math.abs(status.usableSpace! - usableSpaceWith4Ssds) /
        usableSpaceWith4Ssds;
      console.log(
    ...
    test("replaces old 4th SSD with new one", async () => {
      ...
      console.log(
        `Replacing missing device ${missingDevice!.id} with new device ${fourthDeviceId}`,
      );
