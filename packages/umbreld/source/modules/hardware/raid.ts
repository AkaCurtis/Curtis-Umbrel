...
async function getDeviceSize(device: string): Promise<number> {
  const { stdout } =
    await $`lsblk --output SIZE --bytes --nodeps --noheadings ${device}`;
...
export function getRoundedDeviceSize(sizeInBytes: number): number {
  ...
  if (sizeInBytes >= twoFiftyGigabytes)
    return Math.floor(sizeInBytes / twentyFiveGigabytes) * twentyFiveGigabytes;
  return sizeInBytes;
}
...
export default class Raid {
  ...
  constructor(umbreld: Umbreld) {
    ...
    this.logger = umbreld.logger.createChildLogger(
    ...
    this.configStore = new FileStore<ConfigStore>({
      ...
      onAfterWrite: () =>
        pRetry(() => $`mount -o remount,ro ${configPartition}`, {
          ...
          this.logger.error(
            "Failed to remount config partition read-only",
            error,
          );
        }),
    });
  ...
  async start() {
    ...
    try {
      await fse.writeFile(
        "/sys/module/zfs/parameters/l2arc_exclude_special",
        "1",
      );
  ...
  async #updateConfigDevicePaths(): Promise<void> {
    ...
    const devices = status.devices!.map(
      (device) => `/dev/disk/by-umbrel-id/${device.id}`,
    );
    await this.configStore.set("raid.devices", devices);

    if (status.accelerator?.devices) {
      const acceleratorDevices = status.accelerator.devices.map(
        (device) => `/dev/disk/by-umbrel-id/${device.id}`,
      );
      await this.configStore.set(
        "raid.accelerator.devices",
        acceleratorDevices,
      );
    }
  }
      ...
      async () => {
        try {
          ...
          if (pool.expansion) {
            ...
            if (
              last?.state !== pool.expansion.state ||
              last?.progress !== pool.expansion.progress
            ) {
              this.#lastEmittedExpansion = pool.expansion;
              this.#umbreld.eventBus.emit(
          ...
          if (pool.rebuild) {
            ...
            if (
              last?.state !== pool.rebuild.state ||
              last?.progress !== pool.rebuild.progress
            ) {
  ...
  async getStatus() {
    ...
    return {
      ...
      status:
        this.failsafeTransitionStatus?.state === "rebuilding" &&
  ...
  async getPoolStatus(poolName: string): Promise<{
    ...
    if (pool.raidz_expand_stats) {
      ...
      if (state === "finished" || state === "canceled") {
        ...
        const rawProgress =
          stats.to_reflow > 0
            ? Math.floor((stats.reflowed / stats.to_reflow) * 100)
            : 0;
    ...
    if (pool.scan_stats?.function === "RESILVER") {
      ...
      if (state === "finished" || state === "canceled") {
        ...
        const rawProgress =
          stats.to_examine > 0
            ? Math.floor((stats.issued / stats.to_examine) * 100)
            : 0;
        const cappedProgress = Math.min(rawProgress, 99);
        // Never let progress go backwards
    ...
    const toDeviceId = (path: string) =>
      path.replace("/dev/disk/by-umbrel-id/", "").replace(/-part\d+$/, "");
    ...
    const acceleratorDevices = accelerator.devices.map(({ id, status }) => ({
      id,
      status,
    }));
    ...
    if (isMirror) {
      ...
      mirrors = mirrorVdevs
        .map((mirrorVdev) =>
          (membersByMirrorVdev.get(mirrorVdev.name) ?? []).sort(),
        )
    ...
    if (isRaidz && diskVdevs.length > 2) {
      ...
      const isReplacing = [...diskVdevs, ...fileVdevs].some((vdev) =>
        vdev.parent?.includes("replacing"),
      );
      ...
      const smallestDeviceSize = Math.min(
        ...devices.map((d) => d.size).filter((size) => size !== undefined),
      );
      ...
      totalSpace = [...diskVdevs, ...fileVdevs]
        .filter(
          (vdev) =>
            !(vdev.state === "CANT_OPEN" && vdev.parent?.includes("replacing")),
        )
        .reduce(
          (sum, vdev) => sum + (vdev.phys_space || vdev.rep_dev_size || 0),
          0,
        );
    ...
    if (isMirror)
      totalSpace = devices.reduce((sum, device) => sum + (device.size ?? 0), 0);
  ...
  async #getZpoolStatus(poolName: string): Promise<Pool | undefined> {
    try {
      const { stdout } =
        await $`zpool status --json --json-int --json-flat-vdevs ${poolName}`;
  ...
  #parsePoolAccelerator(pool: Pool): ParsedAccelerator {
    const toDeviceId = (path: string) =>
      path.replace("/dev/disk/by-umbrel-id/", "").replace(/-part\d+$/, "");
    const getVdevSize = (vdev: Vdev) =>
      vdev.phys_space || vdev.rep_dev_size || vdev.total_space || 0;
    const vdevs = Object.values(pool.vdevs);
    const cacheVdevs = vdevs.filter(
      (v) => v.vdev_type === "disk" && v.class === "l2cache" && v.path,
    );
    const specialVdevs = vdevs.filter(
      (v) => v.vdev_type === "disk" && v.class === "special" && v.path,
    );

    // Index special vdevs by device id for matching against cache vdevs
    const specialByDeviceId = new Map(
      specialVdevs.map((v) => [toDeviceId(v.path!), v]),
    );
    ...
    return {
      ...
      totalSpecialUsableSize:
        devices.length === 0
          ? 0
          : Math.min(...devices.map((d) => d.specialSize)),
    };
  }
  ...
  async handlePostBootRaidSetupProcess() {
    ...
    if (raidConfigUser?.name && raidConfigUser?.password && !userExists) {
      ...
      try {
        ...
        await this.#umbreld.user.register(
          raidConfigUser.name,
          raidConfigUser.password,
          raidConfigUser.language ?? "en",
        );
        ...
        await this.configStore
          ...
          .catch((error) =>
            this.logger.error(
              "Failed to delete password from RAID config",
              error,
            ),
          );
      } catch (error) {
  ...
  async checkRaidMountFailureDevices(): Promise<
    Array<{ name: string; isOk: boolean }>
  > {
    const { stdout } = await $`zpool import -N -d /dev/disk/by-umbrel-id`;
    const expectedDevices = ((await this.configStore.get("raid.devices")) ??
      []) as string[];
  ...
  async #partitionDevice(
    device: string,
  ): Promise<{ statePartition: string; dataPartition: string }> {
    ...
    const dataPartitionSizeBytes =
      roundedDeviceSize - statePartitionSizeBytes - bufferSizeBytes;
    ...
    this.logger.log(
      `Creating state partition (${statePartitionSizeMiB} MiB) on ${device}`,
    );
    await $`sgdisk --new=1:0:+${statePartitionSizeMiB}M --change-name=1:umbrel-raid-state ${device}`;

    this.logger.log(
      `Creating data partition (${dataPartitionSizeMiB} MiB) on ${device}`,
    );
    ...
    const partitionsExist = await Promise.all([
      ...
      fse.pathExists(dataPartition),
    ]);
    if (!partitionsExist[0])
      throw new Error(`State partition ${statePartition} does not exist`);
    if (!partitionsExist[1])
      throw new Error(`Data partition ${dataPartition} does not exist`);
  ...
  async #partitionAcceleratorDevice(
    ...
    const usableBytes =
      roundedDeviceSize - statePartitionSizeBytes - bufferSizeBytes;
    if (usableBytes <= 0)
      throw new Error("Accelerator device is too small to partition");
    ...
    const requestedL2arcSizeBytes =
      sizes?.l2arcSizeBytes ??
      Math.floor(Math.min(roundedDeviceSize * 0.5, os.totalmem() * 5));
    const l2arcSizeBytes = requestedL2arcSizeBytes;
    if (l2arcSizeBytes <= 0)
      throw new Error("Accelerator device is too small for an L2ARC partition");
    ...
    const specialSizeBytes =
      sizes?.specialSizeBytes ?? usableBytes - l2arcSizeBytes;
    if (specialSizeBytes <= 0)
      throw new Error(
        "Accelerator device is too small for a special partition",
      );
    if (l2arcSizeBytes + specialSizeBytes > usableBytes)
      throw new Error(
        "Accelerator device is too small for the requested partition sizes",
      );
    ...
    this.logger.log(
      `Waiting for accelerator partitions to appear on ${device}`,
    );
    await $`udevadm settle`;
    ...
    if (!partitionsExist[0])
      throw new Error(`State partition ${statePartition} does not exist`);
    if (!partitionsExist[1])
      throw new Error(`L2ARC partition ${l2arcPartition} does not exist`);
    if (!partitionsExist[2])
      throw new Error(`Special partition ${specialPartition} does not exist`);
  ...
  async #createPool(
    ...
    this.logger.log(
      `Creating ZFS pool '${poolName}' (${topology}) with partitions: ${dataPartitions.join(", ")}`,
    );
  ...
  async setup(
    ...
    if (!(await this.#umbreld.hardware.umbrelPro.isUmbrelPro()))
      throw new Error(
        "RAID is currently only supported on Umbrel Pro hardware",
      );
    if (deviceIds.length === 0)
      throw new Error("At least one device is required");
    if (raidType === "failsafe" && deviceIds.length < 2)
      throw new Error("Failsafe mode requires at least two devices");

    const devices = deviceIds.map((id) => `/dev/disk/by-umbrel-id/${id}`);
    for (const device of devices) {
      if (!(await fse.pathExists(device)))
        throw new Error(`Device not found: ${device}`);
    }
    this.logger.log(
      `Setting up RAID with ${devices.length} device(s): ${devices.join(", ")}`,
    );

    // Resolve and validate selected device type(s)
    const internalDevices =
      await this.#umbreld.hardware.internalStorage.getDevices();
    const selectedDeviceTypes = deviceIds.map(
      (id) => internalDevices.find((device) => device.id === id)?.type,
    );
    ...
    if (uniqueDeviceTypes.length > 1)
      throw new Error("Cannot mix SSDs and HDDs in the same RAID array");
    ...
    if (acceleratorDeviceIds?.length) {
      ...
      for (const acceleratorDeviceId of acceleratorDeviceIds) {
        ...
        if (!(await fse.pathExists(acceleratorDevice)))
          throw new Error(`Device not found: ${acceleratorDevice}`);
        if (raidDeviceIds.has(acceleratorDeviceId))
          throw new Error("Cannot add a RAID data device as an accelerator");
    ...
    const partitionResults = await Promise.all(
      devices.map((device) => this.#partitionDevice(device)),
    );
    const dataPartitions = partitionResults.map(
      (result) => result.dataPartition,
    );
    ...
    if (acceleratorDeviceIds?.length)
      await this.addAccelerator(acceleratorDeviceIds);
  ...
  async #assertDeviceTypeMatchesPool(deviceId: string): Promise<void> {
    ...
    if (!poolDeviceId)
      throw new Error("RAID array doesn't exist or has no devices");
    ...
    if (newDevice.type !== poolDevice.type)
      throw new Error(`Cannot mix SSDs and HDDs in the same RAID array`);
  }
  ...
  async #assertAcceleratorDeviceType(deviceId: string): Promise<void> {
    ...
    if (device.type !== "ssd")
      throw new Error("Accelerator devices must be SSDs");
  }
  ...
  async addDevice(deviceId: string): Promise<boolean> {
    ...
    if (pool.topology !== "stripe" && pool.topology !== "raidz")
      throw new Error(
        `Unsupported RAID topology for addDevice: ${pool.topology}`,
      );
    ...
    if (!(await fse.pathExists(device)))
      throw new Error(`Device not found: ${device}`);
    if (poolDeviceIds.includes(deviceId))
      throw new Error("Cannot add a device that is already in the RAID array");
    ...
    if (pool.topology === "raidz") {
      ...
      this.logger.log(
        `Attaching ${dataPartition} to raidz1-0 in pool '${pool.name}'`,
      );
      await $`zpool attach -f ${pool.name} raidz1-0 ${dataPartition}`;
    } else {
      // Storage mode: add as new top-level vdev (stripe)
      this.logger.log(
        `Adding ${dataPartition} as stripe to pool '${pool.name}'`,
      );
    ...
    const updatedDevices = [
      ...poolDeviceIds.map((id) => `/dev/disk/by-umbrel-id/${id}`),
      device,
    ];
    this.logger.log(
      `Updating RAID config with ${updatedDevices.length} device(s)`,
    );
  ...
  async addMirror(deviceIds: [string, string]): Promise<boolean> {
    ...
    if (deviceIds[0] === deviceIds[1])
      throw new Error("Mirror pair requires two different devices");
    ...
    const [leftPartition, rightPartition] = partitionResults.map(
      (result) => result.dataPartition,
    );

    // Add the mirror vdev
    this.logger.log(
      `Adding mirror pair (${leftPartition}, ${rightPartition}) to pool '${pool.name}'`,
    );
  ...
  async addAccelerator(deviceIds: string[]): Promise<boolean> {
    ...
    if ((await this.#getPoolDeviceType()) !== "hdd")
      throw new Error("Accelerators are only supported for HDD RAID arrays");
    if (pool.accelerator?.exists)
      throw new Error("RAID array already has an accelerator");
    ...
    const acceleratorDevices = deviceIds.map(
      (id) => `/dev/disk/by-umbrel-id/${id}`,
    );
    ...
    for (const deviceId of deviceIds) {
      ...
      if (!(await fse.pathExists(device)))
        throw new Error(`Device not found: ${device}`);
      if (poolDeviceIds.has(deviceId))
        throw new Error("Cannot add a RAID data device as an accelerator");
      await this.#assertAcceleratorDeviceType(deviceId);
    }

    this.logger.log(
      `Adding accelerator to RAID array: ${acceleratorDevices.join(", ")}`,
    );

    const partitionResults = await Promise.all(
      acceleratorDevices.map((device) =>
        this.#partitionAcceleratorDevice(device),
      ),
    );
    const l2arcPartitions = partitionResults.map(
      (result) => result.l2arcPartition,
    );
    const specialPartitions = partitionResults.map(
      (result) => result.specialPartition,
    );
  ...
  async #monitorReplaceProgress(
    poolName: string,
    completionCheck: (
      status: Awaited<ReturnType<Raid["getPoolStatus"]>>,
    ) => boolean,
          ...
          try {
            ...
            if (status.rebuild) {
              const cappedProgress =
                status.rebuild.state === "finished"
                  ? 100
                  : Math.min(status.rebuild.progress, 99);
              ...
              this.logger.log(
                "No rebuild status but replacement target is online, considering complete",
              );
              break;
            }
          } catch (error) {
  ...
  async #replaceStorageDevice(
    ...
    const { dataPartition: newDataPartition } =
      await this.#partitionDevice(newDevice);
    const oldDataPartition = `${oldDevice}-part2`;

    this.logger.log(
      `Replacing ${oldDataPartition} with ${newDataPartition} in pool '${pool.name}'`,
    );
    ...
    const updatedDevices = currentDevices.map((device: string) =>
      device === oldDevice ? newDevice : device,
    );
    this.logger.log(
      `Updating RAID config with devices: ${updatedDevices.join(", ")}`,
    );
    await this.configStore.set("raid.devices", updatedDevices);
    ...
    await this.#monitorReplaceProgress(
      ...
      (status) =>
        Boolean(
          status.devices?.some(
            (d) => d.id === newDeviceId && d.status === "ONLINE",
          ),
        ),
      async () => {
  ...
  async #replaceAcceleratorDevice(
    ...
    const oldDevice = `/dev/disk/by-umbrel-id/${oldDeviceId}`;
    const newDevice = `/dev/disk/by-umbrel-id/${newDeviceId}`;
    const acceleratorDeviceIds =
      pool.accelerator!.devices?.map((device) => device.id) ?? [];

    await this.#assertAcceleratorDeviceType(newDeviceId);

    this.logger.log(
      `Replacing accelerator device ${oldDevice} with ${newDevice}`,
    );
    ...
    const acceleratorToReplace = acceleratorDevices.find(
      (device) => device.id === oldDeviceId,
    );
    if (!acceleratorToReplace)
      throw new Error(`Device ${oldDeviceId} is not in the accelerator`);
    const referenceAccelerator =
      pool.raidType === "failsafe"
        ? (acceleratorDevices.find((device) => device.id !== oldDeviceId) ??
          acceleratorToReplace)
        : acceleratorToReplace;

    const { l2arcPartition, specialPartition } =
      await this.#partitionAcceleratorDevice(newDevice, {
    ...
    this.logger.log(
      `Adding accelerator cache partition ${l2arcPartition} to pool '${pool.name}'`,
    );
    await $`zpool add -f ${pool.name} cache ${l2arcPartition}`;

    const currentAcceleratorDevices =
      (await this.configStore.get("raid.accelerator.devices")) ?? [];
    const updatedAcceleratorDevices = currentAcceleratorDevices.map(
      (device: string) => (device === oldDevice ? newDevice : device),
    );
    this.logger.log(
      `Updating RAID config with accelerator devices: ${updatedAcceleratorDevices.join(", ")}`,
    );
    await this.configStore.set(
      "raid.accelerator.devices",
      updatedAcceleratorDevices,
    );
    ...
    await this.#monitorReplaceProgress(pool.name, (status) => {
      const currentAcceleratorDeviceIds =
        status.accelerator?.devices?.map((device) => device.id).sort() ?? [];
      return (
        currentAcceleratorDeviceIds.join(",") ===
        expectedAcceleratorDeviceIds.sort().join(",")
      );
    });

    this.logger.log(
      "Accelerator replacement initiated, resilvering in progress",
    );
    return true;
  }
  ...
  async replaceDevice(
    ...
    const newDevice = `/dev/disk/by-umbrel-id/${newDeviceId}`;
    if (!(await fse.pathExists(newDevice)))
      throw new Error(`New device not found: ${newDevice}`);
    ...
    if (poolDeviceIds.includes(newDeviceId))
      throw new Error(
        "Cannot replace with a device that is already in the RAID array",
      );
    if (acceleratorDeviceIds.includes(newDeviceId))
      throw new Error(
        "Cannot replace with a device that is already in the accelerator",
      );
    ...
    try {
      if (poolDeviceIds.includes(oldDeviceId))
        return await this.#replaceStorageDevice(pool, oldDeviceId, newDeviceId);
      if (acceleratorDeviceIds.includes(oldDeviceId))
        return await this.#replaceAcceleratorDevice(
          pool,
          oldDeviceId,
          newDeviceId,
        );
      throw new Error(
        `Device ${oldDeviceId} is not in the RAID array or accelerator`,
      );
    } catch (error) {
      this.isReplacing = false;
      throw error;
  ...
  async transitionToFailsafeRaidz(newDeviceId: string): Promise<boolean> {
    ...
    if (pool.raidType !== "storage")
      throw new Error("Can only transition from storage mode");

    // Raidz transition only supports SSD arrays with a single existing device
    const deviceType = await this.#getPoolDeviceType();
    if (deviceType !== "ssd")
      throw new Error(
        "transitionToFailsafeRaidz is only supported for SSD arrays",
      );
    if (pool.devices?.length !== 1)
      throw new Error("Can only transition single-disk SSD arrays");

    // Validate new device exists, isn't in the pool, and matches pool type
    const newDevice = `/dev/disk/by-umbrel-id/${newDeviceId}`;
    if (!(await fse.pathExists(newDevice)))
      throw new Error(`Device not found: ${newDevice}`);
    const poolDeviceIds = pool.devices?.map((d) => d.id) ?? [];
    if (poolDeviceIds.includes(newDeviceId))
      throw new Error(
    ...
    if (
      getRoundedDeviceSize(newDeviceSize) <
      getRoundedDeviceSize(currentDeviceSize)
    )
      throw new Error(
        "Cannot transition to a device smaller than the current device",
      );

    if (this.isTransitioningToFailsafe)
      throw new Error("Already transitioning to failsafe mode");
    ...
    try {
      ...
      const currentDeviceDataPartitionSize = await getDeviceSize(
        currentDeviceDataPartition,
      );
      ...
      await this.#createPool(
        migrationPoolName,
        [newDataPartition, this.temporaryDevicePath],
        "raidz",
      );

      // Remove the temp file from the migration pool (making it degraded)
      this.logger.log(
        `Removing temp device from pool to create degraded raidz1`,
      );
      ...
      Promise.resolve()
        .then(async () => {
          ...
          // Using --raw to preserve encryption (sends encrypted blocks without needing key loaded)
          this.logger.log(
            `Sending snapshot to migration pool (this may take a while)...`,
          );
          const sendProcess = $({
            shell: true,
            ...
            async () => {
              try {
                const migrationStatus =
                  await this.getPoolStatus(migrationPoolName);
                if (migrationStatus.exists && estimatedSize > 0) {
                  ...
                  const rawProgress = Math.min(
                    99,
                    Math.floor((usedSpace / estimatedSize) * 100),
                  );
                  const progress = Math.floor((rawProgress / 100) * 49);
                  if (
                    this.failsafeTransitionStatus &&
                    progress > this.failsafeTransitionStatus.progress
                  ) {
          ...
          this.logger.log(
            `Initial sync complete, rebooting to complete migration`,
          );
  ...
  async transitionToFailsafeMirror(
    ...
    if (pairs.length === 0)
      throw new Error("At least one mirror pair is required");
    ...
    if (deviceType !== "hdd")
      throw new Error(
        "transitionToFailsafeMirror is only supported for HDD arrays",
      );
    ...
    for (const pair of pairs) {
      if (!existingPoolDeviceIds.has(pair.existingDeviceId))
        throw new Error(
          `Device ${pair.existingDeviceId} is not in the RAID array`,
        );
      if (seenExisting.has(pair.existingDeviceId))
        throw new Error(
          `Duplicate existing device in mirror pairs: ${pair.existingDeviceId}`,
        );
      if (seenNew.has(pair.newDeviceId))
        throw new Error(
          `Duplicate new device in mirror pairs: ${pair.newDeviceId}`,
        );
    ...
    for (const existingDeviceId of existingDeviceIds) {
      if (!seenExisting.has(existingDeviceId))
        throw new Error(
          `Missing mirror pair for existing device: ${existingDeviceId}`,
        );
    }
    ...
    for (const pair of pairs) {
      ...
      if (getRoundedDeviceSize(newSize) < getRoundedDeviceSize(existingSize))
        throw new Error(
          "Cannot transition with a device smaller than an existing device",
        );
    }
    ...
    const existingAcceleratorDeviceIds =
      existingAccelerator?.devices?.map((device) => device.id) ?? [];
    const existingAcceleratorDevices = existingAcceleratorDeviceIds.map(
      (id) => `/dev/disk/by-umbrel-id/${id}`,
    );
    ...
    if (existingAcceleratorDeviceIds.length > 0) {
      ...
      const existingAcceleratorSize = await getDeviceSize(
        existingAcceleratorDevicePath,
      );
      const newAcceleratorSize = await getDeviceSize(acceleratorDevice);
      if (
        getRoundedDeviceSize(newAcceleratorSize) <
        getRoundedDeviceSize(existingAcceleratorSize)
      )
        throw new Error(
          "Cannot transition with an accelerator device smaller than the existing accelerator",
        );
    } else if (acceleratorDeviceId) {
      throw new Error(
        "Cannot supply an accelerator mirror SSD when no accelerator exists",
      );
    }
    ...
    const newDevices = pairs.map(
      (pair) => `/dev/disk/by-umbrel-id/${pair.newDeviceId}`,
    );
    this.logger.log(
      `Starting mirror failsafe transition with ${newDevices.join(", ")}`,
    );
    ...
    try {
      ...
      for (const pair of pairs) {
        ...
        if (!newDataPartition)
          throw new Error(`Missing partition for device: ${pair.newDeviceId}`);

        this.logger.log(
          `Attaching ${newDataPartition} to ${existingPartition} in pool '${pool.name}'`,
        );
      ...
      if (existingAcceleratorDeviceIds.length > 0 && acceleratorDeviceId) {
        ...
        const { l2arcPartition, specialPartition } =
          await this.#partitionAcceleratorDevice(newAcceleratorDevice, {
        ...
        this.logger.log(
          `Mirroring accelerator special vdev ${existingSpecialPartition} with ${specialPartition}`,
        );
      ...
      const newDeviceByExisting = new Map(
        pairs.map((pair) => [pair.existingDeviceId, pair.newDeviceId]),
      );
        ...
        await set("raid", {
          ...
          accelerator:
            existingAcceleratorDeviceIds.length > 0 && acceleratorDeviceId
              ? {
                  devices: [
                    ...existingAcceleratorDevices,
                    `/dev/disk/by-umbrel-id/${acceleratorDeviceId}`,
                  ],
                }
              : raid?.accelerator,
        });
          ...
          while (true) {
            try {
              ...
              if (status.rebuild) {
                ...
                if (
                  cappedProgress >
                  (this.failsafeTransitionStatus?.progress ?? 0)
                ) {
                  this.failsafeTransitionStatus = {
                    state: "rebuilding",
                    progress: cappedProgress,
                  ...
                  this.logger.log(
                    `Mirror rebuild progress: ${cappedProgress}%`,
                  );
                  this.#umbreld.eventBus.emit(
                    "raid:failsafe-transition-progress",
                    this.failsafeTransitionStatus,
                ...
                const allOnline = status.devices?.every(
                  (d) => d.status === "ONLINE",
                );
                if (
                  allOnline &&
                  (status.devices?.length ?? 0) > existingDeviceIds.length
                )
                  break;
              }
            } catch (error) {
  ...
  async #completeFailsafeTransition(): Promise<void> {
    ...
    if (!previousPool.exists) {
      this.logger.error(
        "Config indicates transition in progress but previous pool not found",
      );
      return;
    }
    ...
    try {
      ...
      if (!oldDevice)
        throw new Error(
          "Could not determine old device from previous migration pool",
        );
      ...
      const { dataPartition: oldDataPartition } =
        await this.#partitionDevice(oldDevicePath);
      ...
      await this.configStore.getWriteLock(async ({ set }) => {
        const pool = await this.getStatus();
        const devices = pool.devices!.map(
          (device) => `/dev/disk/by-umbrel-id/${device.id}`,
        );
        const raid = await this.configStore.get("raid");
        await set("raid", {
        ...
        try {
          ...
          if (status.rebuild) {
            ...
            const scaledProgress =
              51 + Math.floor((status.rebuild.progress / 100) * 48);
            const cappedProgress = Math.min(scaledProgress, 99);
            if (
              cappedProgress > (this.failsafeTransitionStatus?.progress ?? 0)
            ) {
              this.failsafeTransitionStatus = {
                state: "rebuilding",
                progress: cappedProgress,
