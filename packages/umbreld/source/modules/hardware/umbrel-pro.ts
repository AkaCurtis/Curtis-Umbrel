...
export default class UmbrelPro {
  ...
  async start() {
    ...
    await this.setLedDefault().catch((error) =>
      this.logger.error("Failed to set LED to default state", error),
    );

    // Clear min fan speed
    this.logger.log("Clearing min fan speed");
    await this.setMinFanSpeed(0).catch((error) =>
      this.logger.error("Failed to clear min fan speed", error),
    );
  ...
  async #writeEcRegister(register: number, value: number): Promise<void> {
    if (!(await this.isUmbrelPro()))
      throw new Error(
        "Refusing to write EC register on non Umbrel Pro hardware",
      );
    return this.#ecRegisterCommandQueue.add(async () =>
      writeEcRegister(register, value),
    );
  }
  ...
  async #readEcRegister(register: number): Promise<number> {
    if (!(await this.isUmbrelPro()))
      throw new Error(
        "Refusing to read EC register on non Umbrel Pro hardware",
      );
    return this.#ecRegisterCommandQueue.add(async () =>
      readEcRegister(register),
    ) as Promise<number>;
  }
  ...
  async #manageFanSpeed(): Promise<void> {
    ...
    try {
      ...
      const deviceFanSpeeds = devices.map((device) => {
        ...
        const warningTemp =
          device.temperatureWarning ?? FAN_DEFAULT_WARNING_TEMP;
        ...
        return {
          device,
          fanSpeed: Math.round((tempAboveMin / tempRange) * 100),
        };
      });

      // Find the device requiring the highest fan speed
      const highest = deviceFanSpeeds.reduce((max, current) =>
      ...
      const shouldDecrease =
        highest.fanSpeed <= lastFanSpeed - 5 ||
        (highest.fanSpeed === 0 && lastFanSpeed !== 0);
  ...
  setFanManagementEnabled(enabled: boolean) {
    if (enabled) {
      if (!this.#stopManagingFan) {
        ...
        this.#stopManagingFan = runEvery("1 minute", () =>
          this.#manageFanSpeed(),
        );
      }
    } else {
  ...
  async setLedStatic(): Promise<void> {
    ...
    await this.#writeEcRegister(
      this.EC_LED_STATE_ADDRESS,
      LED_STATE_STATIC_VALUE,
    );
  }
  ...
  async setLedBreathe(duration: number = 14): Promise<void> {
    ...
    await this.#writeEcRegister(
      EC_LED_BREATHING_DURATION_ADDRESS,
      clampedDuration,
    );
    await this.#writeEcRegister(
      this.EC_LED_STATE_ADDRESS,
      LED_STATE_BREATHING_VALUE,
    );
  }
  ...
  getSsdSlotFromPciSlotNumber(
    pciSlotNumber: number | undefined,
  ): number | undefined {
