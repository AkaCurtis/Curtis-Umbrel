...
describe("RAID failsafe mode", () => {
  ...
  let expansionSubscription: ReturnType<
    typeof umbreld.subscribeToEvents<ExpansionStatus>
  >;
  let failed = false;
  ...
  test("rejects addMirror endpoint on raidz failsafe arrays", async () => {
    await expect(
      umbreld.client.hardware.raid.addMirror.mutate({
        deviceIds: [firstDeviceId, secondDeviceId],
      }),
  ...
  test("adds third SSD to RAID array and subscribes to expansion events", async () => {
    ...
    expansionSubscription = umbreld.subscribeToEvents<ExpansionStatus>(
  ...
  test("has all three devices in the array", async () => {
    ...
    expect(deviceIds).toEqual(
      [firstDeviceId, secondDeviceId, thirdDeviceId].sort(),
    );
  });
