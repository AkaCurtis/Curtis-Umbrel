...
export default function createTerminalWebSocketHandler({
  ...
  return async function (ws: WebSocket, request: http.IncomingMessage) {
    try {
      ...
      const cols = Number(
        new URL(`https://localhost/${request.url}`).searchParams.get("cols"),
      );
      const rows = Number(
        new URL(`https://localhost/${request.url}`).searchParams.get("rows"),
      );
      ...
      if (appId) {
        ...
        if (!container) {
          container = container =
            compose.services![DEFAULT_SHELL_CONTAINERS[app.id]]?.container_name;
        }

        // If we still don't have a default container use the first container as a fallback
        if (!container) {
          container = Object.values(compose.services!).filter(
            (service) => service.image && service.container_name,
          )[0]?.container_name as string;
        }
