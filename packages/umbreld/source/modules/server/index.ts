...
class Server {
  ...
  mountWebSocketServer(
    path: string,
    setupHandler: (wss: WebSocketServer) => void,
  ) {
  ...
  async start() {
    ...
    this.app.use(
      helmet.contentSecurityPolicy({
        directives: {
          ...
          scriptSrc: this.umbreld.developmentMode
            ? ["'self'", "'unsafe-inline'"]
            : null,
    ...
    this.app.get(
      "/manager-api/v1/system/update-status",
      (request, response) => {
        response.json({
          state: "success",
          progress: 100,
    ...
    this.mountWebSocketServer("/terminal", (wss) => {
      ...
      wss.on(
        "connection",
        createTerminalWebSocketHandler({ umbreld: this.umbreld, logger }),
      );
    });

    // Handle API routes
    const createApi = (
      registerApi: ({ publicApi, privateApi, umbreld }: ApiOptions) => void,
    ) => {
    ...
    this.app.get("/logs/", async (request, response) => {
      ...
      try {
        ...
        response.set(
          "Content-Disposition",
          `attachment;filename=umbrel-${Date.now()}.log.gz`,
        );
    ...
    if (process.env.UMBREL_UI_PROXY) {
      ...
      const cacheAggressively: express.RequestHandler = (_, response, next) => {
        ...
        response.set(
          "Cache-Control",
          `public, max-age=${approximatelyOneYearInSeconds}, immutable`,
        );
        next();
      };
    ...
    // Start the server
    const listen = promisify(this.server.listen.bind(this.server)) as (
      port: number,
    ) => Promise<void>;
