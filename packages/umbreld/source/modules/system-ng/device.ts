...
export default class Device {
  ...
  async #getMemory(): Promise<{
    ...
    try {
      ...
      const types = [...stdout.matchAll(/^\s*Type:\s*(.+)$/gm)].map((m) =>
        m[1].trim(),
      );
  ...
  async #getStorage(): Promise<{
    ...
    try {
      ...
      if (storageSize === null) {
        const df =
          await $`df --output=size --block-size=1 ${this.#umbreld.dataDirectory}`;
    ...
    try {
      ...
      const dfSource =
        await $`df ${this.#umbreld.dataDirectory} --output=source`;
      ...
      const matchedDisk = blockdevices.find(
        (d) => d.type === "disk" && d.name === dataDirDisk,
      );
