...
const device = router({
  ...
  getIdentity: publicProcedureWhenNoUserExists.query(({ ctx }) =>
    ctx.umbreld.systemNg.device.getIdentity(),
  ),
  // Returns device identity and hardware specs (CPU, memory, storage)
  getSpecs: privateProcedure.query(({ ctx }) =>
    ctx.umbreld.systemNg.device.getSpecs(),
  ),
});
