  ...
  function subscribeToEvents<T>(event: (typeof events)[number]) {
    ...
    const subscription = client.eventBus.listen.subscribe(
      { event },
      {
        ...
        onError: (error) =>
          console.error(`Subscription error for ${event}:`, error),
      },
    );
    return {
      collected,
...
export async function createTestVm({
  ...
  async function powerOn() {
    ...
    vmProcess.stdout?.on(
      "data",
      (data: Buffer) => (vmOutput += data.toString()),
    );
    vmProcess.stderr?.on(
      "data",
      (data: Buffer) => (vmOutput += data.toString()),
  ...
  async function powerOff() {
    ...
    try {
      await client.system.shutdown
        .mutate()
        .catch(() => unauthenticatedClient.system.shutdown.mutate());
  ...
  async function addSata({
    ...
    if (size) {
      await $({
        env,
      })`${vmScript} sata add ${slot} --size ${size} --type ${type}`;
    } else {
