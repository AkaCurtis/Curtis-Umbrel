  ...
  enable: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (appId === "umbrel") {
        ...
        if (!(widgetName in umbrelWidgets))
          throw new Error(
            `No widget named ${widgetName} found in Umbrel widgets`,
          );
      } else {
      ...
      await ctx.umbreld.store.getWriteLock(async ({ get, set }) => {
        ...
        if (widgets.includes(input.widgetId))
          throw new Error(`Widget ${input.widgetId} is already enabled`);

        // Check we don't have more than 3 widgets enabled
        if (widgets.length >= MAX_ALLOWED_WIDGETS)
          throw new Error(
            `The maximum number of widgets (${MAX_ALLOWED_WIDGETS}) has already been enabled`,
          );
  ...
  data: privateProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (appId === "umbrel") {
        ...
        widgetData = await umbrelWidgets[
          widgetName as keyof typeof umbrelWidgets
        ](ctx.umbreld);
      } else {
        // This is an app widget
