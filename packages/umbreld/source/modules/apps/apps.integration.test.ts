...
test.sequential(
  "list() throws invalid error when no user is registered",
  async () => {
    await expect(umbreld.client.apps.list.query()).rejects.toThrow(
      "Invalid token",
    );
  },
);
...
test.sequential(
  "install() throws invalid error when no user is registered",
  async () => {
    await expect(
...
test.sequential(
  "state() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.apps.state.query({ appId: "sparkles-hello-world" }),
    ).rejects.toThrow("Invalid token");
  },
);

test.sequential(
  "restart() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.apps.restart.mutate({ appId: "sparkles-hello-world" }),
    ).rejects.toThrow("Invalid token");
  },
);

test.sequential(
  "update() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.apps.update.mutate({ appId: "sparkles-hello-world" }),
    ).rejects.toThrow("Invalid token");
  },
);

test.sequential(
  "trackOpen() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.apps.trackOpen.mutate({ appId: "sparkles-hello-world" }),
    ).rejects.toThrow("Invalid token");
  },
);
...
test.sequential(
  ...
  async () => {
    await expect(
      umbreld.client.apps.setTorEnabled.mutate(true),
    ).rejects.toThrow("Invalid token");
  },
);

test.sequential(
  "getBackupIgnoredPaths() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.apps.getBackupIgnoredPaths.query({
...
test.sequential("install() throws error on unknown app id", async () => {
  await expect(
    umbreld.client.apps.install.mutate({ appId: "unknown-app-id" }),
  ).rejects.toThrow("not found");
});

test.sequential("install() throws error on invalid app id", async () => {
  await expect(
    umbreld.client.apps.install.mutate({ appId: "invalid-id-@/!" }),
...
test.sequential(
  "getBackupIgnoredPaths() throws error on unknown app id",
  async () => {
    await expect(
      umbreld.client.apps.getBackupIgnoredPaths.query({
        appId: "sparkles-hello-world",
...
test.sequential("state() becomes ready once install completes", async () => {
  let lastState: any;
  do {
    lastState = await umbreld.client.apps.state.query({
      appId: "sparkles-hello-world",
    });
...
test.sequential(
  "getBackupIgnoredPaths() returns sanitised absolute paths for installed app",
  async () => {
    const dataDir = umbreld.instance.dataDirectory;
    const expected = ["data", "logs", "cache"].map((p) =>
      path.join(dataDir, "app-data", "sparkles-hello-world", p),
    );
    await expect(
      umbreld.client.apps.getBackupIgnoredPaths.query({
        appId: "sparkles-hello-world",
...
test.sequential("getBackupIgnoredPaths() supports '*' globs", async () => {
  ...
  const result = await umbreld.client.apps.getBackupIgnoredPaths.query({
    appId: "sparkles-hello-world",
  });
...
test.sequential(
  "getBackupIgnoredPaths() ignores unsupported globbing characters",
  async () => {
...
test.sequential(
  "getBackupIgnoredPaths() returns empty array when app has no backupIgnore paths",
  async () => {
...
test.sequential(
  "auto-reinstalls app when data directory is missing on first boot after restore",
  async () => {
    // Ensure the app is currently installed (from previous sequential test)
    const preApps = await umbreld.client.apps.list.query();
    expect(preApps.some((a: any) => a.id === "sparkles-hello-world")).toBe(
      true,
    );
    ...
    const restoreFlagPath = path.join(
      umbreld.instance.dataDirectory,
      BACKUP_RESTORE_FIRST_START_FLAG,
    );
    ...
    let ready = false;
    for (let i = 0; i < 60; i++) {
      const state: any = await umbreld.client.apps.state
        .query({ appId: "sparkles-hello-world" })
        .catch(() => null);
...
test.sequential(
  "does not missing data-dir app on non-restore boot",
  async () => {
...
test.sequential("restart() restarts an installed app", async () => {
  ...
  await umbreld.client.apps.install
    .mutate({ appId: "sparkles-hello-world" })
    .catch(() => {});
  await expect(
    umbreld.client.apps.restart.mutate({ appId: "sparkles-hello-world" }),
  ).resolves.toStrictEqual(true);
...
test.sequential("umbreld restart doesn't start stopped apps", async () => {
  // Stop the app
  await expect(
    umbreld.client.apps.stop.mutate({ appId: "sparkles-hello-world" }),
  ).resolves.toStrictEqual(true);
...
test.sequential("umbreld restart starts all non-stopped apps", async () => {
  ...
  await expect(
    umbreld.client.apps.start.mutate({ appId: "sparkles-hello-world" }),
  ).resolves.toStrictEqual(true);
...
test.sequential("setTorEnabled() toggles the Tor setting", async () => {
  ...
  await expect(
    umbreld.client.apps.getTorEnabled.query(),
  ).resolves.toStrictEqual(true);
  await expect(
    umbreld.client.apps.setTorEnabled.mutate(false),
  ).resolves.toStrictEqual(true);
  await expect(
    umbreld.client.apps.getTorEnabled.query(),
...
test.sequential("uninstall() uninstalls an app", async () => {
  await expect(
    umbreld.client.apps.uninstall.mutate({ appId: "sparkles-hello-world" }),
  ).resolves.toStrictEqual(true);
  const installedApps = await umbreld.client.apps.list.query();
});
