...
test.sequential(
  "registry() throws invalid error when no user is registered",
  async () => {
    await expect(umbreld.client.appStore.registry.query()).rejects.toThrow(
      "Invalid token",
    );
  },
);
...
test.sequential(
  "addRepository() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.appStore.addRepository.mutate({
...
test.sequential(
  "removeRepository() throws invalid error when no user is registered",
  async () => {
    await expect(
      umbreld.client.appStore.removeRepository.mutate({
        url: communityAppStoreGitServer.url,
      }),
    ).rejects.toThrow("Invalid token");
  },
        ...
        apps: [
          {
            ...
            support:
              "https://github.com/getumbrel/umbrel-hello-world-app/issues",
            gallery: [
...
test.sequential(
  "registry() returns both app repositories in registry",
  async () => {
    await expect(
...
test.sequential(
  "addRepository() throws adding a repository that has already been added",
  async () => {
    await expect(
      umbreld.client.appStore.addRepository.mutate({
        url: communityAppStoreGitServer.url,
...
test.sequential(
  "registry() no longer returns an app repository that has been removed",
  async () => {
    await expect(
      umbreld.client.appStore.registry.query(),
    ).resolves.toStrictEqual([
...
test.sequential(
  "removeRepository() throws removing a reposoitory that does not exist",
  async () => {
    await expect(
      umbreld.client.appStore.removeRepository.mutate({
        url: communityAppStoreGitServer.url,
...
test.sequential(
  "removeRepository() throws removing the default reposoitory",
  async () => {
    await expect(
      umbreld.client.appStore.removeRepository.mutate({
        url: umbreld.instance.appStore.defaultAppStoreRepo,
