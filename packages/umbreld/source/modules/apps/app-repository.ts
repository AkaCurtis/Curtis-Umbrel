...
export default class AppRepository {
  ...
  async checkLatestCommit() {
    ...
    const latestCommitInDefaultRemoteBranch = remoteRefs.find(
      (ref) => ref.ref === "HEAD",
    )!.oid;
  ...
  async readRegistry() {
    ...
    if (this.url === UMBREL_APP_STORE_REPO) {
      ...
      meta = (await readYaml(
        `${this.path}/umbrel-app-store.yml`,
      )) as AppRepositoryMeta;
    }
    ...
    const apps = manifests
      ...
      .filter(
        (app) => meta.id === "umbrel-app-store" || app.id.startsWith(meta.id),
      )
      ...
      .map((app) => ({
        ...
        gallery:
          meta.id === "umbrel-app-store"
            ? app.gallery.map(
                (file) =>
                  `https://getumbrel.github.io/umbrel-apps-gallery/${app.id}/${file}`,
              )
            : app.gallery,
