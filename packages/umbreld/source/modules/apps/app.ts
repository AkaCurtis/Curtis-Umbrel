...
export default class App {
  ...
  constructor(umbreld: Umbreld, appId: string) {
    ...
    this.store = new FileStore({
      filePath: `${this.dataDirectory}/settings.yml`,
    });
  }
  ...
  readCompose() {
    return readYaml(
      `${this.dataDirectory}/docker-compose.yml`,
    ) as Promise<Compose>;
  }
  ...
  async readHiddenService() {
    try {
      return await fse.readFile(
        `${this.#umbreld.dataDirectory}/tor/data/app-${this.id}/hostname`,
        "utf-8",
      );
    } catch (error) {
      this.logger.error(
        `Failed to read hidden service for app ${this.id}`,
        error,
      );
      return "";
    }
  ...
  async deriveDeterministicPassword() {
    const umbrelSeed = await fse.readFile(
      `${this.#umbreld.dataDirectory}/db/umbrel-seed/seed`,
    );
    const identifier = `app-${this.id}-seed-APP_PASSWORD`;
    const deterministicPassword = crypto
      .createHmac("sha256", umbrelSeed)
      .update(identifier)
      .digest("hex");
  ...
  async patchComposeFile() {
    ...
    for (const serviceName of Object.keys(compose.services!)) {
      ...
      compose.services![serviceName].volumes = compose.services![
        serviceName
      ].volumes?.map((volume) => {
      ...
      if (shouldEnableGpuPassthrough) {
        compose.services![serviceName].devices =
          compose.services![serviceName].devices || [];
  ...
  async uninstall() {
    ...
    await this.#umbreld.store.getWriteLock(async ({ get, set }) => {
      ...
      recentlyOpenedApps = recentlyOpenedApps.filter(
        (appId) => appId !== this.id,
      );
  ...
  async getDiskUsage() {
    try {
      ...
      return await pRetry(() => getDirectorySize(this.dataDirectory), {
        retries: 2,
      });
    } catch (error) {
  ...
  async getContainerIp(service: string) {
    ...
    if (!containerName)
      throw new Error(
        `No container_name found for service ${service} in app ${this.id}`,
      );
  ...
  async getBackupIgnoredFilePaths() {
    ...
    for (let path of manifest.backupIgnore) {
      ...
      if (!/^[-a-zA-Z0-9._\/*]+$/.test(path)) {
        this.logger.error(
          `Invalid backupIgnore path ${path} for app ${this.id}, skipping`,
        );
  ...
  async getWidgetMetadata(widgetName: string) {
    const manifest = await this.readManifest();
    if (!manifest.widgets)
      throw new Error(`No widgets found for app ${this.id}`);

    const widgetMetadata = manifest.widgets.find(
      (widget) => widget.id === widgetName,
    );
    if (!widgetMetadata)
      throw new Error(`Invalid widget ${widgetName} for app ${this.id}`);
  ...
  async getWidgetData(widgetId: string) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          `An unexpected error occured while fetching data from ${url}: ${error}`,
        );
      }
    }
  }
  ...
  async getDependencies() {
    ...
    return (
      dependencies?.map(
        (dependencyId) => selectedDependencies?.[dependencyId] ?? dependencyId,
      ) ?? []
    );
  }
  ...
  async setSelectedDependencies(selectedDependencies: Record<string, string>) {
    ...
    const success = await this.store.set(
      "dependencies",
      filledSelectedDependencies,
    );
    if (success) {
