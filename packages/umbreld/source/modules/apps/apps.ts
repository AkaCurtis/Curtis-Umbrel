...
export default class Apps {
  ...
  async cleanDockerState() {
    try {
      const containerIds = (await $`docker ps -aq`).stdout
        .split("\n")
        .filter(Boolean);
  ...
  async start() {
    ...
    for (const app of this.instances) {
      const appDataDirectoryExists = await fse
        .pathExists(app.dataDirectory)
        .catch(() => false);
      if (!appDataDirectoryExists) {
        this.logger.error(
          `App ${app.id} does not have a data directory, removing from instances`,
        );
        this.instances = this.instances.filter(
          (instanceApp) => instanceApp.id !== app.id,
        );
      ...
      await Promise.all(
        images.map(async (image) => {
          try {
            ...
            this.logger.error(
              `Failed to pre-load local Docker image ${image}`,
              error,
            );
          }
        }),
    ...
    try {
      ...
      this.logger.error(
        `Failed to set permissions for Tor data directory`,
        error,
      );
    }
    ...
    this.reinstallMissingAppsAfterRestore(appIdsMissingDataDir).catch((error) =>
      this.logger.error(
        "Failed to schedule app reinstalls after restore",
        error,
      ),
    );
  ...
  private async reinstallMissingAppsAfterRestore(appIds: string[]) {
    ...
    this.logger.log(
      `Detected ${appIds.length} app(s) missing a data directory after restore, reinstalling...`,
    );
    try {
      ...
      this.logger.error(
        "Exhausted retries updating app store before reinstalls",
        error,
      );
    ...
    for (const appId of appIds) {
      ...
      this.install(appId).catch((error) =>
        this.logger.error(`Failed to reinstall app ${appId}`, error),
      );
    }
  }
  ...
  async install(appId: string, alternatives?: AppSettings["dependencies"]) {
    ...
    const manifestVersionIsSupported =
      !!umbrelVersionValid &&
      semver.lte(manifestVersionValid, umbrelVersionValid);
    ...
    const filledSelectedDependencies = fillSelectedDependencies(
  ...
  async uninstall(appId: string) {
    ...
    const allDependencies = await Promise.all(
      this.instances.map((app) => app.getDependencies().catch(() => null)),
    );
    const isDependency = allDependencies.some((dependencies) =>
      dependencies?.includes(appId),
    );
    if (isDependency)
      throw new Error(
        `App ${appId} is a dependency of another app and cannot be uninstalled`,
      );
  ...
  async setTorEnabled(torEnabled: boolean) {
    ...
    try {
      ...
      if (currentTorEnabled === torEnabled) {
        throw new Error(
          `Tor is already ${torEnabled ? "enabled" : "disabled"}`,
        );
      }
  ...
  async getDependents(appId: string) {
    ...
    return allDependencies
      .filter(({ dependencies }) => dependencies.includes(appId))
      .map(({ id }) => id);
  }
