...
export const appStore = router({
  ...
  removeRepository: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.appStore.removeRepository(input.url),
    ),
});
...
export const apps = router({
  ...
  list: privateProcedure.query(async ({ ctx }) => {
    ...
    const appData = await Promise.all(
      apps.map(async (app) => {
        try {
          ...
          const showCredentialsBeforeOpen =
            hasCredentials &&
            !(await app.store.get("hideCredentialsBeforeOpen"));
          return {
            id: app.id,
            name,
            version,
          ...
          ctx.apps.logger.error(
            `Failed to read manifest for app ${app.id}`,
            error,
          );
    ...
    const appDataSortedByNames = appData.sort((a, b) =>
  ...
  install: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.apps.install(input.appId, input.alternatives),
    ),
  ...
  stop: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.apps.getApp(input.appId).stop({ persistState: true }),
    ),
  ...
  recentlyOpened: privateProcedure.query(({ ctx }) =>
    ctx.apps.recentlyOpened(),
  ),

  setTorEnabled: privateProcedure
    .input(z.boolean())
    .mutation(({ ctx, input }) => ctx.apps.setTorEnabled(input)),
  ...
  setSelectedDependencies: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.apps.setSelectedDependencies(input.appId, input.dependencies),
    ),

  dependents: privateProcedure
    .input(z.string())
    .query(async ({ ctx, input }) => ctx.apps.getDependents(input)),
  ...
  hideCredentialsBeforeOpen: privateProcedure
    ...
    .mutation(async ({ ctx, input }) =>
      ctx.apps.setHideCredentialsBeforeOpen(input.appId, input.value),
    ),

  isBackupIgnored: privateProcedure
    .input(z.object({ appId: z.string() }))
    .query(async ({ ctx, input }) =>
      ctx.apps.getApp(input.appId).isBackupIgnored(),
    ),

  backupIgnore: privateProcedure
    .input(z.object({ appId: z.string(), value: z.boolean() }))
    .mutation(async ({ ctx, input }) =>
      ctx.apps.getApp(input.appId).setBackupIgnored(input.value),
    ),
  ...
  getBackupIgnoredPaths: privateProcedure
    ...
    .query(async ({ ctx, input }) =>
      ctx.apps.getApp(input.appId).getBackupIgnoredFilePaths(),
    ),
});
