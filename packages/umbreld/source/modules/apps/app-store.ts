...
export default class AppStore {
  ...
  constructor(
    umbreld: Umbreld,
    { defaultAppStoreRepo }: { defaultAppStoreRepo: string },
  ) {
  ...
  async start() {
    ...
    if ((await this.#umbreld.store.get("appRepositories")) === undefined) {
      await this.#umbreld.store.set("appRepositories", [
        this.defaultAppStoreRepo,
      ]);
    }
    ...
    try {
      ...
      if (!defaultRepository)
        throw new Error(
          `Default repository ${this.defaultAppStoreRepo} not found`,
        );
      await pRetry(
        async () => {
          await defaultRepository
            .update()
            .finally(() => (this.attemptedInitialAppStoreUpdate = true));
        },
        {
    ...
    this.logger.log(
      `Checking repositories for updates every ${this.updateInterval}`,
    );
    this.#stopUpdating = runEvery(this.updateInterval, () => this.update(), {
      runInstantly: true,
    });
  }
  ...
  async getRepositories() {
    ...
    const repositories = repositoryUrls.map(
      (url) => new AppRepository(this.#umbreld, url),
    );
  ...
  async getDefaultRepository() {
    ...
    return repositories.find(
      (repository) => repository.url === this.defaultAppStoreRepo,
    );
  }
  ...
  async registry() {
    ...
    const registryPromises = repositories.map((repository) =>
      repository.readRegistry().catch((error) => {
        this.logger.error(
          `Failed to read registry from ${repository.url}`,
          error,
        );
        return null;
      }),
    ...
    return registry.filter(Boolean) as Array<
      Awaited<ReturnType<typeof AppRepository.prototype.readRegistry>>
    >;
  }
  ...
  async removeRepository(url: string) {
    ...
    const existingRepositories = await this.getRepositories();
    if (
      !existingRepositories.some((existingRepo) => existingRepo.url === url)
    ) {
    ...
    await this.#umbreld.store.getWriteLock(async ({ get, set }) => {
      const repositoryUrls = await get("appRepositories");
      const updatedRepositoryUrls = repositoryUrls.filter(
        (repoUrl) => repoUrl !== url,
      );
  ...
  async getAppTemplateFilePath(appId: string) {
    ...
    for (const repo of registry) {
      ...
      if (app) {
        ...
        const repositories = await this.getRepositories();
        const repoPath = repositories.find(
          (repository) => repository.url === repo.url,
        )!.path;

        if (!repoPath)
          throw new Error(`Repository path not found for ${repo.url}`);
