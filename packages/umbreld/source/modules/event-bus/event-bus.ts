...
type _AssertEveryKeyIsListed = MissingInEvents extends never
  ? true
  : [`✘ Add these to events →`, MissingInEvents];
...
export default class EventBus {
  ...
  // Emit an event
  emit: Emittery<EventTypes>["emit"] = (
    event: keyof EventTypes,
    data?: EventTypes[keyof EventTypes],
  ) => {
    this.logger.verbose(
      `${event} ${data === undefined ? "" : JSON.stringify(data)}`,
    );
