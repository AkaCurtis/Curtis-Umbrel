...
function toFetchableUrl(url: string): string {
  return url.startsWith("umbrel:")
    ? `${url.replace("umbrel:", "http://localhost:")}`
    : url;
}

export default router({
  // Fetch metadata (title + icon URL) from a URL
  fetchPageMetadata: privateProcedure
  ...
  add: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.umbreld.store.getWriteLock(async ({ get, set }) => {
        ...
        if (shortcuts.some((s) => s.url === input.url))
  ...
  remove: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.umbreld.store.getWriteLock(async ({ get, set }) => {
        ...
        if (!shortcuts.find((shortcut) => shortcut.url === input.url))
          throw new Error("[shortcut-not-found]");

        // Remove the shortcut
        const updatedShortcuts = shortcuts.filter(
          (shortcut) => shortcut.url !== input.url,
        );
