...
export async function fetchPageMetadata(
  url: string,
): Promise<{ title: string; icon: string | undefined }> {
  ...
  const head = html
    ? (html.match(/<head\b[^>]*>([\s\S]*?)<\/head>/i)?.[1] ?? html)
    : undefined;

  const icon = head
    ? (findAppleTouchIcon(head, base) ??
      (await fetchManifestIcon(findManifestHref(head, base)).catch(
        () => undefined,
      )) ??
...
function findTitle(head: string) {
  ...
  if (index !== -1) {
    ...
    if (left && right)
      return (left.length <= right.length ? left : right).slice(0, 50);
  }
...
async function fetchManifestIcon(href: string | undefined) {
  ...
  const withAny = icons.filter((i) =>
    (i.purpose ?? "any").toLowerCase().includes("any"),
  );
...
async function probeAppleTouchIcon(base: URL) {
  const response = await doFetch(
    new URL("/apple-touch-icon.png", base),
    "image/*,*/*;q=0.8",
  );
  if (!response) return undefined;
  const ct = response.headers.get("content-type") ?? "";
