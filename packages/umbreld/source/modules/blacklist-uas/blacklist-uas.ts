...
export default async function blacklistUASDriver() {
  try {
    ...
    const systemInfo = JSON.parse(systemInfoJson) as {
      boot?: { activeGroup?: string };
    };
    // Rugix's rpi-tryboot layout uses partition 2 for boot-a and partition 3 for boot-b.
    const bootPartition =
      systemInfo.boot?.activeGroup === "a"
        ? 2
        : systemInfo.boot?.activeGroup === "b"
          ? 3
          : undefined;
    if (!bootPartition)
      throw new Error("Could not determine active boot partition");
    const { stdout } =
      await $`rugix-ctrl utils resolve-partition ${bootPartition}`;
    const { device } = JSON.parse(stdout) as { device?: string };
    if (!device)
      throw new Error(`Could not resolve boot partition ${bootPartition}`);
    ...
    try {
      ...
      if (await fse.pathExists(justDidRebootFile)) {
        console.error(
          "We just rebooted, we could be in a bootloop, skipping reboot",
        );
        await fse.remove(justDidRebootFile);
        return;
      }
      ...
      const quirksAlreadyApplied = blacklist.every((deviceId) =>
        cmdline.includes(`${deviceId}:u`),
      );
    ...
    console.error(
      `Failed to blacklist UAS driver: ${(error as Error).message}`,
    );
  }
}
