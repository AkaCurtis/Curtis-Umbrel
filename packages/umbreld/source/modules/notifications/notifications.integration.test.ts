...
test.sequential(
  "notifications.get() throws invalid error without auth token",
  async () => {
    await expect(umbreld.client.notifications.get.query()).rejects.toThrow(
      "Invalid token",
    );
  },
);
...
test.sequential(
  "notifications.get() lists nothing on a fresh install",
  async () => {
    await expect(
      umbreld.client.notifications.get.query(),
    ).resolves.toMatchObject([]);
  },
);

test.sequential(
  "notifications.add(notification) adds a notification",
  async () => {
    await umbreld.instance.notifications.add("test notification");
    await expect(
      umbreld.client.notifications.get.query(),
    ).resolves.toMatchObject(["test notification"]);
  },
);

test.sequential(
  "notifications.clear(notification) clears a notification",
  async () => {
    await expect(
      umbreld.client.notifications.get.query(),
    ).resolves.toMatchObject(["test notification"]);
...
test.sequential(
  "notifications.add(notification) moves duplicate notifications to front",
  async () => {
