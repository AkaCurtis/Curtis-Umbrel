...
export default class Umbreld {
  ...
  constructor({
    ...
    this.store = new FileStore<StoreSchema>({
      filePath: `${dataDirectory}/umbrel.yaml`,
    });
  ...
  async start() {
    ...
    if (!this.developmentMode) {
      await this.apps
        .cleanDockerState()
        .catch((error) =>
          this.logger.error(`Failed to clean Docker state`, error),
        );
    }
  ...
  private async setBackupRestoreFirstStartFlag() {
    try {
      ...
      this.logger.error(
        "Failed checking backup restore first-start flag",
        error,
      );
    }
  }
